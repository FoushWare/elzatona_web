...
export default function ProblemSolvingQuestion({
  ...
  const [editingTestCaseIndex, setEditingTestCaseIndex] = useState<
    number | null
  >(null); // Full index of test case being edited
  const [editingTestCaseData, setEditingTestCaseData] =
    useState<Partial<TestCase> | null>(null); // Data being edited
  ...
  const handleSaveTestCase = () => {
    if (
      ...
      editingTestCaseIndex >= baseTestCases.length
    ) {
      // Custom test case
      ...
      try {
        ...
        if (!data.success) {
          results.push({
            ...
            consoleOutput:
              data.data?.stderr || data.error || "Execution failed",
          });
          continue;
        }
        ...
        const fullConsoleOutput = [stdout, stderr]
          .filter(Boolean)
          .join("\n")
          .trim();
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 px-5 sm:px-6 py-4">
            <div className="flex items-center justify-between sm:justify-start gap-2 sm:gap-4">
              <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-xl bg-gradient-to-br from-indigo-50 to-white dark:from-indigo-900/30 dark:to-gray-700/50 border-2 border-indigo-200/80 dark:border-indigo-700/80 shadow-md">
                ...
                <span className="text-sm sm:text-base font-bold text-indigo-900 dark:text-indigo-200 tracking-tight">
                  Code
                </span>
              </div>
              {isFocusMode && (
