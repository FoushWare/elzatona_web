...
export async function POST(request: NextRequest) {
  try {
    ...
    const token = cookieStore.get("firebase_token")?.value || request.headers.get("authorization")?.replace("Bearer ", "");
...
async function _resolveUserFromData(data: any) {
  ...
  const { data: user, error } = await getSupabaseClient().from("users").select("id, email, role, name").eq("id", data.userId).single();
  return !error && user ? { id: user.id, email: user.email, role: user.role, name: user.name } : null;
}

async function _syncToDatabase(userId: string, data: GuidedProgress) {
  return getSupabaseClient().from("user_progress").upsert({
    user_id: userId,
    plan_id: data.planId,
    progress_data: {
