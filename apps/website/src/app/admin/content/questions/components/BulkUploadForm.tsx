                  ...
                  allSpans.forEach((el) => {
                    ...
                    if (colorMatch) {
                      ...
                      if (text.trim().length > 0) {
                        (el as HTMLElement).setAttribute('style', `${style ? style + '; ' : ''}color: #24292e;`);
                      }
                    } else if (!style) {
                      const text = (el as HTMLElement).textContent || '';
                      if (text.trim().length > 0) {
    ...
    try {
      ...
      if (err instanceof SyntaxError) {
        ...
        if (match) {
          ...
          setJsonError(`JSON Error at line ${lineNumber}, column ${columnNumber}: ${err.message}`);
        } else {
    ...
    if (isJsonMode) {
      ...
      try {
        ...
        if (questions.length === 0) {
          setJsonError('JSON is valid but contains no questions. Please add at least one question.');
          return;
        }
        
        // Create a File object from cleaned JSON text
        const blob = new Blob([cleanedJson], { type: 'application/json' });
        const jsonFile = new File([blob], 'questions.json', { type: 'application/json' });
        onUpload(jsonFile);
      } catch (err: any) {
        ...
        if (err instanceof SyntaxError) {
          ...
          if (match) {
            ...
            const afterError = jsonText.substring(position, Math.min(jsonText.length, position + 50));
    ...
    <form onSubmit={handleSubmit} className='space-y-6 p-4'>
      ...
      <div className='flex items-center justify-between p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50'>
        <div className='flex items-center gap-3'>
          <Label htmlFor='json-mode' className='text-base font-semibold cursor-pointer'>
            Use JSON Format
          </Label>
      ...
      {isJsonMode ? (
        <div className='space-y-2'>
          ...
          <p className='text-xs text-gray-500 dark:text-gray-400'>
            Paste your questions array as JSON. The JSON will be validated before submission.
          </p>
          {jsonError && (
            <div className='mt-2 p-3 bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 rounded-lg'>
              <div className='flex items-start gap-3'>
                ...
                <div className='flex-1'>
                  ...
                  <p className='text-sm whitespace-pre-wrap font-mono text-xs'>{jsonError}</p>
                  <p className='text-xs mt-2 text-red-600 dark:text-red-400'>
                    Tip: Make sure all control characters (newlines, tabs) in string values are properly escaped as \\n, \\t, etc.
                  </p>
                </div>
              </div>
            </div>
                      ...
                      <div className='space-y-1'>
                        ...
                        <div className={`relative rounded-lg overflow-hidden shadow-lg border ${
                          ...
                          <div className={`overflow-x-auto ${codeTheme === 'dark' ? 'bg-gray-900' : 'bg-white border border-gray-200'}`}>
                            {isLoadingShiki ? (
                              <div className="p-2 text-center text-xs text-gray-500">Loading syntax highlighting...</div>
                            ) : highlightedHtml ? (
                                  ...
                                  <code className="block">
                                    {codeLines.map((line, index) => (
                                      <div key={index} className="flex items-start">
                                        ...
                                        <span className={`flex-1 whitespace-pre pl-2 pr-2 py-0.5 text-xs ${
      ...
      <DialogFooter className='gap-3 pt-4'>
        ...
        <Button
          type='submit'
          disabled={(!file && !isJsonMode) || (isJsonMode && !jsonText.trim()) || loading}
