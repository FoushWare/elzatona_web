  ...
  try {
    ...
    const cleanOptionText = (text: string): string => {
      ...
      for (let pass = 0; pass < 5; pass++) {
        cleaned = cleaned
          ...
          .replace(/\s*>$/g, '')
          .replace(/\s+>\s+/g, ' ')
          .replace(/([a-zA-Z0-9])>\s*([a-zA-Z0-9])/g, (match, before, after) => {
    ...
    const cleanContent = (text: string): string => {
      ...
      while ((preMatch = preBlockRegex.exec(cleaned)) !== null) {
        if (preMatch.index > lastIndex) {
          ...
          const processedText = textBeforePre.replace(/<code[^>]*>([^<]+)<\/code>/gi, (_, codeContent) => {
            return `\`${decodeHtmlEntities(codeContent).trim()}\``;
          });
      ...
      if (lastIndex < cleaned.length) {
        ...
        const processedText = textAfterPre.replace(/<code[^>]*>([^<]+)<\/code>/gi, (_, codeContent) => {
          return `\`${decodeHtmlEntities(codeContent).trim()}\``;
        });
        parts.push(processedText);
      }
      ...
      cleaned = cleaned.replace(/<pre[^>]*><code[^>]*>([\s\S]*?)<\/code><\/pre>/gi, (match, codeContent) => {
    ...
    if (questions && questions.length > 0) {
      questions = questions.map(q => {
        ...
        const cleanedTitle = (q.title && typeof q.title === 'string') ? cleanContent(q.title) : (q.title || '');
        const cleanedContent = (q.content && typeof q.content === 'string') ? cleanContent(q.content) : (q.content || '');
        const cleanedCorrectAnswer = (q.correct_answer && typeof q.correct_answer === 'string') ? cleanOptionText(q.correct_answer) : (q.correct_answer || '');
        const cleanedExplanation = (q.explanation && typeof q.explanation === 'string') ? cleanOptionText(q.explanation) : (q.explanation || '');
    ...
    (questions || []).forEach(question => {
        ...
        if (categoryId) {
          ...
          console.error(`❌ Error processing question ${question?.id || 'unknown'}:`, questionError);
      }
    });
    ...
    const cardsWithData = (cards || []).map(card => {
      ...
      const cardCategories = (categories || []).filter(category => {
        ...
        // Match by learning_card_id first, then by card_type as fallback
            ...
            console.log(`🔍 Category matching for card "${card?.title || 'unknown'}":`, {
        ...
        console.log(`🔍 Plan Details Debug: Card "${card?.title || 'unknown'}" (${card?.id || 'unknown'})`, {
          cardType: card?.type,
          cardId: card?.id,
      ...
      const categoriesWithTopics = cardCategories.map(category => {
        ...
        return {
          ...
          questionCount: topicsWithQuestions.reduce(
            ...
            console.error(`❌ Error processing category "${category?.name || category?.id || 'unknown'}":`, categoryError);
            return {
              ...category,
              topics: [],
              questionCount: 0,
      ...
      return {
        ...
        questionCount: categoriesWithTopics.reduce(
          ...
          hasQuestions: categoriesWithTopics.some(cat => (cat?.questionCount || 0) > 0),
        };
      } catch (cardError) {
        console.error(`❌ Error processing card "${card?.title || card?.id || 'unknown'}":`, cardError);
    ...
    console.log('🔍 Plan Details Debug: Final result:', {
      ...
      totalQuestions,
      cardsWithQuestions: cardsWithData.filter(c => (c?.questionCount || 0) > 0).length,
    ...
    try {
      ...
      data: {
        ...
        // Legacy compatibility - create sections from cards for backward compatibility
          sections: cardsWithData.map((card, index) => {
            try {
              ...
              return {
                ...
                weight: totalQuestions > 0 ? Math.round((questionCount / totalQuestions) * 100) : 0,
          order: index + 1,
          ...
          // Add card-specific metadata
              ...
              console.error(`❌ Error creating section for card ${card?.id || index}:`, sectionError);
              return {
                id: card?.id || `card-${index}`,
                name: card?.title || 'Unknown Card',
                category: card?.type || 'unknown',
      ...
      const errorMessage = responseError instanceof Error ? responseError.message : 'Unknown error';
      return NextResponse.json(
        {
          success: false,
