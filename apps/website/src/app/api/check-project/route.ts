...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(`[Check Project API] Admin access: ${decoded.email || decoded.adminId || 'unknown'}`);
    } catch (error) {
    ...
    const urlProject = projects[urlProjectRef || ''] || { name: 'unknown', type: 'UNKNOWN' };
    const keyProject = projects[keyProjectRef || ''] || { name: 'unknown', type: 'UNKNOWN' };
    ...
    return NextResponse.json({
      ...
      status: {
        isTestProject,
        isProductionProject,
        isCorrectlyConfigured: isMatch && (isTestProject || isProductionProject),
        warning: !isMatch ? 'URL and Service Role Key point to different projects!' : null,
      },
      message: isMatch && isTestProject
