...
export function CodeToImageTest({ code, language = 'javascript', theme: initialTheme = 'dark' }: CodeToImageTestProps) {
  ...
  useEffect(() => {
    ...
    try {
      const lang = language === 'typescript' ? 'ts' : 
  ...
  const highlightSyntaxFallback = (line: string) => {
    ...
    const keywords = /\b(function|const|let|var|if|else|return|for|while|do|switch|case|break|continue|try|catch|finally|throw|new|this|class|extends|import|export|default|async|await|static|public|private|protected|interface|type|enum)\b/g;
    
    // Built-in functions/objects (light yellow/orange)
    const builtins = /\b(console|Math|Object|Array|String|Number|Boolean|Date|Promise|fetch|parseInt|parseFloat|isNaN|isFinite|JSON|localStorage|sessionStorage|document|window|prompt|alert|confirm)\b/g;
    ...
    while ((match = builtins.exec(text)) !== null) {
      matches.push({ start: match.index, end: match.index + match[0].length, type: 'builtin' });
    }
    ...
    for (const m of matches) {
      if (m.type === 'string' || !filteredMatches.some(fm => m.start < fm.end && m.end > fm.start)) {
    ...
    for (const m of filteredMatches) {
      if (m.start > lastIndex) {
        parts.push({ text: text.substring(lastIndex, m.start), type: 'normal' });
      }
    ...
    return (
      <>
        {parts.map((part, idx) => {
          const className = 
            part.type === 'keyword' ? 'text-cyan-400 dark:text-cyan-300' :
            part.type === 'builtin' ? 'text-yellow-500 dark:text-yellow-400' :
            part.type === 'string' ? 'text-green-500 dark:text-green-400' :
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <div className={`flex items-center justify-between px-4 sm:px-6 py-3 border-b-2 ${
          theme === 'dark'
          ...
          <div className="flex items-center gap-3">
            ...
            {/* File name */}
            <div className={`flex items-center gap-2 ml-2 px-3 py-1 rounded-md border ${
              theme === 'dark'
              ...
              <span className={`text-xs sm:text-sm font-semibold font-mono ${
                ...
                code.{language === 'python' ? 'py' : language === 'java' ? 'java' : language === 'typescript' ? 'ts' : 'js'}
              </span>
            </div>
          </div>
          {/* Language badge */}
        ...
        <div className={`overflow-x-auto ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
          {isLoadingHighlighter ? (
            <div className="p-4 text-center text-gray-500">Loading syntax highlighter...</div>
          ) : highlightedHtml ? (
            <div className="relative">
              ...
              <div className="relative">
                ...
                <div className="absolute left-0 top-0 flex flex-col" style={{ paddingTop: '1rem' }}>
                  {code.split('\n').map((_, index) => (
                    <span 
                      key={index}
              ...
              <code className="block text-gray-900 dark:text-gray-100">
                {code.split('\n').map((line, index) => (
                  <div key={index} className="flex items-start">
                    <span className={`select-none pr-4 text-right min-w-[4rem] text-sm sm:text-base font-semibold ${
                      theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
                    }`}>
                      {index + 1}
                    </span>
                    <span className={`flex-1 whitespace-pre pl-2 pr-4 py-0.5 ${
      ...
      {imageUrl && (
        <div className="mt-4">
          <h3 className="text-sm font-semibold mb-2">Generated Image Preview:</h3>
          <img
            src={imageUrl}
