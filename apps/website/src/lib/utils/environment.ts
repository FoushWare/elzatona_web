...
export function getEnvironment(): Environment {
  ...
  if (nextAppEnv === 'development' || nextAppEnv === 'dev') return 'development';
  ...
  if (supabaseUrl.includes(TEST_PROJECT_REF) || supabaseUrl.includes(OLD_TEST_PROJECT_REF_1) || supabaseUrl.includes(OLD_TEST_PROJECT_REF_2)) {
    return 'test';
  }
...
export function logEnvironment(context?: string): void {
  ...
  if (info.isTest) {
    ...
    console.log(`${prefix} 🚀 Running in PRODUCTION mode - using production database`);
  } else {
