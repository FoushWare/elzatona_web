...
import { getEnvironment, isTestEnvironment, isProductionEnvironment, isDevelopmentEnvironment } from './environment';
...
export function getApiConfig(): ApiConfig {
  ...
  if (isTest) {
    ...
    const isTestProject = supabaseUrl.includes('kiycimlsatwfqxtfprlr') || 
    ...
    if (isProdProject) {
      console.error('[API Config] ⚠️  WARNING: APP_ENV=test but using PRODUCTION project!');
      console.error('[API Config] Expected: kiycimlsatwfqxtfprlr (zatona-web-testing)');
      console.error('[API Config] Actual:', supabaseUrl);
      console.error('[API Config] Please restart the dev server after updating .env.test.local');
    }
  }
  ...
  const adminEmail = (isTest || isDev)
    ? (process.env.ADMIN_EMAIL || process.env.INITIAL_ADMIN_EMAIL || '')
    : (process.env.INITIAL_ADMIN_EMAIL || '');
  
  const adminPassword = (isTest || isDev)
...
export function logApiConfig(context?: string): void {
  ...
  console.log(`${prefix} Debug Logging: ${config.enableDebugLogging ? 'ON' : 'OFF'}`);
  console.log(`${prefix} Test Data: ${config.enableTestData ? 'ENABLED' : 'DISABLED'}`);
}
