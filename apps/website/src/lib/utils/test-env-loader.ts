...
export function loadTestEnvironment(): {
  ...
  if (process.env.DEBUG_TEST_ENV === 'true' || isCI) {
    ...
    if (loadedFiles.length > 0) {
      console.log(`[Test Env Loader] Loaded files: ${loadedFiles.map(f => f.split('/').pop()).join(', ')}`);
    }
    if (process.env.ADMIN_EMAIL) {
      console.log(`[Test Env Loader] Admin email: ${process.env.ADMIN_EMAIL.substring(0, 10)}...`);
    }
  }
