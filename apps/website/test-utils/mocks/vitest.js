...
module.exports = {
  ...
  vi: {
    ...
    fn: (impl) => (typeof globalObj.jest?.fn === 'function' ? globalObj.jest.fn(impl) : () => {}),
    spyOn: (obj, method) => (typeof globalObj.jest?.spyOn === 'function' ? globalObj.jest.spyOn(obj, method) : { mockImplementation: () => {} }),
    clearAllMocks: () => (globalObj.jest?.clearAllMocks ? globalObj.jest.clearAllMocks() : undefined),
    resetAllMocks: () => (globalObj.jest?.resetAllMocks ? globalObj.jest.resetAllMocks() : undefined),
    // Map vitest.mocked to jest.mocked where available (Jest 29+)
    mocked: (v) => (typeof globalObj.jest?.mocked === 'function' ? globalObj.jest.mocked(v) : v),
    // Utility helpers commonly used in Vitest tests
    isMockFunction: (fn) => (typeof globalObj.jest?.isMockFunction === 'function' ? globalObj.jest.isMockFunction(fn) : false),
    mockReturnValue: (mock, v) => {
      if (mock && typeof mock.mockReturnValue === 'function') return mock.mockReturnValue(v);
      if (mock && typeof mock.mockResolvedValue === 'function') return mock.mockResolvedValue(v);
      return undefined;
    },
