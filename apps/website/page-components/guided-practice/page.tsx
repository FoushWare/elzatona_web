...
function GuidedPracticePageContent() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(
        "Code after blank line removal:",
        JSON.stringify(codeWithNewlines),
      );
        ...
        try {
          ...
          if (preElement) {
            ...
            if (codeElement) {
              ...
              if (!prefersDark) {
                ...
                if (!preStyle.includes("background")) {
                  preElement.setAttribute(
                    "style",
                    `${preStyle ? preStyle + "; " : ""}background-color: #ffffff;`,
                  );
                }
              }
              ...
              console.log(
                `Removed ${removedCount} empty lines from Shiki HTML`,
              );
              if (!prefersDark) {
    ...
    const fetchPlanData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to fetch plan data: ${response.status} - ${errorText.substring(0, 200)}`,
          );
        }
        ...
        if (savedProgress) {
          ...
          saveProgress(newProgress);
          console.log(
            "🔍 Initializing new progress, finding first question with filters:",
            {
              cardId,
              categoryId,
              planId,
  ...
  const resumeFromProgress = (planData: Plan, savedProgress: Progress) => {
    ...
    if (currentPosition.cardIndex < planData.cards.length) {
      ...
      if (cardId && card.id !== cardId) {
        console.log(
          "⏭️ Saved position card does not match cardId filter, skipping resume",
        );
      ...
      if (currentPosition.categoryIndex < card.categories.length) {
        ...
        if (categoryId && category.id !== categoryId) {
          console.log(
            "⏭️ Saved position category does not match categoryId filter, skipping resume",
          );
          // Don't resume - find first question with filter instead
          findFirstQuestion(planData);
          return;
        ...
        if (currentPosition.topicIndex < category.topics.length) {
          ...
          if (
            ...
            if (hasOptions || isCodeQuestion) {
              const isCompleted = savedProgress.completedQuestions.includes(
                question.id,
              );
              ...
              if (latestProgress) {
                ...
                saveProgress(updatedProgress);
                console.log(
                  "💾 Confirmed saved position for resume:",
                  currentPosition,
                );
              }
    ...
    console.log(
      "⚠️ Cannot resume from saved position, finding first question with filters",
    );
  ...
  const findFirstQuestion = (planData: Plan) => {
    ...
    for (const cardTitle of cardOrder) {
      ...
      if (cardIndex !== -1) {
        ...
        const actualCardIndex = planData.cards.findIndex(
          (c) => c.id === card.id,
        );
        ...
        for (
          ...
          if (categoryId && category.id !== categoryId) {
            console.log(
              "⏭️ Skipping category (does not match categoryId filter):",
              {
                categoryName: category.name,
          ...
          for (
            ...
            if (topic.questions && topic.questions.length > 0) {
              ...
              const questionWithOptions = topic.questions.find((q) => {
                const hasOptions =
    ...
    if (!foundQuestion) {
      if (cardId || categoryId) {
        console.warn("⚠️ No questions found in the specified card/category:", {
          cardId,
          categoryId,
          cardsSearched: cardsToSearch.map((c) => ({
            id: c.id,
            title: c.title,
          })),
        });
      ...
      console.log(
        "⚠️ No question found in ordered search, trying fallback (no card/category filter)...",
      );
    ...
    if (foundQuestion) {
      ...
      if (cardId && foundCard?.id !== cardId) {
        console.error("❌ ERROR: Question found from wrong card!", {
          ...
          requestedCardId: cardId,
          requestedCardTitle: planData.cards.find((c) => c.id === cardId)
            ?.title,
          questionId: foundQuestion.id,
        });
        console.warn(
          "⚠️ Not showing question - it belongs to a different card",
        );
        setError(
          `No questions found in the selected card. Please try a different card or category.`,
        );
        setIsLoading(false);
      ...
      if (categoryId && foundCategory?.id !== categoryId) {
        ...
        console.warn(
          "⚠️ Not showing question - it belongs to a different category",
        );
        setError(
          `No questions found in the selected category. Please try a different category.`,
        );
        setIsLoading(false);
  ...
  const proceedToNext = () => {
    ...
    if (currentQuestionIndex + 1 < currentTopic.questions.length) {
      ...
      const hasOptions =
        ...
        Array.isArray(nextQuestion.options) &&
    ...
    if (currentCategoryIndex + 1 < currentCard.categories.length) {
      ...
      for (
        let topicIndex = 0;
        topicIndex < nextCategory.topics.length;
        topicIndex++
      ) {
    ...
    if (currentCardIndex + 1 < plan.cards.length) {
      ...
      for (
        let categoryIndex = 0;
        categoryIndex < nextCard.categories.length;
        categoryIndex++
      ) {
  ...
  const shuffleOptions = React.useCallback(
    (
  ...
  const isCorrectAnswer = (option: string) => {
    ...
    if (currentQuestion.options && Array.isArray(currentQuestion.options)) {
      ...
      if (optionObj) {
        ...
        const cleanedCorrectAnswer = cleanOptionText(
      ...
      if (
        correctOption &&
        cleanOptionText(correctOption.text) === cleanedOption
      ) {
        return true;
      }
    }
  ...
  const getCurrentQuestionNumber = () => {
    ...
    for (let cardIdx = 0; cardIdx < plan.cards.length; cardIdx++) {
      ...
      for (
        let categoryIdx = 0;
        categoryIdx < card.categories.length;
        categoryIdx++
      ) {
        ...
        for (let topicIdx = 0; topicIdx < category.topics.length; topicIdx++) {
          ...
          if (topic.questions && topic.questions.length > 0) {
            for (
              let questionIdx = 0;
              questionIdx < topic.questions.length;
              questionIdx++
            ) {
        ...
        <div className="max-w-2xl mx-auto w-full">
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-xl border-2 border-white/20 dark:border-gray-700/20 text-center">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-xl p-4 sm:p-5 lg:p-6 mb-4 sm:mb-6 lg:mb-8 border border-blue-200 dark:border-blue-800">
              ...
              <div
                className={`text-lg sm:text-xl lg:text-2xl font-semibold mb-3 sm:mb-4 ${gradeColor}`}
              >
                {gradeText}
              </div>
  ...
  return (
    <div
      className={`min-h-screen ${isCodeQuestion ? "bg-gray-50 dark:bg-gray-900" : "bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950"} pt-16 sm:pt-20 lg:pt-24 pb-4 sm:pb-6 lg:pb-8`}
    >
      <div
        className={
          isCodeQuestion
      ...
      >
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-3 sm:mb-4 pb-3 sm:pb-4 border-b border-gray-100 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2.5 sm:gap-3 flex-wrap">
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 text-xs sm:text-sm font-medium ml-2 sm:ml-3 pl-2 sm:pl-3 border-l border-gray-300 dark:border-gray-600">
                    <span className="truncate px-3 py-1.5 rounded-lg bg-gradient-to-r from-indigo-100 to-indigo-200 dark:from-indigo-900/40 dark:to-indigo-900/30 text-indigo-800 dark:text-indigo-200 border border-indigo-300/60 dark:border-indigo-700/60 shadow-sm font-semibold">
                      {currentCard?.icon} {currentCard?.title}
                    </span>
                    {/* Only show category if it's different from card title */}
                    {currentCategory?.name &&
                      currentCategory.name !== currentCard?.title && (
                        <>
                          <span className="hidden sm:inline text-indigo-500 dark:text-indigo-400 font-bold">
                            →
                          </span>
                          <span className="truncate px-3 py-1.5 rounded-lg bg-gradient-to-r from-purple-100 to-purple-200 dark:from-purple-900/40 dark:to-purple-900/30 text-purple-800 dark:text-purple-200 border border-purple-300/60 dark:border-purple-700/60 shadow-sm font-semibold">
                            {currentCategory?.name}
                          </span>
                        </>
                      )}
                    ...
                    {currentTopic?.name && (
                      <>
                        <span className="hidden sm:inline text-purple-500 dark:text-purple-400 font-bold">
                          →
                        </span>
                        <span className="truncate px-3 py-1.5 rounded-lg bg-gradient-to-r from-pink-100 to-pink-200 dark:from-pink-900/40 dark:to-pink-900/30 text-pink-800 dark:text-pink-200 border border-pink-300/60 dark:border-pink-700/60 shadow-sm font-semibold">
                          {currentTopic?.name}
                        </span>
                      </>
                    )}
        ...
        <div
          className={
            isCodeQuestion
              ? ""
              : "bg-white dark:bg-gray-800 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-10 shadow-2xl border-2 border-gray-200 dark:border-gray-700 mb-8 transition-all duration-300"
          }
        >
          ...
          {currentQuestion.type === "code" ? (
              ...
              question={
                {
                  ...
                  topic_description:
                    ...
                    currentQuestion.topic_description,
            ...
            <>
                ...
                <div className="space-y-4 mb-6 sm:mb-8">
                  ...
                  {currentQuestion.code_template &&
                    (() => {
                      ...
                      const formattedCode =
                        formatCodeForDisplay(codeWithNewlines);
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-4 sm:space-x-5">
                          ...
                          <span className="flex-1 capitalize text-base sm:text-lg font-semibold text-left">
                            {option}
                          </span>
                          {isCorrect && (
                            <div className="flex-shrink-0 animate-in fade-in slide-in-from-right-2 duration-300">
              ...
              {currentQuestion.type !== "code" &&
                ...
                Array.isArray(currentQuestion.resources) &&
