...
export async function setupAdminPage(
  ...
  const emailStatus = process.env.ADMIN_EMAIL
    ...
    : "❌ Not set";
  const passwordLength = process.env.ADMIN_PASSWORD
  ...
  if (!adminEmail || adminEmail.length === 0) {
    throw new Error(
      "ADMIN_EMAIL is set but empty. Please check .env.test.local",
    );
  }
  if (!adminPassword || adminPassword.length === 0) {
    throw new Error(
      "ADMIN_PASSWORD is set but empty. Please check .env.test.local",
    );
  }
  ...
  if (emailDisabled || passwordDisabled) {
    throw new Error(
      `Login form inputs are disabled. Email disabled: ${emailDisabled}, Password disabled: ${passwordDisabled}`,
    );
  }
  ...
  let response: Response | null = apiResponse;
  ...
  if (navigatedToDashboard && !response) {
    console.log(
      "✅ Navigation to dashboard detected - login likely succeeded (API response not captured)",
    );
    ...
    if (!page.isClosed()) {
      try {
        ...
        console.log(
          ...
          allRequests.length > 0 ? allRequests.join(", ") : "none",
        );

        // Try one final wait for the response
        try {
          const finalResponse: Response = await page.waitForResponse(
            (response) => {
              const url = response.url();
              return (
                url.includes("/api/admin/auth") &&
  ...
  if (navigatedToDashboard && !response) {
    ...
    try {
      ...
      if (!responseData.success) {
        ...
        throw new Error(
          `Login API failed: ${errorMsg}. Check ADMIN_EMAIL and ADMIN_PASSWORD in .env.test.local`,
        );
      }
      ...
      if (errorBoxCount > 0) {
        ...
        throw new Error(
          `Login failed: ${errorText}. Check ADMIN_EMAIL and ADMIN_PASSWORD in .env.test.local`,
        );
      }
      throw e;
    }
  ...
  if (!currentURLAfterLogin && page.isClosed()) {
    throw new Error(
      "Page was closed after successful login and could not recover.",
    );
  }
  ...
  console.log(
    ...
    currentURLAfterLogin || "(could not determine - page might be navigating)",
  );
  ...
  if (
    ...
    throw new Error(
      `Page navigated to error page: ${currentURLAfterLogin}. Dev server may have crashed.`,
    );
  }
  ...
  if (currentURLAfterLogin.includes("/admin/content/questions")) {
    ...
    if (
      ...
      await page.waitForURL(/.*admin\/(dashboard|content).*/, {
        timeout: 15000,
      });
    }
    ...
    if (currentURLAfterLogin.includes("/admin/dashboard")) {
      ...
      try {
        ...
        if (
          ...
          navErr.message.includes("net::ERR_CONNECTION_REFUSED")
        ) {
          throw new Error(
        ...
        if (!page.isClosed()) {
          ...
          throw new Error(
            `Page was closed during navigation to questions page: ${navErr.message}`,
          );
        }
      }
    }
  ...
  console.log(
    ...
    questionsPageURL || "(could not determine)",
  );

  if (
    questionsPageURL &&
    !questionsPageURL.includes("/admin/content/questions")
  ) {
    throw new Error(
