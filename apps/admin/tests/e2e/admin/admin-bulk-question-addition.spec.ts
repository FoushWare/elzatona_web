...
test.describe("A-E2E-001: Admin Bulk Question Addition", () => {
  ...
  test.beforeEach(async ({ page, browserName }) => {
    ...
    if (!response && !page.isClosed()) {
      ...
      try {
        const promiseResponse = await responsePromise;
        if (promiseResponse) {
    ...
    try {
      ...
      if (
        ...
        throw new Error(`Navigation failed: ${navErrorMsg}`);
      }
        ...
        try {
          ...
          if (!page.isClosed()) {
                ...
                .catch(() => {
                  const navErrorMsg =
                    navError instanceof Error
                      ...
                      : String(navError);
                  throw new Error(
                    `Failed to navigate to questions page: ${navErrorMsg}`,
                  );
                });
            }
          } else {
            const navErrorMsg =
              navError instanceof Error ? navError.message : String(navError);
            throw new Error(
              `Page was closed during navigation to questions page: ${navErrorMsg}`,
            );
          }
        }
      }
  ...
  test("should create a new question", async ({ page }) => {
    ...
    try {
      ...
      if (createResponse && "ok" in createResponse) {
        ...
        const typedResponseData = createResponseData as {
          success?: boolean;
          data?: {
            success?: number;
            failed?: number;
        ...
        const isSuccess =
          typedResponseData.success === true &&
          ((typedResponseData.data?.success ?? 0) > 0 ||
            (typedResponseData.data?.results?.length ?? 0) > 0);

        if (!isSuccess) {
          const errorMessages = typedResponseData.data?.errors || [];
          const errorMsg =
            errorMessages.length > 0
              ? errorMessages.join(", ")
              : typedResponseData.error || JSON.stringify(typedResponseData);
          console.error("❌ Question creation failed:", typedResponseData);
          throw new Error(`Question creation failed: ${errorMsg}`);
        }

        // Verify we have a result with an ID
        const createdQuestion = typedResponseData.data?.results?.[0];
    ...
    try {
      ...
      if (
        ...
        "json" in fetchResponse
      ) {
        const fetchData = await (fetchResponse as Response).json();
