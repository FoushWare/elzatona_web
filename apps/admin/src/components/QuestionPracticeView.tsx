...
export const QuestionPracticeView: React.FC<QuestionPracticeViewProps> = ({ question }) => {
  ...
  const isCorrectAnswer = (answerText: string): boolean => {
    ...
    if (question.correct_answer) {
      const correctAnswerStr = String(question.correct_answer).toLowerCase().trim();
      ...
      const correctIndex = typeof question.correct_answer === 'number' 
        ? question.correct_answer 
        : parseInt(correctAnswerStr, 10);
      
      if (!isNaN(correctIndex) && shuffledOptions[correctIndex]?.text.toLowerCase().trim() === answerStr) {
        return true;
      }
    }
    
    // Check if the option is marked as correct
    const option = shuffledOptions.find(opt => opt.text.toLowerCase().trim() === answerText.toLowerCase().trim());
  ...
  const displayDifficulty = difficultyMap[question.difficulty] || question.difficulty;
  ...
  return (
    <div className='w-full max-w-4xl mx-auto'>
      ...
      {/* Resources */}
      {question.type !== 'code' && showExplanation && parsedResources && parsedResources.length > 0 && (
