...
const JSON_DIR = path.join(__dirname, '../data/json/CSS');
...
function transformQuestion(question, fileBaseName) {
  ...
  // Map subcategory to topic
  const topicMap = {
    'Core Concepts': 'CSS Fundamentals',
    ...
    Filters: 'CSS Filters',
    Masking: 'CSS Masking',
    Clipping: 'CSS Clipping',
    'Blend Modes': 'CSS Blend Modes',
  ...
  const tags = ['css', topic.toLowerCase().replace(/\s/g, '-')];
  ...
  return {
    ...
    learningCardId: 'core-technologies', // Assign to core-technologies card
...
function main() {
  console.log('🔄 Starting transformation of CSS questions...');
  const files = fs.readdirSync(JSON_DIR).filter(file => file.endsWith('.json'));
  console.log(`Found ${files.length} CSS files to transform:`);
