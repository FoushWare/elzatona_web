...
const JSON_DIR = path.join(__dirname, '../data/json/design-patterns');
...
function transformQuestion(question, fileBaseName) {
  const newId = `design-patterns-${fileBaseName}-${question.id}`;
  const topic =
    question.topic || question.subcategory || 'General Design Patterns';

  const tags = ['design-patterns', topic.toLowerCase().replace(/\s/g, '-')];
...
function main() {
  console.log('🔄 Starting transformation of design-patterns questions...');
  const files = fs.readdirSync(JSON_DIR).filter(file => file.endsWith('.json'));
  console.log(`Found ${files.length} design-patterns files to transform:`);
