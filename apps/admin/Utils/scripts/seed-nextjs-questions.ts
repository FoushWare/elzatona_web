// v1.0 - Seed Next.js questions to Firebase
// Run with: npx tsx src/scripts/seed-nextjs-questions.ts
...
// ==========================================
// Load Next.js Questions from JSON Files
// ==========================================

function loadNextJSQuestions(): NextJSQuestion[] {
  const questions: NextJSQuestion[] = [];
  const nextjsDir = path.join(process.cwd(), 'data', 'json', 'NextJS');

  // Check if directory exists
  if (!fs.existsSync(nextjsDir)) {
    console.log(`⚠️  NextJS directory not found: ${nextjsDir}`);
    return questions;
  }

  // Get all Next.js JSON files
  const files = fs
    .readdirSync(nextjsDir)
    .filter(file => file.endsWith('.json'));

  console.log(`📁 Found ${files.length} Next.js JSON files`);
...
function transformQuestion(question: any): NextJSQuestion {
  const now = new Date().toISOString();

  // Normalize the question data
  const transformedQuestion: NextJSQuestion = {
    id:
      question.id ||
      `nextjs-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    title: question.title || 'Untitled Question',
    content: question.content || '',
    type: question.type || 'multiple-choice',
    category: question.category || 'Next.js',
    subcategory: question.subcategory || 'Next.js Basics',
    difficulty: question.difficulty || 'intermediate',
    learningPath: question.learningPath || 'Next.js Developer Path',
    topic: question.topic || 'Next.js Basics',
    ...
    tags: Array.isArray(question.tags) ? question.tags : ['nextjs'],
...
async function seedNextJSQuestions() {
  console.log('🌱 Starting Next.js questions seeding...');

  // Load questions from JSON files
  const rawQuestions = loadNextJSQuestions();
  console.log(`📊 Loaded ${rawQuestions.length} raw questions`);

  if (rawQuestions.length === 0) {
    console.log('⚠️  No Next.js questions found to seed');
    return;
  }
  ...
  console.log('🎉 Next.js questions seeding completed!');
  console.log(`📊 Summary:`);
