...
async function seedHTMLQuestions() {
  ...
  const htmlDir = path.join(process.cwd(), 'apps/admin/network/data/json/html');
  const files = fs.readdirSync(htmlDir).filter(file => file.endsWith('.json'));

  console.log(`📁 Found ${files.length} HTML question files`);
  ...
  console.log('\n🎉 HTML questions seeding completed!');
  console.log('📊 Summary:');
...
seedHTMLQuestions()
  .then(() => {
    console.log('✅ HTML questions seeding process completed successfully');
    process.exit(0);
  })
  .catch(error => {
    console.error('❌ HTML questions seeding process failed:', error);
    process.exit(1);
  });
