/**
 * Create Working Admin Script
 * 
 * This script creates a working admin user with a different email
 * since the original email is already in use.
 * 
 * Usage: node tools/scripts/create-working-admin.js
 */
...
async function createWorkingAdmin() {
  ...
  // Use a different email since the original is already in use
  ...
  try {
    ...
    if (error.code === 'auth/email-already-in-use') {
      console.log('\n💡 The email is already in use. Trying with a different email...');
      
      // Try with a timestamp-based email
      const timestamp = Date.now();
      const alternativeEmail = `admin${timestamp}@elzatona.com`;
      
      try {
        console.log(`\nTrying with alternative email: ${alternativeEmail}`);
        ...
        console.log('❌ Alternative email also failed:', altError.message);
        console.log('\n💡 Please try manually creating a user in Firebase Console.');
      }
