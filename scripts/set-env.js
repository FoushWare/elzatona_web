...
function showCurrentEnv() {
  ...
  if (supabaseUrl.includes('vopfdukvdhnmzzjkxpnj')) {
    detectedEnv = appEnv === 'development' ? 'TEST/DEV (using test database)' : 'TEST';
  ...
  console.log(`   Project: ${supabaseUrl ? supabaseUrl.match(/https?:\/\/([^.]+)\.supabase\.co/)?.[1] || 'unknown' : 'not set'}`);
...
function setEnvironment(env) {
  ...
  if (!['test', 'production', 'prod', 'development', 'dev'].includes(envLower)) {
  ...
  console.log(`🔄 Switching to ${normalizedEnv.toUpperCase()} environment...\n`);
  ...
  if (normalizedEnv === 'development') {
    ...
    if (!fs.existsSync(envDevLocal)) {
      console.log('⚠️  .env.dev.local does not exist. Creating it with test database values...');
      ...
      if (fs.existsSync(envTestLocal)) {
        ...
        const devContent = testContent
          ...
          .replace(/NEXT_PUBLIC_APP_ENV=test/g, 'NEXT_PUBLIC_APP_ENV=development');
  ...
  console.log(`   1. Restart your dev server: npm run dev:${normalizedEnv === 'production' ? 'prod' : normalizedEnv === 'development' ? 'dev' : 'test'}`);
...
if (!command || command === 'help' || command === '--help' || command === '-h') {
  ...
  console.log('   npm run dev         # Run dev server (default: PRODUCTION)\n');
