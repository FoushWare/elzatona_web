...
console.log('🌱 Seeding React Questions (Using Existing Schema)\n');
...
async function mapQuestionsToExistingSchema(questions) {
  console.log('🔄 Mapping questions to existing schema...\n');
  ...
  try {
    ...
    console.log(
      `📊 Found ${topics.length} React topics and ${categories.length} categories`
    );

    // Update questions to match existing schema
    const mappedQuestions = questions.map(question => {
      ...
      // Map category
      let categoryId = 'cat-react'; // Default to React category
    ...
    console.log(
      `✅ Mapped ${mappedQuestions.length} questions to existing schema\n`
    );
    return mappedQuestions;
  } catch (error) {
    console.error('❌ Error mapping questions:', error.message);
    return questions;
  }
}
...
async function seedQuestions(questions) {
  ...
  try {
    // Clear existing React questions (using category_id instead of category)
...
async function main() {
  try {
    ...
    // Map questions to existing schema
    const mappedQuestions = await mapQuestionsToExistingSchema(allQuestions);
