...
function generateInsertStatement(question, categoryId) {
  ...
  const metadata = {
    ...
    technology: question.technology || 'System Design',
  };
  const is_active =
...
async function seedSystemDesignQuestions() {
  try {
    ...
    if (categoryError) {
      console.error('Error fetching System Design category ID:', categoryError);
      return;
    }

    const categoryId = categoryData.id;
    console.log(`Found System Design category ID: ${categoryId}`);

    // Process all System Design files
    const files = fs
      .readdirSync(dataDir)
      .filter(file => file.endsWith('.json'));
    console.log(`Found ${files.length} System Design files`);
    ...
    console.log(`\n=== System Design Seeding Complete ===`);
    console.log(`Total questions inserted: ${totalInserted}`);
    console.log(`Total errors: ${totalErrors}`);
  } catch (error) {
    console.error('Error in seedSystemDesignQuestions:', error);
  }
}
