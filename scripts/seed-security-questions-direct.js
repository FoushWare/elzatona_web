...
const dataDir = path.join(
  __dirname,
  '../apps/admin/network/data/json/security'
);
...
function generateInsertStatement(question, categoryId) {
  ...
  const metadata = {
    ...
    technology: question.technology || 'Security',
  };
  const is_active =
...
async function seedSecurityQuestions() {
  try {
    ...
    if (categoryError) {
      console.error('Error fetching Security category ID:', categoryError);
      return;
    }

    const categoryId = categoryData.id;
    console.log(`Found Security category ID: ${categoryId}`);

    // Process all Security files
    const files = fs
      .readdirSync(dataDir)
    ...
    console.log(`\n=== Security Seeding Complete ===`);
    console.log(`Total questions inserted: ${totalInserted}`);
    console.log(`Total errors: ${totalErrors}`);
  } catch (error) {
    console.error('Error in seedSecurityQuestions:', error);
  }
}
