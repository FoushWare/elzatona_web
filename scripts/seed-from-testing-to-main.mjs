...
const MAIN_URL_FROM_LOCAL = (MAIN_URL_FROM_LOCAL_RAW && MAIN_URL_FROM_LOCAL_RAW.includes(MAIN_PROJECT_ID)) 
...
const TESTING_KEY = process.env.TESTING_SUPABASE_SERVICE_ROLE_KEY || 
                    (SUPABASE_KEY_AFTER_TEST !== MAIN_KEY_FROM_LOCAL ? SUPABASE_KEY_AFTER_TEST : null);
...
if (!TESTING_KEY) {
  ...
  console.error('   TESTING_SUPABASE_SERVICE_ROLE_KEY=your_testing_service_role_key');
  console.error('\n🔗 Get it from: https://supabase.com/dashboard/project/kiycimlsatwfqxtfprlr/settings/api');
  console.error('   Steps: Dashboard → Settings → API → service_role key (click 👁️ to reveal)');
  process.exit(1);
}
...
console.log(`   Main URL from .env.local: ${MAIN_URL_FROM_LOCAL || 'not set (using fallback)'}\n`);
...
async function testConnections() {
  ...
  try {
    ...
    console.error('   Please verify TESTING_SUPABASE_SERVICE_ROLE_KEY in .env.test.local');
    throw error;
  }
  ...
  try {
    ...
    console.error(`   MAIN_URL_FROM_LOCAL was: ${MAIN_URL_FROM_LOCAL || 'undefined'}`);
...
async function mapCategories() {
  ...
  for (const testCat of testingCats || []) {
    ...
    if (mainCat) {
      ...
      console.warn(`  ⚠️  Category not found in main: ${testCat.name} (${testCat.slug})`);
    }
  }
...
async function mapTopics() {
  ...
  for (const testTopic of testingTopics || []) {
    ...
    if (mainTopic) {
      ...
      console.warn(`  ⚠️  Topic not found in main: ${testTopic.name} (${testTopic.slug})`);
    }
  }
...
async function copyQuestions() {
  ...
  while (true) {
    ...
    const transformedQuestions = questions.map(q => ({
      ...
      learning_card_id: idMappings.learningCards.get(q.learning_card_id) || null,
    ...
    if (insertError) {
      console.error(`  ❌ Error inserting batch ${offset}-${offset + questions.length}:`, insertError.message);
      ...
      for (const q of transformedQuestions) {
        ...
        if (singleError) {
          console.error(`    ❌ Failed to insert: ${q.title.substring(0, 50)}...`, singleError.message);
          totalSkipped++;
        } else {
          totalCopied++;
        }
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      const newQuestion = inserted?.find(nq => nq.title === questions[i].title);
      if (newQuestion) {
    ...
    offset += batchSize;
    console.log(`  📦 Processed ${offset} questions (copied: ${totalCopied}, skipped: ${totalSkipped})`);
...
async function copyPlanCards() {
  ...
  const transformedPlanCards = planCards
    ?.map(pc => ({
  ...
  if (transformedPlanCards.length === 0) {
    console.log('  ⚠️  No plan_cards to copy (missing plan or card mappings)\n');
    return;
  }
...
async function copyPlanQuestions() {
  ...
  while (true) {
    ...
    if (transformedPlanQuestions.length > 0) {
      ...
      if (insertError) {
        console.error(`  ❌ Error inserting batch ${offset}-${offset + planQuestions.length}:`, insertError.message);
      } else {
    ...
    offset += batchSize;
    console.log(`  📦 Processed ${offset} plan_questions (copied: ${totalCopied})`);
...
async function copyCardCategories() {
  ...
  const transformedCardCategories = cardCategories
    ?.map(cc => ({
  ...
  if (transformedCardCategories.length === 0) {
    console.log('  ⚠️  No card_categories to copy (missing card or category mappings)\n');
    return;
  }
  ...
  console.log(`✅ Copied ${transformedCardCategories.length} card_categories\n`);
}
 ...
 */
async function main() {
  console.log('🚀 Starting data seeding from zatona-web-testing to zatona-web\n');
