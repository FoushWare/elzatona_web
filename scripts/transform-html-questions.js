...
const JSON_DIR = path.join(__dirname, '../data/json/html');
...
function transformQuestion(question, fileBaseName) {
  ...
  const topic = question.topic || question.subcategory || 'HTML Basics';

  const tags = ['html', topic.toLowerCase().replace(/\s/g, '-')];
  ...
  return {
    ...
    category: 'HTML',
    topic: topic,
    difficulty: difficultyMap[question.difficulty] || 'beginner',
...
function main() {
  console.log('🔄 Starting transformation of HTML questions...');
  const files = fs.readdirSync(JSON_DIR).filter(file => file.endsWith('.json'));
  console.log(`Found ${files.length} HTML files to transform:`);
