 ...
 * Creates 10 questions for each category with proper mapping to:
 * - Categories
...
const { getFirestore, collection, addDoc, doc, getDocs, query, where } = require('firebase/firestore');
...
const QUESTION_TYPES = ['single', 'multiple', 'text', 'code', 'open-ended', 'yes-no'];
...
const CATEGORIES = {
  'HTML & CSS': {
    ...
    sectors: ['html-fundamentals', 'css-fundamentals']
  },
  'JavaScript (Core)': {
    topics: ['Variables & Scope', 'Functions', 'Async Programming', 'DOM Manipulation', 'ES6+ Features'],
    learningPath: 'javascript-deep-dive',
    sectors: ['javascript-fundamentals', 'javascript-deep-dive', 'problem-solving-javascript']
  },
  'React': {
    topics: ['Components', 'Hooks', 'State Management', 'Lifecycle', 'Performance'],
    learningPath: 'react-mastery',
    sectors: ['react-fundamentals', 'advanced-react-patterns']
  },
  'TypeScript': {
    topics: ['Types', 'Interfaces', 'Generics', 'Utility Types', 'Advanced Patterns'],
    learningPath: 'typescript-essentials',
    sectors: ['typescript-essentials']
  },
  'Performance': {
    topics: ['Bundle Optimization', 'Runtime Performance', 'Caching', 'Lazy Loading', 'Monitoring'],
    learningPath: 'performance-optimization',
    sectors: ['performance-optimization']
  },
  'System Design': {
    topics: ['Architecture Patterns', 'Scalability', 'Microservices', 'Caching Strategies', 'Load Balancing'],
    learningPath: 'frontend-system-design',
    sectors: ['system-design-frontend', 'design-patterns-architecture']
  },
  'Security': {
    topics: ['XSS Prevention', 'CSRF Protection', 'Content Security Policy', 'Authentication', 'Data Validation'],
    learningPath: 'security-essentials',
    sectors: ['frontend-security']
  },
  'Testing': {
    topics: ['Unit Testing', 'Integration Testing', 'E2E Testing', 'Mocking', 'Test Coverage'],
    learningPath: 'testing-strategies',
    sectors: ['testing-strategies']
  },
  'Build Tools': {
    topics: ['Webpack', 'Vite', 'Babel', 'ESLint', 'CI/CD'],
    learningPath: 'build-tools-devops',
    sectors: ['build-tools-devops']
  },
  'API Integration': {
    topics: ['REST APIs', 'GraphQL', 'Data Fetching', 'Error Handling', 'Caching'],
    learningPath: 'api-integration',
    sectors: ['api-integration-communication']
  }
};
...
function generateQuestionsForCategory(category, count = 10) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const question = {
      ...
      sectionId: categoryData.sectors[0], // Use first sector
...
async function createAllQuestions() {
  try {
    console.log('🚀 Starting comprehensive question creation...');
    ...
    console.log(`- Question types: ${QUESTION_TYPES.join(', ')}`);
