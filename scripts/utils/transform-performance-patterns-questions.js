...
const JSON_DIR = path.join(__dirname, '../data/json/performance-patterns');
...
function transformQuestion(question, fileBaseName) {
  const newId = `performance-patterns-${fileBaseName}-${question.id}`;
  const topic =
    question.topic || question.subcategory || 'General Performance Patterns';
...
function main() {
  console.log(
    '🔄 Starting transformation of Performance Patterns questions...'
  );
  const files = fs.readdirSync(JSON_DIR).filter(file => file.endsWith('.json'));
  console.log(`Found ${files.length} Performance Patterns files to transform:`);
