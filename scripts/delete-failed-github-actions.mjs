...
try {
  ...
  if (isDryRun) {
    ...
    runsToDelete.forEach((run, index) => {
      console.log(`  ${index + 1}. Run ID: ${run.id} | ${run.name} | ${run.head_branch} | ${run.conclusion} | ${run.created_at}`);
    });
  ...
  for (const run of runsToDelete) {
    try {
      ...
      console.log(`✅ Deleted run ${run.id} - ${run.name} (${run.head_branch}) - ${run.conclusion}`);
      deleted++;
    } catch (error) {
