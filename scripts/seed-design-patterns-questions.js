...
const outputSqlFile = path.join(
  __dirname,
  'seed-design-patterns-questions.sql'
);
const outputBatchesDir = path.join(__dirname, 'design-patterns-batches');
...
function generateInsertStatement(question, categoryId) {
  ...
  const metadata = JSON.stringify({
    ...
    technology: question.technology || 'Design Patterns',
  });
  const is_active =
...
async function generateDesignPatternsQuestionsSql() {
  let allInsertStatements = [];
  const files = fs.readdirSync(dataDir).filter(file => file.endsWith('.json'));

  // Design Patterns category ID
  const designPatternsCategoryId = '2d6223f9-020c-45e7-a3bd-6aa9b0a6af3c';

  console.log('Processing Design Patterns questions from JSON files...');
  console.log(`Found ${files.length} Design Patterns files`);

  for (const file of files) {
    const filePath = path.join(dataDir, file);
    const statements = processQuestionsFromFile(
      filePath,
      designPatternsCategoryId
    );
    allInsertStatements = allInsertStatements.concat(statements);
  }

  console.log(
    `\nGenerated ${allInsertStatements.length} Design Patterns INSERT statements`
  );
  ...
  for (let i = 0; i < statements.length; i += BATCH_SIZE) {
    ...
    const batchFileName = path.join(
      outputBatchesDir,
      `design-patterns-batch-${batchNum}.sql`
    );
  ...
  console.log(
    'All Design Patterns batch files created! Execute them using Supabase MCP tools.'
  );
}

generateDesignPatternsQuestionsSql();
