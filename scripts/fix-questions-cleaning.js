...
async function processBatch(batchNumber, questions) {
    ...
    if (needsUpdate) {
      try {
        ...
        if (error) {
          ...
          fixedCount++;
          console.log(`  ✅ Fixed question: ${question.title?.substring(0, 50) || question.id}`);
        }
      } catch (err) {
  ...
  console.log(`  ✅ Batch ${batchNumber}: Fixed ${fixedCount}/${questions.length} questions`);
...
async function main() {
  try {
    ...
    console.log(`📦 Processing ${batches.length} batches (${BATCH_SIZE} questions per batch)\n`);
