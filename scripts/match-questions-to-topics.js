...
const existingTopics = [
  { id: "8dcdb2c6-5056-4b47-a5ce-a3cd4a1e20eb", name: "Basics", slug: "javascript-basics", description: "Fundamental JavaScript concepts: variables (var, let, const), hoisting, scope, closures, data types, operators, and control flow" },
  { id: "1b62c368-375b-4df1-aadd-eb0504ae3612", name: "This Binding", slug: "this-binding", description: "The 'this' keyword: context binding, call/apply/bind methods, arrow functions vs regular functions, and lexical scope" },
  { id: "4d2a6785-b6db-4985-953f-f3ba4a388e8e", name: "Classes", slug: "classes", description: "ES6+ classes: class syntax, inheritance, static methods, private fields, constructors, and class-based object-oriented programming" },
  { id: "ea7a64be-4db2-4acd-abdb-13bd93a7218d", name: "Data Structures", slug: "data-structures", description: "JavaScript data structures: Arrays, Objects, Sets, Maps, WeakMap, WeakSet, and their methods and use cases" },
  { id: "2f38feb2-86f3-4ff6-ab47-08e78b5e80c8", name: "Async/Await", slug: "async-await", description: "Asynchronous JavaScript: Promises, async/await syntax, Promise methods (all, race, allSettled), error handling, and event loop" }
];
...
function findMatchingTopic(question) {
  ...
  for (const topic of existingTopics) {
    ...
    if (topicNameLower === "this binding" && (tagsLower.includes("this") || titleLower.includes("this"))) {
      return topic;
    }
    
    // Match "Classes" topic
    if (topicNameLower === "classes" && (tagsLower.includes("class") || tagsLower.includes("static") || titleLower.includes("class"))) {
      return topic;
    }
    
    // Match "Basics" topic for hoisting, scope, var, let (but not if it's about this or classes)
    if (topicNameLower === "basics" && 
      ...
      if (!tagsLower.includes("this") && !tagsLower.includes("class") && !titleLower.includes("this") && !titleLower.includes("class")) {
        return topic;
      }
    }
    
    // Match "Data Structures" for object-related questions (but not this binding or functions as objects)
    if (topicNameLower === "data structures" && 
...
function getTopicNameForQuestion(question) {
  ...
  if (tagsLower.includes("type-coercion") || 
  ...
  if ((tagsLower.includes("function") && tagsLower.includes("first-class-functions")) ||
      (tagsLower.includes("function") && tagsLower.includes("object") && titleLower.includes("function"))) {
    return "Functions";
  }
  
  // Objects (property access, references, etc.)
  if (tagsLower.includes("object") && 
      !tagsLower.includes("this") && 
      !tagsLower.includes("arrow-functions") &&
      !tagsLower.includes("first-class-functions") &&
