...
const JSON_DIR = path.join(__dirname, '../data/json/nextjs');
...
function transformQuestion(question, fileBaseName) {
  ...
  const topic = question.topic || question.subcategory || 'General Next.js';

  const tags = ['nextjs', topic.toLowerCase().replace(/\s/g, '-')];
...
function main() {
  console.log('🔄 Starting transformation of Next.js questions...');
  const files = fs.readdirSync(JSON_DIR).filter(file => file.endsWith('.json'));
  console.log(`Found ${files.length} Next.js files to transform:`);
