...
async function seedDesignPatternsQuestions() {
  ...
  console.log('\n🎉 Design Patterns questions seeding completed!');
  console.log('📊 Summary:');
...
seedDesignPatternsQuestions()
  .then(() => {
    console.log(
      '✅ Design Patterns questions seeding process completed successfully'
    );
    process.exit(0);
  })
  .catch(error => {
    console.error(
      '❌ Design Patterns questions seeding process failed:',
      error
    );
    process.exit(1);
  });
