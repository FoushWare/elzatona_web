...
interface GuidedLearningPlan {
  ...
  metadata: {
    version: string;
    cumulative: boolean;
    previousPlanId: string; // Empty string if no previous plan
    cumulativeQuestions: number; // Total questions including previous days
  };
}

// Enhanced question distribution per day and card type (cumulative)
...
// Enhanced card configurations with detailed categories and topics
const cardConfigs = {
  'core-technologies': {
  ...
  if (day > 1) {
    ...
    for (let prevDay = 1; prevDay < day; prevDay++) {
      const prevDistribution =
        cumulativeQuestionDistribution[
          prevDay as keyof typeof cumulativeQuestionDistribution
        ];
...
async function seedComprehensiveGuidedLearningPlans() {
  console.log(
    '🚀 Starting Comprehensive Guided Learning Plans seeding process...'
  );
  console.log(
  ...
  console.log('\n🎉 Comprehensive Guided Learning Plans seeding completed!');
  console.log('📊 Summary:');
...
seedComprehensiveGuidedLearningPlans()
  .then(() => {
    console.log(
      '✅ Comprehensive Guided Learning Plans seeding process completed successfully'
    );
    process.exit(0);
  })
  .catch(error => {
    console.error(
      '❌ Comprehensive Guided Learning Plans seeding process failed:',
      error
    );
    process.exit(1);
  });
