...
export class SectorService {
  ...
  static async createSector(
    sectorData: Omit<Sector, 'id' | 'createdAt' | 'updatedAt'>
  ): Promise<{ success: boolean; sectorId?: string; error?: string }> {
    try {
  ...
  static async removeQuestionsFromSector(
    ...
    try {
      ...
      const currentQuestionIds = sectorResult.sector.questionIds || [];
      const newQuestionIds = currentQuestionIds.filter(
  ...
  static async getSectorProgress(
    userId: string,
    sectorId: string
  ...
  static async updateSectorProgress(
    progressData: Omit<SectorProgress, 'id' | 'createdAt' | 'updatedAt'>
  ): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  static async getSectorsWithProgress(
    userId: string,
    learningPathId: string
  ): Promise<{
    success: boolean;
    sectorsWithProgress?: Array<Sector & { progress?: SectorProgress }>;
    error?: string;
  }> {
    try {
      ...
      const sectorsWithProgress = await Promise.all(
        sectorsResult.sectors.map(async sector => {
          const progressResult = await this.getSectorProgress(
            userId,
            sector.id
          );
          return {
