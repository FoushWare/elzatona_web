import { LearningItem, LearningGoal, ProgressStats, LearningRecommendation } from "@/types/progress";
...
export const sampleLearningItems: LearningItem[] = [
  // JavaScript Fundamentals
  {
    ...
    description: "Learn about var, let, const, and primitive data types in JavaScript",
    category: "javascript",
  ...
  },
  {
    ...
    description: "Master array methods like map, filter, reduce, and array manipulation",
    category: "javascript",
    subCategory: "arrays",
  ...
  },
  {
    id: "js-5",
    title: "ES6+ Features",
    description: "Master modern JavaScript features like destructuring, spread, modules",
    category: "javascript",
    subCategory: "es6",
    difficulty: "intermediate",
    estimatedTime: 60,
  ...
  // React Fundamentals
  {
    ...
    description: "Learn React components, JSX syntax, and component composition",
    category: "react",
  ...
  },
  {
    ...
    description: "Learn React.memo, useMemo, useCallback, and performance best practices",
    category: "react",
  ...
  // HTML and Semantics
  {
    ...
    description: "Learn semantic HTML elements for better accessibility and SEO",
    category: "html",
  ...
  // Performance
  {
    ...
    description: "Learn techniques for optimizing web performance and Core Web Vitals",
  ...
  // Testing
  {
    ...
    description: "Learn to write tests for React components and JavaScript functions",
    category: "testing",
...
export const sampleLearningGoals: LearningGoal[] = [
  {
    ...
    description: "Complete all JavaScript fundamentals to build a strong foundation",
  ...
  },
  {
    ...
    description: "Become proficient with Flexbox and Grid for responsive layouts",
...
export const calculateProgressStats = (items: LearningItem[]): ProgressStats => {
  const totalItems = items.length;
  const completedItems = items.filter(item => item.isCompleted).length;
  const completionPercentage = totalItems > 0 ? Math.round((completedItems / totalItems) * 100) : 0;
  
  const totalEstimatedTime = items.reduce((sum, item) => sum + item.estimatedTime, 0);
  const totalActualTime = items.reduce((sum, item) => sum + (item.actualTimeSpent || 0), 0);
  const timeEfficiency = totalEstimatedTime > 0 ? Math.round((totalActualTime / totalEstimatedTime) * 100) : 0;

  // Category breakdown
  const categoryBreakdown: Record<string, { total: number; completed: number; percentage: number }> = {};
  items.forEach(item => {
    if (!categoryBreakdown[item.category]) {
      categoryBreakdown[item.category] = { total: 0, completed: 0, percentage: 0 };
    }
  ...
  Object.keys(categoryBreakdown).forEach(category => {
    ...
    categoryBreakdown[category].percentage = total > 0 ? Math.round((completed / total) * 100) : 0;
  });

  // Difficulty breakdown
  const difficultyBreakdown: Record<string, { total: number; completed: number; percentage: number }> = {};
  items.forEach(item => {
    if (!difficultyBreakdown[item.difficulty]) {
      difficultyBreakdown[item.difficulty] = { total: 0, completed: 0, percentage: 0 };
    }
  ...
  Object.keys(difficultyBreakdown).forEach(difficulty => {
    ...
    difficultyBreakdown[difficulty].percentage = total > 0 ? Math.round((completed / total) * 100) : 0;
  });
...
export const getLearningRecommendations = (items: LearningItem[]): LearningRecommendation[] => {
