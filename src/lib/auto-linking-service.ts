...
export interface SectionData {
  ...
  name: string;
  description: string;
  category: 'foundation' | 'frontend' | 'advanced' | 'specialized' | 'career' | 'emerging';
  learningPath: string;
  questions: string[];
  order: number;
  isActive: boolean;
...
class AutoLinkingService {
  ...
  async linkQuestionToSections(
    ...
    try {
      console.log('🔗 Auto-linking question to sections:', { questionId, category, learningPath });
      ...
      console.log(`✅ Question ${questionId} will be automatically linked to sections with category: ${category}, learningPath: ${learningPath}`);
      console.log('ℹ️ Admin sections are virtual - questions are filtered dynamically based on category and learningPath');
  ...
  async linkQuestionToSectors(
    ...
    try {
      console.log('🔗 Auto-linking question to sectors:', { questionId, category, learningPath });
      ...
      console.log(`📋 Found ${sectorsSnapshot.docs.length} sectors for learning path: ${learningPath}`);
      ...
      for (const doc of sectorsSnapshot.docs) {
        ...
        let matchScore = 0;
        if (sectorName.includes(categoryLower) || categoryLower.includes(sectorName)) {
          matchScore += 10;
        }
      ...
      if (!targetSector) {
        targetSector = { id: sectorsSnapshot.docs[0].id, ...sectorsSnapshot.docs[0].data() };
        console.log('ℹ️ No specific category match found, using first available sector');
      }
      ...
      if (!currentQuestionIds.includes(questionId)) {
        ...
        console.log(`✅ Added question to sector: ${targetSector.name} (${targetSector.id})`);
      } else {
        console.log(`ℹ️ Question already exists in sector: ${targetSector.name}`);
      }
  ...
  async linkQuestionToLearningPaths(
    ...
    try {
      console.log('🔗 Auto-linking question to learning paths:', { questionId, category, learningPath });
      ...
      console.log(`📋 Found ${learningPathsSnapshot.docs.length} matching learning paths`);
      ...
      const updatePromises = learningPathsSnapshot.docs.map(async (doc) => {
        ...
        if (!questions.includes(questionId)) {
          ...
          console.log(`✅ Added question to learning path: ${learningPathData.name}`);
        } else {
          console.log(`ℹ️ Question already exists in learning path: ${learningPathData.name}`);
        }
      });
   ...
   */
  async createQuestionWithAutoLinking(questionData: QuestionData): Promise<string> {
    try {
      console.log('🚀 Creating question with auto-linking:', questionData);

      // 1. Create question in unifiedQuestions
      const questionRef = await addDoc(collection(db, this.COLLECTIONS.QUESTIONS), {
        ...questionData,
   ...
   */
  async bulkImportQuestionsWithAutoLinking(questionsData: QuestionData[]): Promise<string[]> {
    try {
      console.log(`🚀 Bulk importing ${questionsData.length} questions with auto-linking`);
      ...
      const linkingPromises = questionsData.map(async (questionData, index) => {
        ...
        await Promise.all([
          this.linkQuestionToSections(questionId, questionData.category, questionData.learningPath),
          this.linkQuestionToLearningPaths(questionId, questionData.category, questionData.learningPath)
        ]);
      });
      ...
      console.error('❌ Error bulk importing questions with auto-linking:', error);
   ...
   */
  private async getFirebaseLearningPathIdByName(name: string): Promise<string | null> {
    try {
      ...
      console.error('❌ Error getting Firebase learning path ID by name:', error);
      return null;
    }
  }
  ...
  async getQuestionsForSection(sectionId: string): Promise<QuestionData[]> {
    try {
      ...
      console.log(`🔗 Using section ID directly as learning path: ${sectionId}`);
      ...
      console.log(`📋 Found ${questions.length} questions for section "${sectionId}"`);
      return questions;
    } catch (error) {
   ...
   */
  async getSectionsForPlan(category?: string, learningPath?: string): Promise<SectionData[]> {
    try {
      ...
      const { getDefaultAdminSections } = await import('./learning-path-mapping');
      ...
      if (learningPath) {
        filteredSections = filteredSections.filter(section => section.learningPathId === learningPath);
      }
  ...
  async removeQuestionFromAllSections(questionId: string): Promise<void> {
    try {
      ...
      console.log(`📋 Found ${sectionsSnapshot.docs.length} sections containing this question`);
      ...
      const updatePromises = sectionsSnapshot.docs.map(async (doc) => {
        ...
        const updatedQuestions = sectionData.questions.filter(id => id !== questionId);
