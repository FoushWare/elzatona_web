...
export const validateQuestion = (question: Partial<UnifiedQuestion>): QuestionValidationResult => {
  ...
  if (question.type === 'multiple-choice') {
    if (!question.options || question.options.length < QUESTION_VALIDATION_RULES.options.minCount) {
      errors.push({
        field: 'options',
...
// Firebase operations
export const getQuestions = async (filters?: QuestionFilter): Promise<Question[]> => {
  try {
...
export const getRandomQuestions = async (count: number = 10, filters?: QuestionFilter): Promise<Question[]> => {
  try {
...
export const getQuestionStats = async (): Promise<QuestionStats> => {
  try {
    ...
    const totalDifficulty = questions.reduce((sum, q) => sum + difficultyValues[q.difficulty], 0);
...
export const saveQuestionAttempt = async (attempt: Omit<QuestionAttempt, 'id' | 'attemptedAt'>): Promise<string> => {
  try {
    if (!db) throw new Error('Firebase not initialized');
...
export const searchQuestions = async (searchTerm: string, filters?: QuestionFilter): Promise<Question[]> => {
  try {
    const allQuestions = await getQuestions(filters);
...
export const getQuizQuestions = async (category?: string, difficulty?: string, count: number = 10): Promise<Question[]> => {
  try {
