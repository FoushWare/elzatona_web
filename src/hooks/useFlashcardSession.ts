...
interface UseFlashcardSessionReturn {
  ...
  // Session controls
  startSession: (type: 'review' | 'new' | 'mixed', maxCards?: number) => Promise<void>;
...
export function useFlashcardSession(): UseFlashcardSessionReturn {
  ...
  const [cardsDueForReview, setCardsDueForReview] = useState<FlashcardProgress[]>([]);
  ...
  // Start a new session
  const startSession = useCallback(async (type: 'review' | 'new' | 'mixed', maxCards: number = 20) => {
    if (!user) {
    ...
    try {
      ...
      if (type === 'review') {
        ...
        const dueProgress = await progressService.getCardsDueForReview(user.uid);
        ...
        const validDueCards = dueCards.filter(card => card !== null) as Flashcard[];
        
        const newCards = await progressService.getNewCards(user.uid, Math.max(1, maxCards - validDueCards.length));
  ...
  const endSession = useCallback(async () => {
    ...
    try {
      ...
      const duration = Math.round((endTime.getTime() - sessionStartTime.getTime()) / 60000); // minutes
  ...
  const handleAnswer = useCallback(async (isCorrect: boolean) => {
    ...
    try {
      ...
      await progressService.updateProgress(user.uid, currentCard.id, isCorrect);
      ...
      setSessionStats(prev => {
        ...
        const newIncorrect = isCorrect ? prev.incorrectAnswers : prev.incorrectAnswers + 1;
  ...
  const nextCard = useCallback(() => {
    setCurrentIndex(prev => {
      ...
      console.log('Moving to next card:', { prev, nextIndex, sessionCardsLength: sessionCards.length });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const now = new Date();
      const timeSpent = Math.round((now.getTime() - sessionStartTime.getTime()) / 60000);
