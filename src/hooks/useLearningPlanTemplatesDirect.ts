...
interface LearningPlanTemplate {
  ...
  sections: Array<{ id: string; name: string; questions: any[]; weight: number }>;
  features: string[];
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      ...
      if (apiData.success && apiData.templates && apiData.templates.length > 0) {
        console.log('✅ DirectHook: Successfully loaded templates from API:', apiData.templates.length);
        ...
        console.log('⚠️ DirectHook: API failed or returned no data, using mock data');
