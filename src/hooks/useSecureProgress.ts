...
export function useSecureProgress(): UseSecureProgressReturn {
  ...
  const saveProgress = useCallback(async (data: Omit<ProgressData, 'userId'>): Promise<boolean> => {
    if (!isAuthenticated || !user) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.error === 'Authentication required' || errorData.error === 'Invalid authentication token') {
          console.warn('Authentication cookie not set, trying to set it again...');
          ...
          try {
            ...
            if (success) {
              console.log('✅ Auth cookie set successfully, retrying progress save...');
            ...
            console.warn('Retry failed, continuing with local storage only:', retryError);
          }
        }
        
        console.error('❌ Progress save failed:', errorData.error || 'Failed to save progress');
  ...
  const updateLocalCache = useCallback((progressData: ProgressData) => {
    try {
      ...
      cachedData.data.accuracy = Math.round((cachedData.data.correctAnswers / cachedData.data.totalQuestions) * 100);
      ...
      if (progressData.learningMode === 'guided' && progressData.planId) {
        if (!cachedData.data.currentPlan || cachedData.data.currentPlan.planId !== progressData.planId) {
        ...
        cachedData.data.currentPlan.progress = Math.round((cachedData.data.currentPlan.questionsCompleted / cachedData.data.currentPlan.totalQuestions) * 100);
      }
  ...
  const syncProgress = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to sync progress');
    } finally {
      setIsLoading(false);
    }
  }, [isAuthenticated, user]);
