...
describe('IProgressRepository', () => {
  ...
  it('createProgress with userId and flashcardId', async () => {
    const newProgress = await progressRepository.createProgress({ userId: 'u3', flashcardId: '1', status: 'new' });
    expect(newProgress.id).toBeDefined();
    expect(newProgress.userId).toBe('u3');
  });

  it('updateProgress updates status', async () => {
    const updated = await progressRepository.updateProgress('1', { userId: 'u1', flashcardId: '1', status: 'reviewed' });
