 ...
 */
export const LearningModeSwitcher: React.FC<LearningModeSwitcherProps> = ({ isScrolled }) => {
    ...
    >
      ...
      <button
        ...
        className={`relative z-10 flex items-center justify-center space-x-2 px-4 py-1.5 rounded-lg transition-colors duration-300 ${
          isGuided 
            ? (isScrolled ? "text-white" : "text-indigo-900 dark:text-white") 
            : (isScrolled ? "text-gray-500 hover:text-gray-700 dark:text-gray-400" : "text-white/70 hover:text-white")
        }`}
      >
      ...
      >
        ...
        <span className="text-sm font-semibold whitespace-nowrap">Free Style</span>
      </button>
    </div>
  );
};
