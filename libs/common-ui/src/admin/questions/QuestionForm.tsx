...
import {
  ...
  CollapsibleTrigger,
  CollapsibleContent,
} from "../../components/ui"; // Adjust path based on your structure
import {
  BookOpen,
  BarChart3,
  TrendingUp,
...
export interface QuestionFormProps {
  initialData?: AdminUnifiedQuestion;
  onSubmit: (question: Partial<AdminUnifiedQuestion>) => void;
  onCancel: () => void;
...
const initializeFormState = (
  initialData: Partial<AdminUnifiedQuestion> | null,
  readOnly: boolean,
) => ({
...
const createFormHandlers = (
  setFormData: React.Dispatch<
    React.SetStateAction<Partial<AdminUnifiedQuestion>>
  >,
) => ({
  handleChange: (
    e: React.ChangeEvent<
  ...
  handleSelectChange: (name: keyof AdminUnifiedQuestion, value: string) => {
...
const useCollapsibleSections = (
  initialData: AdminUnifiedQuestion | null,
  readOnly: boolean,
) => {
...
export const AdminQuestionForm = React.forwardRef<
  HTMLFormElement,
  QuestionFormProps
>(
  (
    ...
    const initialState = initializeFormState(initialData || null, readOnly);
    const [formData, setFormData] = useState<Partial<AdminUnifiedQuestion>>(
