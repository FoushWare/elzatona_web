  ...
  const parseJsonText = useCallback((text: string) => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        setJsonError('Invalid format. Expected an array of topics or an object with a "topics" property.');
        setParsedTopics([]);
        return;
      }
      ...
      topicsArray.forEach((topicData, index) => {
        ...
        if (topicData.categoryId && !categories.find(c => c.id === topicData.categoryId)) {
          topicErrors.push(`Topic ${index + 1}: categoryId "${topicData.categoryId}" not found`);
        }

        // Validate difficulty
        if (topicData.difficulty && !['beginner', 'intermediate', 'advanced'].includes(topicData.difficulty)) {
          topicErrors.push(`Topic ${index + 1}: difficulty must be "beginner", "intermediate", or "advanced"`);
        }
  ...
  return (
    <form onSubmit={handleSubmit} className='space-y-6'>
      ...
      {isJsonMode && !topic ? (
        <div className='space-y-2'>
          ...
          <p className='text-xs text-gray-500 dark:text-gray-400'>
            Paste your topics array as JSON. The JSON will be validated before submission.
            {parsedTopics.length > 0 && (
              <span className='ml-2 text-green-600 dark:text-green-400 font-semibold'>
                ✓ {parsedTopics.length} valid topic{parsedTopics.length !== 1 ? 's' : ''} found
              </span>
            )}
          </p>
          {jsonError && (
      ...
      <div className='flex justify-end gap-3 pt-4'>
        ...
        <Button type='submit' disabled={isLoading || (isJsonMode && parsedTopics.length === 0)}>
          {isLoading 
            ? 'Saving...' 
            : topic 
              ? 'Update Topic' 
