  ...
  return (
    <div className='space-y-5 sm:space-y-6'>
      {parts.map((part, index) => {
        if (part.type === 'code') {
          return (
            <div key={index} className='relative group my-4 sm:my-6' style={{ backgroundColor: '#1e1e1e' }}>
              <div className='flex items-center justify-end px-4 py-2.5 rounded-t-xl border-b-2 shadow-sm' style={{ background: 'linear-gradient(to right, #252526, #1e1e1e)', borderColor: '#3e3e42' }}>
                <span className='text-xs font-semibold uppercase tracking-wider px-2 py-1 rounded-md' style={{ color: '#cccccc', backgroundColor: 'rgba(62, 62, 66, 0.5)' }}>
                  {part.language || 'javascript'}
                </span>
              </div>
              <div className='relative overflow-hidden rounded-b-xl border-x-2 border-b-2 shadow-lg' style={{ backgroundColor: '#1e1e1e', borderColor: '#3e3e42' }}>
                <SyntaxHighlighter
