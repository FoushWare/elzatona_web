...
const TESTING_KEY =
  ...
  (process.env.SUPABASE_SERVICE_ROLE_KEY !== MAIN_KEY_FROM_LOCAL
...
console.log(
  `   URL Match: ${TESTING_URL === TESTING_PROJECT_URL ? "✅" : "⚠️"}`,
);
console.log(`   Service Role Key: ${TESTING_KEY ? "✅ Set" : "❌ Missing"}`);
if (TESTING_KEY) {
  console.log(
    `   Key Source: ${process.env.TESTING_SUPABASE_SERVICE_ROLE_KEY ? ".env.test.local (TESTING_SUPABASE_SERVICE_ROLE_KEY)" : ".env.test.local (SUPABASE_SERVICE_ROLE_KEY)"}`,
  );
}
...
if (TESTING_KEY) {
  try {
    ...
    const { error } = await testingClient
      .from("categories")
      .select("id")
      .limit(1);
    if (error) {
...
if (allGood) {
  ...
  console.log(
    "   - NEXT_PUBLIC_SUPABASE_URL (optional, will use verified URL)",
  );
  console.log("\n   In .env.test.local:");
  ...
  console.log(
    `   - Testing: https://supabase.com/dashboard/project/${TESTING_PROJECT_ID}/settings/api`,
  );
  console.log(
    `   - Main: https://supabase.com/dashboard/project/${MAIN_PROJECT_ID}/settings/api\n`,
  );
  process.exit(1);
}
