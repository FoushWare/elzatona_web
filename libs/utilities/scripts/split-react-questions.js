...
const REACT_QUESTIONS_FILE = path.join(
  __dirname,
  "../../../apps/website/lib/reactQuestions.ts",
);
const QUESTIONS_DIR = path.join(
  __dirname,
  "../../../apps/website/lib/questions",
);
...
function splitReactQuestions() {
  ...
  const interfaceMatch = content.match(
    /export interface ReactQuestion[\s\S]*?^}/m,
  );
  const interfaceCode = interfaceMatch ? interfaceMatch[0] : "";

  // Extract the array
  const arrayMatch = content.match(
    /export const reactQuestions: ReactQuestion\[\] = \[([\s\S]*)\];/,
  );
  if (!arrayMatch) {
  ...
  const questionMatches = arrayContent.match(
    /\{\s*id:\s*\d+,[\s\S]*?(?=\{\s*id:\s*\d+|$)/g,
  );
  ...
  console.log(
    `Splitting into ${chunks.length} files (${chunkSize} questions each)\n`,
  );
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const endId = parseInt(
      chunk[chunk.length - 1].match(/id:\s*(\d+)/)?.[1] || "0",
    );
    ...
    imports.push(
      `import { ${importName} } from './questions/${fileName.replace(".ts", "")}';\n`,
    );
  ...
  // Create types file
  fs.writeFileSync(path.join(QUESTIONS_DIR, "types.ts"), interfaceCode + "\n");
  ...
  fs.writeFileSync(path.join(QUESTIONS_DIR, "index.ts"), indexContent);

  console.log(`\n✅ Created index.ts`);
  console.log(`\n📝 Next steps:`);
  console.log(
    `  1. Update imports to use: import { reactQuestions } from '@/lib/questions'`,
  );
