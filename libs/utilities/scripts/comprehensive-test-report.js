...
const OUTPUT_FILE = path.join(__dirname, "../../../docs/COMPREHENSIVE_TEST_REPORT.md");
...
function runTestCommand(command) {
  try {
    const output = execSync(command, { encoding: "utf8", maxBuffer: 10 * 1024 * 1024, stdio: "pipe" });
...
function parseTestOutput(output) {
  ...
  // Parse snapshots
  const snapMatch = output.match(/Snapshots:\s+(\d+)\s+failed,\s+(\d+)\s+passed,\s+(\d+)\s+total/);
  if (snapMatch) {
...
function generateReport() {
  ...
  const totalSnapshotFiles = files.reduce((sum, f) => sum + f.snapshots.length, 0);
...
`;
    ...
    for (const file of typeFiles.slice(0, 50)) { // Limit to first 50 per type
      report += `##### ${file.name}\n`;
...
`;
  ...
  console.log(`   - Files with tests: ${filesWithTests} (${((filesWithTests / totalFiles) * 100).toFixed(1)}%)`);
