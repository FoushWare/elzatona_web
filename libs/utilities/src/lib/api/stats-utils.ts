 ...
 */
export function calculateAchievements(questionsCompleted: number, totalPoints: number, currentStreak: number, longestStreak: number) {
  const achievements = [];
  const now = new Date().toISOString();

  const addIf = (cond: boolean, id: string, name: string, desc: string) => {
    if (cond) achievements.push({ id, name, description: desc, unlockedAt: now });
  };

  addIf(questionsCompleted >= 10,  "questions-10", "Getting Started", "Completed 10 questions");
  addIf(questionsCompleted >= 50,  "questions-50", "Learning Fast", "Completed 50 questions");
  addIf(questionsCompleted >= 100, "questions-100", "Century Club", "Completed 100 questions");
  addIf(totalPoints >= 100,        "points-100", "Point Collector", "Earned 100 points");
  ...
  addIf(longestStreak >= 30,       "longest-streak-30", "Consistency Champion", "Longest streak of 30 days");
