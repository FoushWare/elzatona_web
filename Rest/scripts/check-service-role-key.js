...
(async () => {
  try {
    ...
    if (keyRole !== 'service_role') {
      ...
      console.error(
        '   Please get the service_role key from Supabase dashboard.'
      );
      process.exit(1);
    }
    ...
    if (keyProjectRef !== projectRef) {
      ...
      console.error(
        '   3. Update SUPABASE_SERVICE_ROLE_KEY in .env.test.local\n'
      );
      process.exit(1);
    }
    ...
    if (error) {
      if (
        ...
        console.log(
          '   Run the schema SQL first: Rest/scripts/test-database-schema.sql\n'
        );
      } else if (
        ...
        console.error(
          '❌ Invalid API key - key does not have access to this project'
        );
