...
const renderingPatternsDir = path.join(__dirname, '../../apps/admin/network/data/json/rendering-patterns');
...
const files = fs.readdirSync(renderingPatternsDir).filter(f => f.endsWith('.json')).sort();
...
Object.entries(questionsByTopic).forEach(([topic, questions]) => {
  for (let i = 0; i < questions.length; i += 3) {
    ...
    const batchFileName = path.join(batchesDir, `add_rendering_patterns_${topic.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '')}_batch${Math.floor(i / 3) + 1}.js`);
