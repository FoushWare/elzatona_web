...
const dataDir = path.join(__dirname, '../apps/admin/network/data/json/React');
...
function generateInsertStatement(question, categoryId) {
  ...
  const metadata = {
    ...
    technology: question.technology || 'React',
  };
  const is_active =
...
async function seedReactQuestions() {
  try {
    ...
    if (categoryError) {
      console.error('Error fetching React category ID:', categoryError);
      return;
    }

    const categoryId = categoryData.id;
    console.log(`Found React category ID: ${categoryId}`);
    ...
    console.log(`\n=== React Seeding Complete ===`);
    console.log(`Total questions inserted: ${totalInserted}`);
    console.log(`Total errors: ${totalErrors}`);
  } catch (error) {
    console.error('Error in seedReactQuestions:', error);
  }
}
