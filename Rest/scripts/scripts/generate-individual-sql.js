...
console.log('🚀 Continuing React Questions Seeding via Supabase MCP\n');
...
function generateSingleQuestionSQL(question) {
...
async function main() {
  try {
    ...
    // Step 3: Generate individual SQL statements
    console.log('🔄 Generating individual SQL statements...\n');

    const sqlStatements = mappedQuestions.map((question, index) => {
      const sql = generateSingleQuestionSQL(question);
      return {
        index: index + 1,
        sql: sql,
        title: question.title,
    ...
    console.log(
      `✅ Generated ${sqlStatements.length} individual SQL statements\n`
    );

    // Step 4: Save to file for manual execution
    const outputFile = path.join(
      process.cwd(),
      'scripts',
      'react-questions-individual.sql'
    );
    const sqlContent = sqlStatements
      .map(stmt => `-- Question ${stmt.index}: ${stmt.title}\n${stmt.sql}\n`)
      .join('\n');

    fs.writeFileSync(outputFile, sqlContent);

    console.log(`✅ SQL statements saved to: ${outputFile}`);
    console.log('\n📋 Next steps:');
    console.log('   1. Execute individual statements using Supabase MCP');
    console.log('   2. Or copy-paste statements into Supabase SQL Editor');
    console.log('   3. Verify the questions in your Supabase dashboard\n');

    // Show first few statements as examples
    console.log('📄 First 3 SQL statements:');
    sqlStatements.slice(0, 3).forEach(stmt => {
      console.log(`\n-- Question ${stmt.index}: ${stmt.title}`);
