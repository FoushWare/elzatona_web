...
const outputSqlFile = path.join(__dirname, 'seed-javascript-questions.sql');
const outputBatchesDir = path.join(__dirname, 'javascript-batches');
...
function generateInsertStatement(question, categoryId) {
  ...
  const metadata = JSON.stringify({
    ...
    technology: question.technology || 'JavaScript',
  });
  const is_active =
...
async function generateJavaScriptQuestionsSql() {
  let allInsertStatements = [];
  const files = fs.readdirSync(dataDir).filter(file => file.endsWith('.json'));

  // JavaScript category ID
  const javascriptCategoryId = '70970873-3e49-46ec-91e0-2777ff9b9b42';

  console.log('Processing JavaScript questions from JSON files...');
  console.log(`Found ${files.length} JavaScript files`);

  for (const file of files) {
    const filePath = path.join(dataDir, file);
    const statements = processQuestionsFromFile(filePath, javascriptCategoryId);
    allInsertStatements = allInsertStatements.concat(statements);
  }

  console.log(
    `\nGenerated ${allInsertStatements.length} JavaScript INSERT statements`
  );
  ...
  console.log(
    'All JavaScript batch files created! Execute them using Supabase MCP tools.'
  );
}

generateJavaScriptQuestionsSql();
