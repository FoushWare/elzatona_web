...
const dataDir = path.join(__dirname, '../apps/admin/network/data/json/css');
const outputSqlFile = path.join(__dirname, 'seed-css-questions.sql');
const outputBatchesDir = path.join(__dirname, 'css-batches');
...
function generateInsertStatement(question, categoryId) {
  ...
  const test_cases =
    ...
    question.test_cases.length > 0
      ? `ARRAY[${question.test_cases.map(tc => `'${escapeSingleQuotes(tc)}'`).join(',')}]::text[]`
      : `ARRAY[]::text[]`;
  const hints =
...
async function generateCssQuestionsSql() {
  let allInsertStatements = [];

  // CSS category ID from database
  const cssCategoryId = '3bd9c23e-6e43-4234-b741-ef78955a90a0';

  console.log('Processing CSS questions from JSON files...');
  ...
  for (const file of files) {
    ...
    const statements = processQuestionsFromFile(filePath, cssCategoryId);
    allInsertStatements = allInsertStatements.concat(statements);
  }

  console.log(
    `\nGenerated ${allInsertStatements.length} CSS INSERT statements`
  );
  ...
  console.log(
    'All CSS batch files created! Execute them using Supabase MCP tools.'
  );
}

generateCssQuestionsSql();
