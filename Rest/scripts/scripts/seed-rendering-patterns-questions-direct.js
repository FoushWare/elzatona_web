...
function generateInsertStatement(question, categoryId) {
  ...
  const metadata = {
    ...
    technology: question.technology || 'Rendering Patterns',
  };
  const is_active =
...
async function seedRenderingPatternsQuestions() {
  try {
    // Get Rendering Patterns category ID
    const { data: categoryData, error: categoryError } = await supabase
      .from('categories')
      .select('id')
      .eq('slug', 'rendering-patterns')
      .single();

    if (categoryError) {
      console.error(
        'Error fetching Rendering Patterns category ID:',
        categoryError
      );
      return;
    }
    ...
    console.log(`Found Rendering Patterns category ID: ${categoryId}`);

    // Process all Rendering Patterns files
    const files = fs
      .readdirSync(dataDir)
      .filter(file => file.endsWith('.json'));
    console.log(`Found ${files.length} Rendering Patterns files`);
    ...
    console.log(`\n=== Rendering Patterns Seeding Complete ===`);
    console.log(`Total questions inserted: ${totalInserted}`);
    console.log(`Total errors: ${totalErrors}`);
  } catch (error) {
    console.error('Error in seedRenderingPatternsQuestions:', error);
  }
}

seedRenderingPatternsQuestions();
