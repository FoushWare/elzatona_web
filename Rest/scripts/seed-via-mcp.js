...
async function executeBatch(batchNum) {
  const batchFile = path.join(BATCHES_DIR, `batch-${String(batchNum).padStart(2, '0')}.sql`);
  ...
  console.log(`📦 Executing batch ${batchNum} (${(sql.length / 1024).toFixed(1)} KB)...`);
  ...
  try {
    ...
    console.log(`   ✅ Batch ${batchNum} SQL loaded (${sql.split('VALUES').length - 1} questions)`);
...
async function main() {
  ...
  console.log(`   Total SQL size: ${batches.reduce((sum, b) => sum + (b.sql?.length || 0), 0) / 1024} KB`);
}
