...
function generateQuestion(id, title, content, topic, options, explanation, difficulty = 'intermediate') {
  return {
    id: `nextjs-senior-${id}`,
    title: title,
...
newQuestions.push(
  generateQuestion('sc-1', 
    ...
    'Server Components',
    [
      { text: 'RSC send serialized component trees as JSON, allowing selective client hydration, while SSR sends full HTML', isCorrect: true, explanation: 'RSC enables zero-JS components that never hydrate, reducing bundle size' },
      { text: 'RSC and SSR are identical - both render on the server', isCorrect: false },
      { text: 'RSC only work in Pages Router, SSR only in App Router', isCorrect: false },
      { text: 'RSC require manual hydration, SSR is automatic', isCorrect: false }
    ],
    'RSC send component trees as JSON, enabling zero-JS components and selective hydration, unlike SSR which sends full HTML.'
  ),
  generateQuestion('sc-2',
    ...
    'Server Components',
    [
      { text: 'It will cause a build-time error - Server Components cannot use hooks or browser APIs', isCorrect: true, explanation: 'Server Components run only on server, cannot use hooks, state, or browser APIs' },
      { text: 'It will work fine - Next.js handles it automatically', isCorrect: false },
      { text: 'It will cause a runtime error only in production', isCorrect: false },
      { text: 'It will automatically convert to a Client Component', isCorrect: false }
    ],
    'Server Components cannot use hooks, state, or browser APIs. They run only on the server.'
  ),
  generateQuestion('sc-3',
    ...
    'Server Components',
    [
      { text: 'Pass data as props - Server Components can render Client Components with props', isCorrect: true, explanation: 'Server Components can render Client Components and pass serializable props' },
      { text: 'Use Context API - it works across server/client boundary', isCorrect: false },
      { text: 'Use global variables - they are shared between server and client', isCorrect: false },
      { text: 'Use localStorage - it persists across server/client', isCorrect: false }
    ],
    'Server Components can render Client Components and pass serializable props (JSON-serializable data).'
  ),
  generateQuestion('sc-4',
    ...
    'Server Components',
    [
      { text: 'Async Server Components can use await for data fetching, regular ones cannot', isCorrect: true, explanation: 'Async Server Components can directly await promises and fetch data' },
      { text: 'Async Server Components run on client, regular ones on server', isCorrect: false },
      { text: 'There is no difference - all Server Components are async', isCorrect: false },
      { text: 'Async Server Components cannot receive props', isCorrect: false }
    ],
    'Async Server Components can directly await promises, making data fetching simpler without useEffect.'
  ),
  generateQuestion('sc-5',
    ...
    'Server Components',
    [
      { text: 'Yes, Server Components can import and render Client Components, but not the reverse', isCorrect: true, explanation: 'Server Components can render Client Components, but Client Components cannot import Server Components' },
      { text: 'No, Server and Client Components cannot interact', isCorrect: false },
      { text: 'Yes, but only if both are in the same directory', isCorrect: false },
      { text: 'No, Server Components can only render other Server Components', isCorrect: false }
    ],
  ...
  generateQuestion('sc-6',
    ...
    'Server Components',
    [
      { text: 'Build error - Server Components cannot have event handlers, use Client Components instead', isCorrect: true, explanation: 'Event handlers require client-side JavaScript, so they need Client Components' },
      { text: 'It works fine - Next.js handles it automatically', isCorrect: false },
      { text: 'Runtime error - handlers are undefined on server', isCorrect: false },
  ...
  generateQuestion('sc-7',
    ...
    'Server Components',
    [
      { text: 'Server Components re-render on the server when their data changes, sending updated JSON to client', isCorrect: true, explanation: 'RSC re-render on server and stream updates to client' },
      { text: 'Server Components never re-render - they are static', isCorrect: false },
      { text: 'Server Components re-render on client using React state', isCorrect: false },
      { text: 'Server Components re-render only on page navigation', isCorrect: false }
    ],
    'Server Components re-render on the server when their data changes, and Next.js streams the updates to the client.'
  ),
  generateQuestion('sc-8',
    ...
    'Server Components',
    [
      { text: 'Server Components reduce JavaScript bundle size since they never ship to client', isCorrect: true, explanation: 'Zero-JS components reduce bundle size and improve performance' },
      { text: 'Server Components are faster to render on client', isCorrect: false },
      { text: 'Server Components have better SEO than Client Components', isCorrect: false },
  ...
  generateQuestion('sc-9',
    'Can Server Components access environment variables directly?',
    'In a Server Component, can you access process.env variables?',
    'Server Components',
    [
      { text: 'Yes, Server Components can access all environment variables, including server-only ones', isCorrect: true, explanation: 'Server Components run on server, so they can access all env vars' },
      { text: 'No, only Client Components can access environment variables', isCorrect: false },
      { text: 'Yes, but only NEXT_PUBLIC_ prefixed variables', isCorrect: false },
      { text: 'No, environment variables are not available in Server Components', isCorrect: false }
    ],
...
newQuestions.push(
  generateQuestion('rendering-1',
    ...
    'Rendering',
    [
      { text: 'SSG generates pages at build time, ISR allows re-generating pages on-demand after build', isCorrect: true, explanation: 'ISR enables updating static pages without rebuilding the entire site' },
      { text: 'SSG is for Pages Router, ISR is for App Router only', isCorrect: false },
      { text: 'SSG uses getStaticProps, ISR uses getServerSideProps', isCorrect: false },
  ...
  generateQuestion('rendering-2',
    ...
    'Rendering',
    [
      { text: 'Use SSG for content that changes infrequently, SSR for content that needs to be fresh on every request', isCorrect: true, explanation: 'SSG for static/pre-rendered content, SSR for dynamic/fresh content' },
      { text: 'Always use SSR - it is faster than SSG', isCorrect: false },
      { text: 'Always use SSG - it is more secure than SSR', isCorrect: false },
      { text: 'Use SSR for public pages, SSG for authenticated pages', isCorrect: false }
    ],
  ...
  generateQuestion('rendering-3',
    'How does Next.js handle hybrid rendering strategies?',
    'Can you mix SSG, SSR, and client-side rendering in the same Next.js app?',
    'Rendering',
    [
      { text: 'Yes, Next.js supports per-page rendering strategies - you can mix SSG, SSR, and CSR', isCorrect: true, explanation: 'Next.js allows different rendering strategies per route' },
      { text: 'No, you must choose one strategy for the entire app', isCorrect: false },
      { text: 'Yes, but only in App Router, not Pages Router', isCorrect: false },
  ...
  generateQuestion('rendering-4',
    ...
    'Rendering',
    [
      { text: 'It enables ISR - pages are regenerated at most once per revalidate seconds', isCorrect: true, explanation: 'revalidate enables ISR with time-based revalidation' },
      { text: 'It controls how often the page is cached', isCorrect: false },
      { text: 'It sets the cache expiration time for the page', isCorrect: false },
  ...
  generateQuestion('rendering-5',
    ...
    'Rendering',
    [
      { text: 'Next.js automatically splits code by route and component, loading only what is needed', isCorrect: true, explanation: 'Automatic code splitting reduces initial bundle size' },
      { text: 'Next.js bundles everything into a single file for faster loading', isCorrect: false },
      { text: 'Next.js requires manual code splitting configuration', isCorrect: false },
  ...
  generateQuestion('rendering-6',
    ...
    'Rendering',
    [
      { text: 'getStaticProps runs at build time (SSG), getServerSideProps runs on every request (SSR)', isCorrect: true, explanation: 'getStaticProps = build time, getServerSideProps = request time' },
      { text: 'getStaticProps runs on client, getServerSideProps runs on server', isCorrect: false },
      { text: 'They are identical - both run on server', isCorrect: false },
      { text: 'getStaticProps is for App Router, getServerSideProps is for Pages Router', isCorrect: false }
    ],
    'getStaticProps runs at build time for SSG. getServerSideProps runs on every request for SSR.'
  ),
  generateQuestion('rendering-7',
    'How does Next.js handle streaming SSR in App Router?',
    'What is streaming SSR and how does Next.js implement it?',
    'Rendering',
    [
      { text: 'Next.js streams HTML as it renders, allowing progressive page loading with Suspense', isCorrect: true, explanation: 'Streaming enables progressive rendering and better perceived performance' },
      { text: 'Next.js waits for entire page to render before sending HTML', isCorrect: false },
      { text: 'Streaming SSR is not supported in Next.js', isCorrect: false },
      { text: 'Streaming SSR only works with Client Components', isCorrect: false }
    ],
  ...
  generateQuestion('rendering-8',
    ...
    'Rendering',
    [
      { text: 'generateStaticParams returns an array of params to pre-render at build time for dynamic routes', isCorrect: true, explanation: 'It enables SSG for dynamic routes by specifying which params to pre-render' },
      { text: 'generateStaticParams is used for API routes only', isCorrect: false },
      { text: 'generateStaticParams generates client-side routes', isCorrect: false },
      { text: 'generateStaticParams is not a valid Next.js function', isCorrect: false }
    ],
    'generateStaticParams returns an array of params to pre-render at build time, enabling SSG for dynamic routes.'
  ),
  generateQuestion('rendering-9',
    'How does Next.js handle partial prerendering?',
    'What is partial prerendering and how does Next.js support it?',
    'Rendering',
    [
      { text: 'Partial prerendering combines static and dynamic content, prerendering static parts and streaming dynamic parts', isCorrect: true, explanation: 'PPR optimizes performance by prerendering static shell and streaming dynamic content' },
      { text: 'Partial prerendering is not supported in Next.js', isCorrect: false },
      { text: 'Partial prerendering only works with Pages Router', isCorrect: false },
      { text: 'Partial prerendering requires manual configuration in next.config.js', isCorrect: false }
    ],
...
newQuestions.push(
  generateQuestion('config-1',
    ...
    'Configuration',
    [
      { text: 'Use webpack function in next.config.js to modify webpack configuration', isCorrect: true, explanation: 'next.config.js exports a webpack function for customization' },
      { text: 'Create a webpack.config.js file in the root directory', isCorrect: false },
      { text: 'Webpack configuration cannot be customized in Next.js', isCorrect: false },
      { text: 'Use webpack.config.js only in Pages Router, not App Router', isCorrect: false }
    ],
  ...
  generateQuestion('config-2',
    'What is the purpose of the experimental section in next.config.js?',
    'How do you enable experimental Next.js features?',
    'Configuration',
    [
      { text: 'The experimental section enables experimental features that are not yet stable', isCorrect: true, explanation: 'Experimental features are opt-in and may change' },
      { text: 'Experimental section is for testing only and does not affect production', isCorrect: false },
      { text: 'Experimental features are enabled by default', isCorrect: false },
      { text: 'Experimental section is not a valid Next.js configuration option', isCorrect: false }
    ],
  ...
  generateQuestion('config-3',
    ...
    'Configuration',
    [
      { text: 'Add domains to images.domains array in next.config.js', isCorrect: true, explanation: 'images.domains allows external image domains for next/image' },
      { text: 'Set NEXT_PUBLIC_IMAGE_DOMAINS environment variable', isCorrect: false },
      { text: 'next/image works with all domains by default', isCorrect: false },
  ...
  generateQuestion('config-4',
    'What is the purpose of the output option in next.config.js?',
    'How do you configure Next.js build output format?',
    'Configuration',
    [
      { text: 'output option controls build output - "standalone" creates minimal server, "export" creates static HTML', isCorrect: true, explanation: 'output controls whether to export static HTML or create standalone server' },
      { text: 'output option is for configuring API routes only', isCorrect: false },
      { text: 'output option controls where build files are saved', isCorrect: false },
      { text: 'output option is not a valid Next.js configuration', isCorrect: false }
    ],
    'The output option controls build output format. "standalone" creates a minimal server, "export" creates static HTML files.'
  ),
  generateQuestion('config-5',
    ...
    'Configuration',
    [
      { text: 'Use headers function in next.config.js to add custom headers', isCorrect: true, explanation: 'headers function allows adding/modifying HTTP headers' },
      { text: 'Create a headers.js file in the root directory', isCorrect: false },
      { text: 'Custom headers can only be added in middleware', isCorrect: false },
  ...
  generateQuestion('config-6',
    ...
    'Configuration',
    [
      { text: 'transpilePackages allows transpiling node_modules packages that are not ES5 compatible', isCorrect: true, explanation: 'Useful for packages that need to be transpiled for browser compatibility' },
      { text: 'transpilePackages is for transpiling TypeScript files only', isCorrect: false },
      { text: 'transpilePackages is not a valid Next.js option', isCorrect: false },
      { text: 'transpilePackages automatically transpiles all node_modules', isCorrect: false }
    ],
...
newQuestions.push(
  generateQuestion('deploy-1',
    ...
    'Deployment',
    [
      { text: '"next start" runs a Node.js server for SSR/API routes, "next export" creates static HTML files', isCorrect: true, explanation: 'next start = Node server, next export = static files' },
      { text: 'Both methods are identical', isCorrect: false },
      { text: '"next start" is for development, "next export" is for production', isCorrect: false },
      { text: '"next export" requires Node.js server, "next start" creates static files', isCorrect: false }
    ],
    '"next start" runs a Node.js server supporting SSR and API routes. "next export" creates static HTML files for static hosting.'
  ),
  generateQuestion('deploy-2',
    'How do you deploy a Next.js app to Vercel?',
    'What is the recommended way to deploy Next.js to Vercel?',
    'Deployment',
    [
      { text: 'Connect your Git repository to Vercel - it automatically detects Next.js and deploys', isCorrect: true, explanation: 'Vercel has built-in Next.js support with automatic detection' },
      { text: 'You must manually build and upload files to Vercel', isCorrect: false },
      { text: 'Vercel does not support Next.js deployment', isCorrect: false },
      { text: 'You need to configure next.config.js specifically for Vercel', isCorrect: false }
    ],
  ...
  generateQuestion('deploy-3',
    ...
    'Deployment',
    [
      { text: 'NEXT_PUBLIC_ variables are exposed to browser, regular variables are server-only', isCorrect: true, explanation: 'NEXT_PUBLIC_ prefix makes variables available to client-side code' },
      { text: 'All environment variables are available everywhere', isCorrect: false },
      { text: 'NEXT_PUBLIC_ variables are server-only', isCorrect: false },
      { text: 'Environment variables are not supported in Next.js', isCorrect: false }
    ],
  ...
  generateQuestion('deploy-4',
    ...
    'Deployment',
    [
      { text: 'Create a custom server.js file and use next() to handle requests, then run "node server.js"', isCorrect: true, explanation: 'Custom server wraps Next.js app and handles requests before passing to Next.js' },
  ...
  generateQuestion('deploy-5',
    ...
    'Deployment',
    [
      { text: 'Standalone mode creates a minimal server with only necessary files, useful for Docker deployments', isCorrect: true, explanation: 'Standalone reduces deployment size by including only required files' },
      { text: 'Standalone mode creates static HTML files only', isCorrect: false },
      { text: 'Standalone mode is for development only', isCorrect: false },
      { text: 'Standalone mode is not a valid Next.js feature', isCorrect: false }
    ],
  ...
  generateQuestion('deploy-6',
    ...
    'Deployment',
    [
      { text: 'Use .env.local, .env.development, .env.production files and environment variables in deployment platform', isCorrect: true, explanation: 'Next.js loads env files based on NODE_ENV and deployment platform env vars' },
      { text: 'Create separate next.config.js files for each environment', isCorrect: false },
      { text: 'Environment-specific configs are not supported', isCorrect: false },
...
newQuestions.push(
  generateQuestion('api-1',
    'How do you handle CORS in Next.js API routes?',
    'You need to enable CORS for your Next.js API route. How do you do this?',
    'API Routes',
    [
      { text: 'Set CORS headers manually in the API route handler response', isCorrect: true, explanation: 'Manually set Access-Control-Allow-Origin and related headers in the response' },
      { text: 'CORS is automatically enabled for all API routes', isCorrect: false },
  ...
  generateQuestion('api-2',
    ...
    'API Routes',
    [
      { text: 'Pages Router uses pages/api, App Router uses app/api with route handlers (async functions)', isCorrect: true, explanation: 'App Router uses route handlers (async functions) instead of default exports' },
  ...
  generateQuestion('api-3',
    ...
    'API Routes',
    [
      { text: 'Use a library like formidable or multer to parse multipart/form-data, or use FormData API', isCorrect: true, explanation: 'Parse multipart data using libraries or FormData API' },
      { text: 'Next.js handles file uploads automatically', isCorrect: false },
      { text: 'File uploads are not supported in API routes', isCorrect: false },
  ...
  generateQuestion('api-4',
    'Can Next.js API routes be deployed as serverless functions?',
    'How are API routes deployed on platforms like Vercel?',
    'API Routes',
    [
      { text: 'Yes, API routes are automatically deployed as serverless functions on Vercel', isCorrect: true, explanation: 'Each API route becomes a separate serverless function' },
  ...
  generateQuestion('api-5',
    ...
    'API Routes',
    [
      { text: 'Use ReadableStream or Response with streaming body in App Router, or res.write() in Pages Router', isCorrect: true, explanation: 'Streaming enables sending data incrementally' },
...
newQuestions.push(
  generateQuestion('auth-1',
    ...
    'Authentication',
    [
      { text: 'Use middleware to protect routes and handle auth logic, or use libraries like NextAuth.js', isCorrect: true, explanation: 'Middleware runs before route handlers, perfect for auth checks' },
      { text: 'Authentication must be handled only in API routes', isCorrect: false },
      { text: 'Next.js has built-in authentication - no setup needed', isCorrect: false },
  ...
  generateQuestion('auth-2',
    ...
    'Authentication',
    [
      { text: 'Check authentication in the API route handler or use middleware to protect /api routes', isCorrect: true, explanation: 'Verify auth tokens/sessions in route handler or use middleware' },
      { text: 'API routes are automatically protected', isCorrect: false },
      { text: 'Use getServerSideProps to protect API routes', isCorrect: false },
  ...
  generateQuestion('auth-3',
    ...
    'Authentication',
    [
      { text: 'Read cookies/headers in Server Components and verify tokens server-side', isCorrect: true, explanation: 'Server Components can access cookies and headers to verify tokens' },
      { text: 'JWT tokens can only be verified in Client Components', isCorrect: false },
      { text: 'Use localStorage to store tokens in Server Components', isCorrect: false },
  ...
  generateQuestion('auth-4',
    ...
    'Authentication',
    [
      { text: 'Store auth tokens in httpOnly cookies for security, access via Server Components or middleware', isCorrect: true, explanation: 'httpOnly cookies prevent XSS attacks and are accessible server-side' },
      { text: 'Store tokens in localStorage for easy access', isCorrect: false },
  ...
  generateQuestion('auth-5',
    ...
    'Authentication',
    [
      { text: 'Check user roles in middleware or Server Components, redirect unauthorized users', isCorrect: true, explanation: 'Verify roles server-side and redirect or show error for unauthorized access' },
