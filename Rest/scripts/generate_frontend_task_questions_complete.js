...
function generateQuestion(id, title, content, topic, options, explanation, difficulty = 'intermediate') {
  return {
    ...
    createdBy: 'admin',
    updatedBy: 'admin',
    tags: ['react', 'frontend-tasks', topic.toLowerCase().replace(/\s+/g, '-'), difficulty],
...
allQuestions.push(
  generateQuestion('1', 'When building a social media dashboard frontend task, what is the best approach for component structure?', 'You need to build a social media dashboard with posts, comments, and user profiles. How should you organize your React components?', 'Core React', [
    { text: 'Create reusable components (PostCard, CommentList, UserProfile) and compose them in a Dashboard container', isCorrect: true },
    { text: 'Put everything in one large component for simplicity', isCorrect: false },
    { text: 'Create separate components for each feature without composition', isCorrect: false },
  ...
  generateQuestion('2', 'How should you structure components for a todo list frontend task?', 'You need to build a todo list with add, edit, delete, and filter functionality. What component structure is best?', 'Core React', [
    { text: 'TodoList container, TodoItem component, TodoForm component, FilterBar component', isCorrect: true },
  ...
  generateQuestion('3', 'What is the best pattern for sharing data between sibling components in a frontend task?', 'You have a ProductList and ShoppingCart component that need to share cart state. What approach should you use?', 'Core React', [
    { text: 'Lift state up to a common parent component or use Context API', isCorrect: true },
    { text: 'Use props to pass data between siblings directly', isCorrect: false },
...
allQuestions.push(
  generateQuestion('4', 'How should you handle state management in a complex frontend task like a video streaming app?', 'You\'re building a Netflix-style app with video player, recommendations, and watchlist. What state management approach is best?', 'Core React', [
    { text: 'Use Context API for global state (current video, user preferences) and useState for local component state', isCorrect: true },
    { text: 'Put all state in a single useState hook at the root', isCorrect: false },
  ...
  generateQuestion('5', 'When should you use useReducer instead of useState in a frontend task?', 'You\'re building a complex form with multiple interdependent fields and validation. Which hook is more appropriate?', 'Core React', [
    { text: 'useReducer when state logic is complex with multiple sub-values or when next state depends on previous state', isCorrect: true },
  ...
  generateQuestion('6', 'How do you prevent unnecessary re-renders in a frontend task with many child components?', 'Your dashboard has 100+ product cards that re-render when parent state changes. How do you optimize this?', 'Core React', [
    { text: 'Use React.memo for child components and useMemo/useCallback for expensive computations and callbacks', isCorrect: true },
...
allQuestions.push(
  generateQuestion('7', 'What is the best way to handle loading states in a frontend task with multiple API calls?', 'Your dashboard needs to fetch users, posts, and comments. How do you manage loading states?', 'Core React', [
    { text: 'Use separate loading states for each data fetch, or use React Query/SWR for automatic loading state management', isCorrect: true },
    { text: 'Show a single loading spinner for the entire page', isCorrect: false },
    { text: 'Don\'t show loading states - just render empty data', isCorrect: false },
  ...
  generateQuestion('8', 'How should you handle data fetching in a frontend task with pagination?', 'You need to implement infinite scroll for a product list. What approach is best?', 'Core React', [
    { text: 'Use Intersection Observer API to detect when user scrolls near bottom, then fetch next page and append to existing data', isCorrect: true },
    { text: 'Fetch all data at once and paginate client-side', isCorrect: false },
  ...
  generateQuestion('9', 'What is the best way to handle error states in API calls for a frontend task?', 'Your app makes multiple API calls. How should you handle and display errors?', 'Core React', [
    { text: 'Use try-catch blocks, set error state, and display user-friendly error messages with retry options', isCorrect: true },
...
allQuestions.push(
  generateQuestion('10', 'How do you optimize a frontend task with large lists of items?', 'You need to render 1000+ items in a list. What optimization technique should you use?', 'Core React', [
    { text: 'Use virtualization (react-window or react-virtualized) to render only visible items', isCorrect: true },
  ...
  generateQuestion('11', 'How should you optimize images in a frontend task like an e-commerce site?', 'Your product catalog has hundreds of product images. How do you optimize loading?', 'Core React', [
    { text: 'Use lazy loading, responsive images with srcset, WebP format, and image CDN with proper sizing', isCorrect: true },
  ...
  generateQuestion('12', 'What is code splitting and how should you implement it in a frontend task?', 'Your app is getting large. How do you reduce initial bundle size?', 'Core React', [
    { text: 'Use React.lazy() and Suspense for route-based code splitting, and dynamic imports for heavy components', isCorrect: true },
...
allQuestions.push(
  generateQuestion('13', 'How should you handle form validation in a frontend task?', 'You need to build a registration form with validation. What approach is best?', 'Core React', [
    { text: 'Use controlled components with validation logic, or libraries like React Hook Form or Formik for complex forms', isCorrect: true },
  ...
  generateQuestion('14', 'What is the difference between controlled and uncontrolled components in forms?', 'When building a form in a frontend task, which approach should you use?', 'Core React', [
    { text: 'Controlled: React controls form state via state. Uncontrolled: DOM handles form state via refs. Use controlled for React integration.', isCorrect: true },
  ...
  generateQuestion('15', 'How do you handle file uploads in a frontend task?', 'You need to allow users to upload profile pictures. How should you implement this?', 'Core React', [
    { text: 'Use input type="file", handle File API, show preview, validate file type/size, and upload via FormData to API', isCorrect: true },
...
allQuestions.push(
  generateQuestion('16', 'How do you implement real-time updates in a frontend task like a chat app?', 'You need to show new messages as they arrive. What technology should you use?', 'Core React', [
    { text: 'Use WebSockets (Socket.io) or Server-Sent Events for real-time communication, update state when messages arrive', isCorrect: true },
  ...
  generateQuestion('17', 'How should you handle WebSocket connections in a React frontend task?', 'You\'re building a collaborative editor with real-time updates. How do you manage the WebSocket connection?', 'Core React', [
    { text: 'Create WebSocket connection in useEffect, store in useRef, cleanup on unmount, handle reconnection logic', isCorrect: true },
    { text: 'Create connection in component body', isCorrect: false },
    { text: 'Use only one global connection for entire app', isCorrect: false },
  ...
  generateQuestion('18', 'How do you prevent memory leaks when using WebSockets in a frontend task?', 'Your chat component unmounts but WebSocket listeners remain. How do you fix this?', 'Core React', [
    { text: 'Remove event listeners in useEffect cleanup function and close WebSocket connection on unmount', isCorrect: true },
...
allQuestions.push(
  generateQuestion('19', 'How should you implement routing in a frontend task with multiple pages?', 'Your app needs multiple pages: home, products, cart, checkout. What routing solution should you use?', 'React Router', [
    { text: 'Use React Router with BrowserRouter, Routes, and Route components for declarative routing', isCorrect: true },
  ...
  generateQuestion('20', 'How do you protect routes in a frontend task that requires authentication?', 'Some pages should only be accessible to logged-in users. How do you implement this?', 'React Router', [
    { text: 'Create a ProtectedRoute component that checks auth state and redirects to login if not authenticated', isCorrect: true },
  ...
  generateQuestion('21', 'How should you handle deep linking in a frontend task?', 'Users should be able to bookmark and share specific product pages. How do you enable this?', 'React Router', [
    { text: 'Use React Router with proper route structure, ensure server supports client-side routing (catch-all route)', isCorrect: true },
...
allQuestions.push(
  generateQuestion('22', 'How should you test a frontend task component?', 'You need to test a ProductCard component. What testing approach is best?', 'React Testing', [
    { text: 'Use React Testing Library to test user interactions and rendered output, not implementation details', isCorrect: true },
  ...
  generateQuestion('23', 'How do you test async operations in a frontend task?', 'Your component fetches data from an API. How should you test this?', 'React Testing', [
    { text: 'Mock the API call, use waitFor to wait for async updates, and assert on final rendered state', isCorrect: true },
  ...
  generateQuestion('24', 'What should you test in a frontend task form component?', 'You have a registration form. What aspects should be tested?', 'React Testing', [
    { text: 'Test user interactions (typing, submitting), validation errors, successful submission, and form state changes', isCorrect: true },
...
allQuestions.push(
  generateQuestion('25', 'How do you ensure accessibility in a frontend task?', 'Your dashboard needs to be accessible to screen readers. What should you implement?', 'Core React', [
    { text: 'Use semantic HTML, ARIA attributes where needed, keyboard navigation, focus management, and test with screen readers', isCorrect: true },
  ...
  generateQuestion('26', 'How should you handle focus management in a modal frontend task?', 'You have a modal dialog. How do you manage focus for accessibility?', 'Core React', [
    { text: 'Trap focus within modal when open, return focus to trigger when closed, use useRef and useEffect for focus management', isCorrect: true },
  ...
  generateQuestion('27', 'How do you implement keyboard shortcuts in a frontend task?', 'You want Ctrl+K to open search. How should you implement this?', 'Core React', [
    { text: 'Use useEffect with keydown event listener, check for modifier keys (Ctrl/Cmd), cleanup on unmount', isCorrect: true },
...
allQuestions.push(
  generateQuestion('28', 'How should you structure a large frontend task application?', 'Your app is growing with many features. How should you organize the codebase?', 'Core React', [
    { text: 'Organize by features/modules, use barrel exports, separate concerns (components, hooks, utils, types), and use absolute imports', isCorrect: true },
  ...
  generateQuestion('29', 'How do you handle error boundaries in a frontend task?', 'You want to prevent one component error from crashing the entire app. How do you implement this?', 'Core React', [
    { text: 'Create ErrorBoundary class component with componentDidCatch, wrap error-prone sections, display fallback UI', isCorrect: true },
  ...
  generateQuestion('30', 'How should you optimize bundle size for a production frontend task?', 'Your app bundle is too large. What optimization strategies should you use?', 'Core React', [
    { text: 'Use code splitting, tree shaking, analyze bundle, remove unused dependencies, use dynamic imports, and optimize images/assets', isCorrect: true },
