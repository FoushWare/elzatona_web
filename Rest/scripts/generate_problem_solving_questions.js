...
const outputFile = path.join(__dirname, '../final-questions-v01/problem-solving-questions.json');
...
function generateQuestion(id, title, content, difficulty, options, explanation, codeExample = '') {
  return {
...
questions.push(generateQuestion('easy-1',
  ...
  'easy',
  [
    { text: 'Use a hash map to store complements. Iterate once, checking if complement exists. Time: O(n), Space: O(n)', isCorrect: true },
    { text: 'Use nested loops to check all pairs. Time: O(n²), Space: O(1)', isCorrect: false },
    { text: 'Sort the array first, then use two pointers. Time: O(n log n), Space: O(1)', isCorrect: false },
    { text: 'Use recursion to check all combinations. Time: O(2ⁿ), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-2',
  'Validate Balanced Parentheses',
  'Given a string containing only parentheses, brackets, and braces, determine if the string is valid (properly closed).\n\nExample: "()[]{}" → true, "([)]" → false',
  'easy',
  [
    { text: 'Use a stack. Push opening brackets, pop when matching closing bracket. Check if stack is empty at end. Time: O(n), Space: O(n)', isCorrect: true },
    { text: 'Count opening and closing brackets. If counts match, it\'s valid. Time: O(n), Space: O(1)', isCorrect: false },
    { text: 'Use regex to remove pairs repeatedly until no more pairs. Time: O(n²), Space: O(n)', isCorrect: false },
    { text: 'Check if string length is even. Time: O(1), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-3',
  'Reverse a String',
  'Given a string, reverse it in-place (if array) or return reversed string.\n\nExample: "hello" → "olleh"',
  'easy',
  [
    { text: 'Use two pointers from start and end, swap characters. Time: O(n), Space: O(1) for array, O(n) for string', isCorrect: true },
    { text: 'Create new string by iterating backwards. Time: O(n), Space: O(n)', isCorrect: false },
    { text: 'Use built-in reverse() method. Time: O(n), Space: O(n)', isCorrect: false },
    { text: 'Use recursion to swap characters. Time: O(n), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-4',
  'Check for Duplicate Values',
  'Given an array, determine if any value appears at least twice.\n\nExample: [1, 2, 3, 1] → true',
  'easy',
  [
    { text: 'Use a Set to track seen values. If value exists in Set, return true. Time: O(n), Space: O(n)', isCorrect: true },
    { text: 'Sort array and check adjacent elements. Time: O(n log n), Space: O(1)', isCorrect: false },
    { text: 'Use nested loops to compare all pairs. Time: O(n²), Space: O(1)', isCorrect: false },
    { text: 'Use filter and length comparison. Time: O(n), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-5',
  ...
  'easy',
  [
    { text: 'Track minimum price seen so far. For each price, calculate profit and update max profit. Time: O(n), Space: O(1)', isCorrect: true },
    { text: 'Try all buy-sell pairs using nested loops. Time: O(n²), Space: O(1)', isCorrect: false },
    { text: 'Find min and max values. Time: O(n), Space: O(1)', isCorrect: false },
    { text: 'Sort prices and find difference. Time: O(n log n), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-6',
  'Check if Two Strings are Anagrams',
  'Given two strings, determine if they are anagrams (same characters, different order).\n\nExample: "listen" and "silent" → true',
  'easy',
  [
    { text: 'Sort both strings and compare. Time: O(n log n), Space: O(n)', isCorrect: true },
    { text: 'Count character frequencies using hash map. Compare counts. Time: O(n), Space: O(1) - limited alphabet', isCorrect: true },
    { text: 'Use nested loops to check each character. Time: O(n²), Space: O(1)', isCorrect: false },
    { text: 'Compare string lengths only. Time: O(1), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-7',
  'Find Maximum Sum Subarray',
  'Given an array of integers, find the contiguous subarray with the largest sum.\n\nExample: [-2, 1, -3, 4, -1, 2, 1, -5, 4] → 6 ([4, -1, 2, 1])',
  'easy',
  [
    { text: 'Kadane\'s algorithm: track current sum and max sum. Reset current sum if negative. Time: O(n), Space: O(1)', isCorrect: true },
    { text: 'Try all possible subarrays using nested loops. Time: O(n²), Space: O(1)', isCorrect: false },
    { text: 'Use divide and conquer approach. Time: O(n log n), Space: O(log n)', isCorrect: false },
    { text: 'Find sum of all positive numbers. Time: O(n), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-8',
  'Merge Two Sorted Arrays',
  'Given two sorted arrays, merge them into one sorted array.\n\nExample: [1, 2, 3] and [2, 5, 6] → [1, 2, 2, 3, 5, 6]',
  'easy',
  [
    { text: 'Use two pointers, compare elements and add smaller one. Time: O(n + m), Space: O(n + m)', isCorrect: true },
    { text: 'Concatenate and sort. Time: O((n+m) log(n+m)), Space: O(n+m)', isCorrect: false },
    { text: 'Use nested loops to insert elements. Time: O(n*m), Space: O(n+m)', isCorrect: false },
    { text: 'Use Set to remove duplicates then sort. Time: O((n+m) log(n+m)), Space: O(n+m)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-9',
  ...
  'easy',
  [
    { text: 'Use dynamic programming: ways[i] = ways[i-1] + ways[i-2]. Time: O(n), Space: O(n) or O(1) with optimization', isCorrect: true },
    { text: 'Use recursion without memoization. Time: O(2ⁿ), Space: O(n)', isCorrect: false },
    { text: 'Try all possible combinations. Time: O(2ⁿ), Space: O(n)', isCorrect: false },
    { text: 'Use factorial calculation. Time: O(n), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('easy-10',
  'Check if Number is Palindrome',
  'Determine if an integer is a palindrome (reads same forwards and backwards).\n\nExample: 121 → true, -121 → false',
  'easy',
  [
    { text: 'Reverse half of the number and compare with other half. Time: O(log n), Space: O(1)', isCorrect: true },
    { text: 'Convert to string and check if reversed equals original. Time: O(log n), Space: O(log n)', isCorrect: true },
    { text: 'Use modulo and division to extract digits. Time: O(log n), Space: O(log n)', isCorrect: false },
    { text: 'Compare first and last digit only. Time: O(1), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-1',
  'Find Longest Substring Without Repeating Characters',
  'Given a string, find the length of the longest substring without repeating characters.\n\nExample: "abcabcbb" → 3 ("abc")',
  'medium',
  [
    { text: 'Use sliding window with hash map. Expand window, shrink when duplicate found. Time: O(n), Space: O(min(n, m)) where m is charset size', isCorrect: true },
    { text: 'Check all possible substrings. Time: O(n³), Space: O(n)', isCorrect: false },
    { text: 'Use Set and check each substring. Time: O(n²), Space: O(n)', isCorrect: false },
    { text: 'Sort characters and find longest sequence. Time: O(n log n), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-2',
  ...
  'medium',
  [
    { text: 'Use two pointers at start and end. Move pointer with smaller height. Calculate area and track max. Time: O(n), Space: O(1)', isCorrect: true },
    { text: 'Try all pairs of lines. Time: O(n²), Space: O(1)', isCorrect: false },
    { text: 'Find two tallest lines. Time: O(n), Space: O(1)', isCorrect: false },
    { text: 'Sort heights and use top two. Time: O(n log n), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-3',
  'Find All Unique Triplets That Sum to Zero',
  'Given an array, find all unique triplets that sum to zero.\n\nExample: [-1, 0, 1, 2, -1, -4] → [[-1, -1, 2], [-1, 0, 1]]',
  'medium',
  [
    { text: 'Sort array. For each element, use two pointers to find pairs that sum to negative of current element. Skip duplicates. Time: O(n²), Space: O(1) excluding output', isCorrect: true },
    { text: 'Use three nested loops. Time: O(n³), Space: O(1)', isCorrect: false },
    { text: 'Use hash map for all pairs, then check for third element. Time: O(n²), Space: O(n²)', isCorrect: false },
    { text: 'Find all combinations using recursion. Time: O(2ⁿ), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-4',
  'Group Strings by Anagram',
  'Given an array of strings, group anagrams together.\n\nExample: ["eat", "tea", "tan", "ate", "nat", "bat"] → [["eat","tea","ate"], ["tan","nat"], ["bat"]]',
  'medium',
  [
    { text: 'Use sorted string as key in hash map. Group strings with same sorted key. Time: O(n*k log k) where k is avg string length, Space: O(n*k)', isCorrect: true },
    { text: 'Compare each string with all others. Time: O(n²*k), Space: O(n)', isCorrect: false },
    { text: 'Use character frequency as key. Time: O(n*k), Space: O(n*k)', isCorrect: true },
    { text: 'Sort entire array and group adjacent anagrams. Time: O(n*k log(n*k)), Space: O(n*k)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-5',
  'Find Longest Palindromic Substring',
  'Given a string, find the longest palindromic substring.\n\nExample: "babad" → "bab" or "aba"',
  'medium',
  [
    { text: 'Expand around center for each possible center (odd and even length). Track longest. Time: O(n²), Space: O(1)', isCorrect: true },
    { text: 'Check all possible substrings. Time: O(n³), Space: O(1)', isCorrect: false },
    { text: 'Use dynamic programming. Time: O(n²), Space: O(n²)', isCorrect: true },
    { text: 'Reverse string and find longest common substring. Time: O(n²), Space: O(n²)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-6',
  'Product of Array Except Self',
  'Given an array, return an array where each element is the product of all other elements (without using division).\n\nExample: [1, 2, 3, 4] → [24, 12, 8, 6]',
  'medium',
  [
    { text: 'Two passes: left products, then right products. Multiply corresponding elements. Time: O(n), Space: O(1) excluding output', isCorrect: true },
    { text: 'Calculate product of all elements, divide by each element. Time: O(n), Space: O(1)', isCorrect: false },
    { text: 'For each element, calculate product of all others. Time: O(n²), Space: O(1)', isCorrect: false },
    { text: 'Use nested loops to calculate products. Time: O(n²), Space: O(n)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-7',
  ...
  'medium',
  [
    { text: 'Use three sets of hash maps: one for rows, one for columns, one for boxes. Check each cell against all three. Time: O(1) - fixed 81 cells, Space: O(1)', isCorrect: true },
    { text: 'Check each row, column, and box separately. Time: O(1), Space: O(1)', isCorrect: true },
    { text: 'Use nested loops to check all combinations. Time: O(n⁴), Space: O(1)', isCorrect: false },
    { text: 'Sort each row, column, and box. Time: O(1), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-8',
  ...
  'medium',
  [
    { text: 'Use dynamic programming: dp[i] = length of LIS ending at i. Time: O(n²), Space: O(n)', isCorrect: true },
    { text: 'Use binary search with patience sorting. Maintain array of smallest tail of increasing subsequences. Time: O(n log n), Space: O(n)', isCorrect: true },
    { text: 'Try all possible subsequences. Time: O(2ⁿ), Space: O(n)', isCorrect: false },
    { text: 'Sort array and find longest consecutive sequence. Time: O(n log n), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-9',
  'Minimum Coins to Make Amount',
  'Given coins of different denominations and a total amount, find the minimum number of coins needed.\n\nExample: coins = [1, 2, 5], amount = 11 → 3 (5 + 5 + 1)',
  'medium',
  [
    { text: 'Use dynamic programming: dp[i] = min coins for amount i. For each coin, update dp[i + coin]. Time: O(amount * coins.length), Space: O(amount)', isCorrect: true },
    { text: 'Use greedy: always pick largest coin. Time: O(amount), Space: O(1)', isCorrect: false },
    { text: 'Try all combinations using recursion. Time: O(2ⁿ), Space: O(n)', isCorrect: false },
    { text: 'Sort coins and use largest first. Time: O(n log n), Space: O(1)', isCorrect: false }
  ],
...
questions.push(generateQuestion('medium-10',
  ...
  'medium',
  [
    { text: 'Use backtracking with DFS. For each cell, explore all directions, mark visited, backtrack. Time: O(m*n*4^L) where L is word length, Space: O(L)', isCorrect: true },
    { text: 'Try all possible paths using BFS. Time: O(m*n*4^L), Space: O(m*n)', isCorrect: false },
    { text: 'Check each cell independently. Time: O(m*n*L), Space: O(1)', isCorrect: false },
    { text: 'Convert board to string and search. Time: O(m*n), Space: O(m*n)', isCorrect: false }
  ],
