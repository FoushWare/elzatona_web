...
function fixNestedCodeTags(text) {
  ...
  while (previous !== fixed && iterations < maxIterations) {
    ...
    fixed = fixed.replace(/<code>([^<]*)<code>([^<]*)<\/code>([^<]*)<\/code>/g, 
    ...
    fixed = fixed.replace(/<code>((?:[^<]|<(?!\/?code>))*?)<code>([^<]*)<\/code>((?:[^<]|<(?!\/?code>))*?)<code>([^<]*)<\/code>((?:[^<]|<(?!\/?code>))*?)<\/code>/g,
    ...
    fixed = fixed.replace(/<code>((?:[^<]|<(?!\/?code>))*?)<code>([^<]*)<\/code>((?:[^<]|<(?!\/?code>))*?)<code>([^<]*)<\/code>((?:[^<]|<(?!\/?code>))*?)<code>([^<]*)<\/code>((?:[^<]|<(?!\/?code>))*?)<\/code>/g,
      (match, before, inner1, mid1, inner2, mid2, inner3, after) => {
        const combined = (before + inner1 + mid1 + inner2 + mid2 + inner3 + after).trim();
        return `<code>${combined}</code>`;
      }
    );
  }
...
function formatQuestion(question) {
  ...
  if (formatted.options && Array.isArray(formatted.options)) {
    formatted.options = formatted.options.map(option => ({
      ...
      explanation: option.explanation ? fixNestedCodeTags(option.explanation) : option.explanation
    }));
  }
