...
if (!urlProjectRef) {
  console.error(
    '❌ Could not extract project reference from URL:',
    supabaseUrl
  );
  process.exit(1);
}
...
try {
  ...
  if (keyProjectRef === urlProjectRef && keyRole === 'service_role') {
    ...
    console.error(
      '❌ MISMATCH: Service role key does not match the project URL!'
    );
    console.error('');
    ...
    console.error(
      '4. Copy the service_role key (click the eye icon to reveal)'
    );
    console.error('5. Update SUPABASE_SERVICE_ROLE_KEY in .env.test.local');
    console.error('');
    console.error(
      `Direct link: https://supabase.com/dashboard/project/${urlProjectRef}/settings/api`
    );
    process.exit(1);
  }
} catch (error) {
