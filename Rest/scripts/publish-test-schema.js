...
async function publishSchema() {
  try {
    ...
    if (!response.ok) {
      ...
      console.log(
        `   supabase db push --db-url "postgresql://postgres:[PASSWORD]@${supabaseUrl.split('//')[1].split('.')[0]}.supabase.co:5432/postgres" < ${sqlFilePath}`
      );
      return;
    }
    ...
    console.error(
      `   File: ${path.resolve(projectRoot, 'Rest/scripts/test-database-schema.sql')}`
    );
    console.error(
      `   Dashboard: https://supabase.com/dashboard/project/${supabaseUrl.split('//')[1].split('.')[0]}/sql/new`
    );
    process.exit(1);
  }
}
...
async function publishSchemaAlternative() {
  ...
  if (fs.existsSync(sqlFilePath)) {
    ...
    console.log(
      `   Contains ${sql.split('CREATE TABLE').length - 1} CREATE TABLE statements`
    );
    console.log('');
  }
}
