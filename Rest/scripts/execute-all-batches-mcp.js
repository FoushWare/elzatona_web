...
async function executeSQL(sql) {
  try {
    ...
    console.log(
      '   💡 Use MCP tools (mcp_supabase_execute_sql) to execute SQL batches.'
    );
...
async function main() {
  ...
  for (let i = 1; i <= 37; i++) {
    const batchFile = path.join(
      BATCHES_DIR,
      `batch-${String(i).padStart(2, '0')}.sql`
    );
    if (fs.existsSync(batchFile)) {
      ...
      console.log(
        `   ✅ Batch ${i}: ${questionCount} questions, ${(sql.length / 1024).toFixed(1)} KB`
      );
    }
  }
