...
async function seedTopics() {
  ...
  Object.entries(categoryMapping).forEach(([prodId, testId]) => {
    console.log(`  ${prodId.substring(0, 8)}... -> ${testId.substring(0, 8)}...`);
  });
  
  console.log('\n💡 Note: This script requires topics data from production.');
  console.log('   Use MCP to get topics, then pass them to this script or insert directly via MCP.');
  console.log('   Or update this script to query production if you have the production service key.');
}
