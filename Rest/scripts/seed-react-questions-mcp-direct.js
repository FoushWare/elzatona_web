...
async function getTopicsFromDB() {
  ...
  const topics = [
    { id: 'd9e02bfd-4293-42de-a9a8-65eeb7c61e32', name: 'Core React', slug: 'react-core-react' },
...
function findTopicId(questionTopic, allTopics) {
  ...
  for (const topic of allTopics) {
    if (topic.name.toLowerCase() === topicKey || topic.slug.toLowerCase() === topicKey) {
      return topic.id;
    }
  }
  ...
  // Partial match
  for (const topic of allTopics) {
    if (topic.name.toLowerCase().includes(topicKey) || topicKey.includes(topic.name.toLowerCase())) {
      return topic.id;
    }
  }
...
function generateInsertSQL(questions, categoryId, topics) {
  ...
  const values = questions.map(q => {
    ...
    } else if (q.options && typeof q.options === 'object' && q.correctAnswer) {
...
async function main() {
  ...
  console.log('   Run: SELECT id, name, slug FROM topics WHERE category_id = \'1d54dd10-68fe-4ea9-874e-c960930e0402\';\n');
  ...
  for (let i = 0; i < questions.length; i += BATCH_SIZE) {
    ...
    console.log(`   ✅ Generated batch ${sqlBatches.length}: ${batch.length} questions`);
  }
  ...
  const allSQL = sqlBatches.map(b => 
    `-- Batch ${b.batchNum} (${b.count} questions)\n${b.sql}\n`
  ).join('\n');
