...
const questionTemplates = {
  HTML: [
    { id: 1, topic: 'HTML5 Semantics', title: 'What is the difference between <code>&lt;article&gt;</code> and <code>&lt;section&gt;</code> in terms of accessibility and SEO?', content: 'How do these semantic elements differ in their semantic meaning and impact on assistive technologies?', options: [
    ...
    ], explanation: '<article> is for standalone, distributable content. <section> is for thematic grouping. This affects how screen readers and search engines understand content structure.' },
    { id: 2, topic: 'Performance', title: 'How does the <code>loading="lazy"</code> attribute on images affect Core Web Vitals?', content: 'What is the impact of lazy loading images on LCP (Largest Contentful Paint) and CLS (Cumulative Layout Shift)?', options: [
    ...
    ], explanation: 'Lazy loading defers offscreen images, improving initial LCP. However, if image dimensions are not set, it can cause CLS when images load. Always set width and height attributes.' },
    { id: 3, topic: 'HTML5 APIs', title: 'What is the purpose of the <code>&lt;template&gt;</code> element and when should you use it?', content: 'How does the template element differ from hidden divs for client-side templating?', options: [
    ...
    ], explanation: '<template> holds inert HTML that is not rendered until cloned via JavaScript. Unlike hidden divs, template content is not part of the DOM and does not affect rendering or accessibility.' }
...
console.log('📝 Due to the large volume, questions will be generated in batches');
...
console.log(`✅ Created master runner script at: ${path.join(batchesBaseDir, 'run_all_batches.js')}`);
console.log('\n📝 Next steps:');
console.log('   1. Generate question batches (this requires expanding the question templates)');
console.log('   2. Run: node Rest/scripts/comprehensive-batches/run_all_batches.js');
