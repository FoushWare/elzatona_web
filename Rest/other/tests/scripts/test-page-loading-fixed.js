...
async function testPageLoading() {
  ...
  try {
    ...
    const cardsSection = await page.$('h2');
    if (cardsSection) {
      const cardsText = await page.evaluate(el => el.textContent, cardsSection);
      if (cardsText.includes('Learning Cards')) {
    ...
    const plansSection = await page.$('h2');
    if (plansSection) {
      const plansText = await page.evaluate(el => el.textContent, plansSection);
      if (plansText.includes('Learning Plans')) {
