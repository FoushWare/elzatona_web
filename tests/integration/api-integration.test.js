...
const createMockApp = () => {
  ...
  app.post('/api/admin/questions', async (req, res) => {
    const { title, content, category, learningPath, type, difficulty } = req.body;
    ...
    const matchingSections = mockSections.filter(section => 
  ...
  app.get('/api/guided-learning/plans', async (req, res) => {
    ...
    if (getSections) {
      ...
      if (learningPath) {
        filteredSections = filteredSections.filter(s => s.learningPathId === learningPath);
      }
...
describe('API Integration Tests for Auto-linking System', () => {
  ...
  describe('Guided Learning Plans API', () => {
    ...
    test('should filter sections by learning path', async () => {
      const response = await request(app)
        .get('/api/guided-learning/plans?getSections=true&learningPath=frontend')
        .expect(200);
      
      expect(response.body.success).toBe(true);
      expect(response.body.data).toHaveLength(2);
      expect(response.body.data.every(s => s.learningPathId === 'frontend')).toBe(true);
    });
    
    test('should filter sections by both category and learning path', async () => {
      const response = await request(app)
        .get('/api/guided-learning/plans?getSections=true&category=JavaScript&learningPath=frontend')
        .expect(200);
    ...
    test('should complete full workflow: create question -> auto-link -> add to plan', async () => {
      ...
      const planResponse = await request(app)
        ...
        .send({
          ...
          difficulty: 'beginner',
          sections: [
            { sectionId: 'section1', order: 1, weight: 100 }
          ]
        })
        .expect(200);
      ...
      expect(updateResponse.body.totalQuestions).toBe(sectionsResponse.body.count);
    });
  });
});
