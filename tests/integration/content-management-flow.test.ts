...
const testData = {
  categories: [
    { id: 'javascript-core', name: 'JavaScript (Core)', description: 'Core JavaScript concepts' },
    { id: 'react-core', name: 'React.js (Core)', description: 'React fundamentals and advanced patterns' },
    { id: 'css-styling', name: 'CSS & Styling', description: 'CSS fundamentals and advanced techniques' }
  ],
  topics: [
    { id: 'hoisting', name: 'Hoisting', description: 'JavaScript hoisting behavior', category: 'javascript-core', difficulty: 'intermediate' },
    { id: 'closures', name: 'Closures', description: 'JavaScript closures and lexical scoping', category: 'javascript-core', difficulty: 'intermediate' },
    { id: 'react-hooks', name: 'React Hooks', description: 'React hooks and state management', category: 'react-core', difficulty: 'intermediate' },
    { id: 'css-grid', name: 'CSS Grid', description: 'CSS Grid layout system', category: 'css-styling', difficulty: 'intermediate' }
  ],
  learningPaths: [
    { id: 'javascript-deep-dive', name: 'JavaScript Deep Dive', description: 'Master advanced JavaScript', order: 1, category: 'javascript-core' },
    { id: 'react-mastery', name: 'React Mastery', description: 'Advanced React patterns', order: 2, category: 'react-core' },
    { id: 'css-mastery', name: 'CSS Mastery', description: 'Advanced CSS techniques', order: 3, category: 'css-styling' }
  ],
  questions: [
    {
      id: 'q1',
      ...
      options: ['Variables are moved to top', 'Functions are moved to top', 'Both variables and functions', 'None of the above'],
      correctAnswer: 'Both variables and functions',
      explanation: 'Hoisting moves both variable and function declarations to the top of their scope.',
    ...
    },
    {
      ...
      type: 'multiple-choice',
      options: ['Function with access to outer scope', 'Function inside another function', 'Both A and B', 'None of the above'],
      correctAnswer: 'Both A and B',
      explanation: 'Closures are functions that have access to variables in their outer scope.',
      difficulty: 'intermediate',
      category: 'javascript-core',
      topics: ['closures'],
      learningPaths: ['javascript-deep-dive']
  ...
  plans: [
    {
      ...
      sections: [
        { id: 'js-fundamentals', name: 'JavaScript Fundamentals', category: 'javascript-core', questions: ['q1', 'q2'] }
      ]
    }
  ]
};
...
describe('Content Management Flow Integration Tests', () => {
  ...
  describe('2. Topic Management Flow', () => {
    ...
    it('should retrieve topics by category', async () => {
      ...
      const jsTopics = result.data.filter((topic: any) => topic.category === 'javascript-core');
  ...
  describe('3. Learning Path Management Flow', () => {
    ...
    it('should retrieve learning paths with proper ordering', async () => {
      ...
      const jsDeepDive = result.data.find((path: any) => path.id === 'javascript-deep-dive');
  ...
  describe('4. Question Management Flow', () => {
    ...
    it('should retrieve questions by learning path', async () => {
      const response = await fetch(`${baseUrl}/api/questions/unified?learningPath=javascript-deep-dive`);
      expect(response.status).toBe(200);
    ...
    it('should retrieve questions by category', async () => {
      const response = await fetch(`${baseUrl}/api/questions/unified?category=javascript-core`);
      expect(response.status).toBe(200);
    ...
    it('should retrieve questions by topic', async () => {
      const response = await fetch(`${baseUrl}/api/questions/unified?topic=hoisting`);
      expect(response.status).toBe(200);
  ...
  describe('5. Plan Management Flow', () => {
    ...
    it('should add questions to plan sections', async () => {
      ...
      const response = await fetch(`${baseUrl}/api/guided-learning/plans/${planId}/sections/${sectionId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    it('should retrieve plan with questions', async () => {
      const response = await fetch(`${baseUrl}/api/guided-learning/plans/1-day-plan`);
      expect(response.status).toBe(200);
      ...
      const jsSection = Object.values(result.data.sections).find((section: any) => 
        section.name === 'JavaScript Fundamentals'
      );
  ...
  describe('6. Learning Paths Page Integration', () => {
    it('should display questions in learning paths page', async () => {
      ...
      const questionsResponse = await fetch(`${baseUrl}/api/questions/unified?learningPath=${learningPathId}`);
      ...
      const pathResponse = await fetch(`${baseUrl}/api/learning-paths/${learningPathId}`);
    ...
    it('should show topics in learning path detail page', async () => {
      ...
      const response = await fetch(`${baseUrl}/learning-paths/${learningPathId}`);
      expect(response.status).toBe(200);
  ...
  describe('7. End-to-End Content Flow Validation', () => {
    it('should maintain data consistency across all entities', async () => {
      ...
      const jsTopics = topics.data.filter((topic: any) => topic.category === 'javascript-core');
      expect(jsTopics.length).toBeGreaterThan(0);

      // 3. Verify learning paths are linked to categories
      const learningPathsResponse = await fetch(`${baseUrl}/api/learning-paths`);
      ...
      const jsQuestions = questions.data.filter((q: any) => q.category === 'javascript-core');
