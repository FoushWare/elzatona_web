...
describe('AdminNavbar Component - Advanced Tests', () => {
  ...
  test('handles scroll effect correctly', () => {
    ...
    expect(navbar).toHaveClass('bg-gradient-to-r', 'from-red-600', 'to-red-800');
    ...
    expect(navbar).toHaveClass('bg-white/95', 'dark:bg-gray-900/95', 'backdrop-blur-md', 'shadow-lg');
  });
  ...
  test('handles navigation link clicks', async () => {
    ...
    expect(questionsLink.closest('a')).toHaveAttribute('href', '/admin/content/questions');
  });
  ...
  test('handles dropdown menu item descriptions', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Admin overview and statistics')).toBeInTheDocument();
      expect(screen.getByText('Add, edit, and manage questions')).toBeInTheDocument();
      expect(screen.getByText('Create and manage learning plans')).toBeInTheDocument();
    });
  });
