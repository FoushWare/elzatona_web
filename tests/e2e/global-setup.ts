...
async function globalSetup() {
  ...
  if (isTestEnv) {
    ...
    console.log('⚠️  Using DEV environment (.env.local) - consider creating .env.test.local for test isolation');
  }
  
  if (loadedFiles.length > 0) {
    console.log(`   Loaded: ${loadedFiles.map(f => f.split('/').pop()).join(', ')}`);
  }
