...
describe('Auto-linking System E2E Tests', () => {
  ...
  describe('Question Creation and Auto-linking', () => {
    test('should create question via form and verify auto-linking', async () => {
      ...
      await page.waitForSelector('[data-testid="add-question-btn"]', { timeout: 10000 });
      ...
      // Fill question form
      await page.type('[data-testid="question-title"]', 'E2E Test JavaScript Question');
      await page.type('[data-testid="question-content"]', 'What is the output of console.log(typeof null)?');
      ...
      // Add explanation
      await page.type('[data-testid="explanation"]', 'typeof null returns "object" due to a historical bug in JavaScript.');
    ...
    test('should create questions via bulk markdown import', async () => {
      // Navigate to questions page
      await page.goto(`${baseUrl}/admin/content/questions`);
      await page.waitForSelector('[data-testid="bulk-import-btn"]', { timeout: 10000 });
      
      // Click bulk import button
      await page.click('[data-testid="bulk-import-btn"]');
      
      // Wait for bulk import modal
      await page.waitForSelector('[data-testid="bulk-import-modal"]', { timeout: 10000 });
...
**Explanation:** useEffect is used for side effects in React components.
      ...
      // Wait for processing
      await page.waitForSelector('[data-testid="processing-indicator"]', { timeout: 10000 });
      
      // Wait for completion
      await page.waitForSelector('[data-testid="import-complete"]', { timeout: 30000 });
      
      // Verify success message
      const successMessage = await page.textContent('[data-testid="import-success"]');
  ...
  describe('Section Management and Question Display', () => {
    test('should view questions in sections with auto-linking', async () => {
      ...
      await page.waitForSelector('[data-testid="sections-list"]', { timeout: 10000 });
      
      // Find JavaScript section
      const jsSection = await page.$('[data-testid="section-JavaScript-Fundamentals"]');
      ...
      await page.waitForSelector('[data-testid="section-questions"]', { timeout: 10000 });
      ...
      const questionTitle = await page.evaluate(el => el.textContent, firstQuestion);
  ...
  describe('Guided Learning Plan Creation and Management', () => {
    test('should create guided learning plan with auto-linked sections', async () => {
      ...
      await page.waitForSelector('[data-testid="create-plan-btn"]', { timeout: 10000 });
      
      // Click create plan button
      await page.click('[data-testid="create-plan-btn"]');
      
      // Wait for plan creation form
      await page.waitForSelector('[data-testid="plan-form"]', { timeout: 10000 });
      
      // Fill plan details
      await page.type('[data-testid="plan-title"]', 'E2E Test Learning Plan');
      await page.type('[data-testid="plan-description"]', 'Test plan for E2E testing');
      ...
      await page.waitForSelector('[data-testid="available-sections"]', { timeout: 10000 });
      
      // Select sections
      await page.click('[data-testid="section-JavaScript-Fundamentals"]');
      
      // Verify section is selected
      const selectedSections = await page.$$('[data-testid="selected-section"]');
      ...
      // Wait for success message
      await page.waitForSelector('[data-testid="plan-saved"]', { timeout: 10000 });
      
      // Verify plan was created
      const successMessage = await page.textContent('[data-testid="plan-saved"]');
    ...
    test('should edit guided learning plan and manage questions', async () => {
      ...
      await page.waitForSelector('[data-testid="working-on-section"]', { timeout: 10000 });
      
      // Verify "Working on" indicator
      const workingOnText = await page.textContent('[data-testid="working-on-section"]');
      expect(workingOnText).toContain('JavaScript Fundamentals');
      
      // Wait for available questions to load
      await page.waitForSelector('[data-testid="available-questions"]', { timeout: 10000 });
      ...
      await page.waitForSelector('[data-testid="add-questions-modal"]', { timeout: 10000 });
      
      // Select "Add from existing" option
      await page.click('[data-testid="add-from-existing"]');
      
      // Wait for question selection
      await page.waitForSelector('[data-testid="question-selection"]', { timeout: 10000 });
      
      // Select some questions
      const questionCheckboxes = await page.$$('[data-testid="question-checkbox"]');
      ...
      await page.waitForSelector('[data-testid="questions-added"]', { timeout: 10000 });
  ...
  describe('User-facing Guided Learning Experience', () => {
    test('should display guided learning plan to users', async () => {
      ...
      await page.waitForSelector('[data-testid="learning-plans"]', { timeout: 10000 });
      ...
      await page.waitForSelector('[data-testid="plan-details"]', { timeout: 10000 });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should handle empty states', async () => {
      ...
      if (emptyState) {
        const emptyMessage = await page.textContent('[data-testid="empty-sections"]');
  ...
  describe('Performance and Load Testing', () => {
    test('should handle large number of questions efficiently', async () => {
      ...
      await page.waitForSelector('[data-testid="filtered-questions"]', { timeout: 5000 });
