...
test.describe('Content Management E2E Flow', () => {
  ...
  test('Complete Content Management Flow', async () => {
    ...
    await test.step('Create Category', async () => {
      ...
      await page.fill(
        ...
        'Test category for JavaScript core concepts'
      );
      await page.click('button[type="submit"]');

      await expect(
        page.locator('text=Category created successfully')
      ).toBeVisible();
      ...
      const categoryElement = await page
        ...
        .first();
      testData.categoryId =
        (await categoryElement.getAttribute('data-category-id')) || '';
    });
    ...
    await test.step('Create Topic', async () => {
      ...
      await page.fill(
        'textarea[name="description"]',
        'Test topic for JavaScript hoisting'
      );
      ...
      await expect(
        page.locator('text=Topic created successfully')
      ).toBeVisible();

      // Store topic ID
      const topicElement = await page
        .locator('[data-testid="topic-item"]')
        .first();
      testData.topicId =
        (await topicElement.getAttribute('data-topic-id')) || '';
    });
    ...
    await test.step('Create Learning Path', async () => {
      ...
      await page.fill(
        'textarea[name="description"]',
        'Test learning path for advanced JavaScript'
      );
      await page.selectOption('select[name="category"]', testData.categoryId);
      ...
      await expect(
        page.locator('text=Learning path created successfully')
      ).toBeVisible();

      // Store learning path ID
      const pathElement = await page
        .locator('[data-testid="learning-path-item"]')
        .first();
      testData.learningPathId =
        (await pathElement.getAttribute('data-path-id')) || '';
    });
    ...
    await test.step('Create Questions', async () => {
      ...
      await page.fill(
        'input[name="title"]',
        'Test Question: What is hoisting?'
      );
      await page.fill(
      ...
      await page.check(
        `input[name="learningPaths"][value="${testData.learningPathId}"]`
      );
      ...
      // Store question ID
      const questionElement = await page
        .locator('[data-testid="question-item"]')
        .first();
      testData.questionId =
        (await questionElement.getAttribute('data-question-id')) || '';
    });
    ...
    await test.step('Create Learning Plan', async () => {
      ...
      await page.fill(
        'textarea[name="description"]',
        'Test intensive preparation plan'
      );
      ...
      await page.check(
        `input[name="question"][value="${testData.questionId}"]`
      );
    ...
    await test.step('Verify Learning Paths Display', async () => {
      ...
      await expect(
        page.locator('text=Test JavaScript Deep Dive')
      ).toBeVisible();
      ...
      await expect(page).toHaveURL(
        /\/learning-paths\/test-javascript-deep-dive/
      );
    ...
    await test.step('Test Question Practice', async () => {
      ...
      await expect(
        page.locator('text=Test Question: What is hoisting?')
      ).toBeVisible();
      ...
      await expect(
        page.locator(
          'text=Hoisting moves both variable and function declarations'
        )
      ).toBeVisible();
    });
  });
  ...
  test('Learning Paths Page Functionality', async () => {
    ...
    const learningPaths = await page
    ...
    await expect(
      firstPath.locator('[data-testid="path-content"]')
    ).toBeVisible();
  });
