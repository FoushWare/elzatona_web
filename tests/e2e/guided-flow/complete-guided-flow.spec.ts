...
test.describe('G-E2E-004: Complete Guided Flow', () => {
  ...
  test('should complete full flow from homepage to guided learning page', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText(/Choose Your Learning Style/i, { timeout: 10000 });
    ...
    const guidedCard = page.getByText(/I need guidance/i).locator('..').locator('..');
    ...
    await expect(page.getByText(/Loading your learning experience/i)).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.locator('h1')).toContainText(/Choose Your Learning Journey|Learning Journey/i, { timeout: 10000 });
  });
  ...
  test('should show loading transition during navigation', async ({ page }) => {
    ...
    await Promise.race([
      expect(loadingMessage).toBeVisible({ timeout: 10000 }).catch(() => null),
      expect(spinner).toBeVisible({ timeout: 10000 }).catch(() => null),
    ]);
  ...
  test('should display selection feedback when option is clicked', async ({ page }) => {
    // Navigate to get-started
    await page.goto('/get-started', { waitUntil: 'domcontentloaded' });
    ...
    const feedbackVisible = await feedback.first().isVisible({ timeout: 1000 }).catch(() => false);
  ...
  test('should navigate to browse-practice-questions when self-directed is selected', async ({ page }) => {
    // Navigate to get-started
    await page.goto('/get-started', { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1000); // Wait for page initialization
    ...
    const selfDirectedCard = page.getByText(/I'm self-directed/i).locator('..').locator('..');
  ...
  test('should persist user type selection across navigation', async ({ page }) => {
    // Navigate to get-started
    await page.goto('/get-started', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1000); // Wait for page initialization
  ...
  test('should display guided learning plans on guided learning page', async ({ page }) => {
    // Navigate directly to guided learning page
    await page.goto('/features/guided-learning', { waitUntil: 'domcontentloaded' });
    ...
    const plansSection = page.locator('text=/Learning Plan|Day Plan|Plan/i').or(
      page.locator('[class*="plan"]')
    );
    ...
    const hasPlans = await plansSection.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should handle theme toggle in guided flow pages', async ({ page }) => {
    ...
    const themeToggleOnGuidedPage = page.getByRole('button', { name: /Toggle theme/i });
    await expect(themeToggleOnGuidedPage).toBeVisible({ timeout: 10000 });
  });

  test('should navigate back from guided learning to get-started', async ({ page }) => {
    // Navigate to guided learning page
    await page.goto('/features/guided-learning', { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle');
    
    // Look for back button or navigation link
    const backButton = page.getByRole('link', { name: /Back|Go Back/i }).or(
      page.getByRole('button', { name: /Back/i })
    );
    
    const backButtonVisible = await backButton.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should handle rapid clicking on get-started options', async ({ page }) => {
    // Navigate to get-started
    await page.goto('/get-started', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1000); // Wait for page initialization
    ...
    // Increase timeout to account for 1500ms delay plus navigation time
    await page.waitForURL(/\/features\/guided-learning|\/browse-practice-questions/, { timeout: 15000 });
  ...
  test('should display correct content for guided vs self-directed selection', async ({ page }) => {
    // Test guided path
    await page.goto('/get-started', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1000); // Wait for page initialization
