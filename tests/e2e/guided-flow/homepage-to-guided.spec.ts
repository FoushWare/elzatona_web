...
test.describe("G-E2E-001: Homepage Basic Navigation", () => {
  ...
  test("should load homepage without errors", async ({ page }) => {
    ...
    const heading = page
      .getByRole("heading", {
        name: /Start Your Learning Path|Master Frontend Development|Build Your Custom Roadmap/i,
      })
      .first();
    await expect(heading).toBeVisible();
  });

  test("should navigate to /get-started when default CTA is clicked", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    const choosePlanLinks = page.getByRole("link", {
    ...
    const ctaButton =
      choosePlanCount > 0 ? choosePlanLinks.first() : getStartedLinks.first();
  ...
  test('should navigate to /learn when "Explore Learning Paths" is clicked', async ({
    page,
  }) => {
    await page.goto("/");
...
test.describe("G-E2E-002: Learning Style Selection", () => {
  ...
  test("should navigate to guided learning when Guided Learning card is clicked", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    const guidedHeading = page.getByRole("heading", {
      name: /Guided Learning/i,
    });
    ...
    const navigationPromise = page.waitForURL(/\/features\/guided-learning/, {
      timeout: 15000,
    });
    ...
    const userType = await page.evaluate(() => {
      return (
        localStorage.getItem("learning-preferences:type") ||
  ...
  test("should navigate to browse practice questions when Free Style Learning card is clicked", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForTimeout(2000); // Wait for animations and component initialization
    ...
    const freestyleHeading = page.getByRole("heading", {
      name: /Free Style Learning/i,
    });
    ...
    // Click the card and wait for navigation
    // Start navigation wait before clicking to ensure we catch it
    const navigationPromise = page.waitForURL(/\/browse-practice-questions/, {
      timeout: 15000,
    });
...
test.describe("G-E2E-003: Personalized Content Display", () => {
  ...
  test("should show guided content when no userType is set (default for unauthenticated users)", async ({
    page,
  }) => {
    ...
    const heading = page
      .getByRole("heading", { name: /Master Frontend Development/i })
      .first();
    await expect(heading).toBeVisible({ timeout: 15000 });

    // Should show subtitle for guided users
    const subtitle = page
      .getByText(/The complete platform to ace your frontend interviews/i)
      .first();
  ...
  test('should show guided content when userType is "guided" (default for unauthenticated users)', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/Master Frontend Development/i).first(),
    ).toBeVisible();
    await expect(
      page
        .getByText(/The complete platform to ace your frontend interviews/i)
    ...
    await expect(
      page.getByText(/Ready to Ace Your Interviews\? 🚀/i),
    ).not.toBeVisible();
  });

  test('should show self-directed content when userType is "self-directed"', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/Create and manage your personalized learning journey/i),
    ).toBeVisible();
  ...
  test("should show active plan content when guided user has active plan", async ({
    page,
  }) => {
    const activePlan = {
      id: "plan-123",
    ...
    await expect(
      page.getByText(/Continue Frontend Fundamentals/i).first(),
    ).toBeVisible();
    await expect(
      page.getByText(/Pick up where you left off/i).first(),
    ).toBeVisible();

    // Should show plan details
    await expect(page.getByText(/Current Plan:/i)).toBeVisible();
    await expect(
      page.getByText(/Frontend Fundamentals/i).first(),
    ).toBeVisible();
    await expect(page.getByText(/50 questions/i)).toBeVisible();
  });

  test("should handle invalid active plan JSON gracefully", async ({
    page,
  }) => {
    ...
    const continueButtons = page.getByRole("link", {
...
test.describe("G-E2E-004: User Statistics Display", () => {
  ...
  test("should display user statistics component", async ({ page }) => {
    ...
    const statsSection = page
      .locator("section")
      .filter({ hasText: /Active Learners|Success Rate|Questions Solved/i });
...
test.describe("G-E2E-005: Complete User Flow from Homepage", () => {
  ...
  test("should complete full flow: homepage -> get-started -> guided learning", async ({
    page,
  }) => {
  ...
  test("should complete flow: homepage -> learning style selection -> guided learning", async ({
    page,
  }) => {
    // Step 1: Start at homepage
    await page.goto("/");
    ...
    const learningType = await page.evaluate(() => {
      return (
        localStorage.getItem("learning-preferences:type") ||
        localStorage.getItem("learning-type:type")
  ...
  test("should complete flow: homepage -> learning style selection -> freestyle learning", async ({
    page,
  }) => {
    // Step 1: Start at homepage
    await page.goto("/");
...
test.describe("G-E2E-006: Active Plan Integration", () => {
  ...
  test("should display continue practice button when active plan exists", async ({
    page,
  }) => {
    const activePlan = {
      id: "plan-test",
    ...
    const continueButton = page
      .getByRole("link", { name: /Continue Practice/i })
      .first();
  ...
  test("should navigate to guided practice when continue button is clicked", async ({
    page,
  }) => {
    const activePlan = {
      id: "plan-test",
  ...
  test("should persist active plan across page reloads", async ({ page }) => {
    ...
    await page.waitForTimeout(1500);
    await expect(
      page.getByText(/Continue Persistent Plan/i).first(),
    ).toBeVisible();
