...
test.describe('G-E2E-005: Guided Learning Page', () => {
  ...
  test('should display quick stats section', async ({ page }) => {
    ...
    const statsSection = page.getByText(/Questions per Plan|Days Available|Success Rate/i);
  ...
  test('should show sign-in CTA for unauthenticated users', async ({ page }) => {
    ...
    const bannerVisible = await signupBanner.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (bannerVisible) {
      ...
      const ctaButton = page.getByRole('link', { name: /Create free account|Sign up/i });
      ...
      const welcomeVisible = await welcomeMessage.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should display learning plans when loaded', async ({ page }) => {
    ...
    const plansContainer = page.locator('[class*="grid"]').filter({ hasText: /Day|Plan|Questions/i });
    const plansVisible = await plansContainer.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should handle plan selection when plans are available', async ({ page }) => {
    await page.waitForLoadState('networkidle');
    ...
    const planCards = page.locator('text=/Day|Plan|Questions/i').or(
    ...
    const planVisible = await firstPlan.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    if (planVisible) {
      ...
      const emptyState = page.getByText(/No learning plans|No plans available/i);
      
      const hasErrorOrEmpty = await errorState.or(emptyState).first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should display loading state while fetching plans', async ({ page }) => {
    // Navigate to page
    await page.goto('/features/guided-learning', { waitUntil: 'domcontentloaded' });
    
    // Check for loading indicator immediately
    const loadingIndicator = page.getByText(/Loading Learning Plans|Fetching/i).or(
  ...
  test('should handle error state when plans fail to load', async ({ page }) => {
  ...
  test('should display completion statistics for authenticated users', async ({ page }) => {
    await page.waitForLoadState('networkidle');
    ...
    const statsVisible = await completionStats.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should handle theme toggle on guided learning page', async ({ page }) => {
    await page.waitForLoadState('networkidle');
  ...
  test('should navigate back to get-started page', async ({ page }) => {
    ...
    const backVisible = await backButton.first().isVisible({ timeout: 5000 }).catch(() => false);
