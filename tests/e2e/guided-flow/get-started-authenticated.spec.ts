...
test.describe('G-E2E-003: Complete guided flow entry (authenticated user)', () => {
  ...
  test('should navigate to get-started page when authenticated', async ({ page }) => {
    await expect(page).toHaveURL(/\/get-started/);
    await expect(page.locator('h1')).toContainText(/Choose Your Learning Style/i);
  });

  test('should allow immediate selection without sign-in popup', async ({ page }) => {
  ...
  test('should navigate immediately without authentication delay', async ({ page }) => {
    const guidedOption = page.getByText(/I need guidance/i).locator('..').locator('..');
    ...
    const currentUrl = page.url();
    expect(currentUrl.includes('/features/guided-learning') || currentUrl.includes('/guided')).toBeTruthy();
  });
});
