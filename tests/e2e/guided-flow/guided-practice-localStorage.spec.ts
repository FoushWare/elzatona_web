...
test.describe('G-E2E-010: Complete Practice Flow Without Authentication', () => {
  ...
  test('should complete practice flow with localStorage only', async ({ page }) => {
    ...
    await expect(page.locator('h2, h3').filter({ hasText: /question/i }).first()).toBeVisible();
    ...
    const optionButton = page.locator('button').filter({ hasText: /option/i }).first();
    if (await optionButton.isVisible().catch(() => false)) {
      await optionButton.click();
      
      // Wait for explanation to appear
      await expect(page.getByText(/explanation/i)).toBeVisible({ timeout: 5000 });
...
test.describe('G-E2E-011: Browser Refresh Resume', () => {
  ...
  test('should resume from saved position after browser refresh', async ({ page }) => {
    const planId = 'test-plan-id';
    await page.goto(`/guided-practice?plan=${planId}`);
    ...
    const firstQuestionTitle = await page.locator('h2, h3').filter({ hasText: /question/i }).first().textContent();
    ...
    const secondQuestionTitle = await page.locator('h2, h3').filter({ hasText: /question/i }).first().textContent();
    ...
    const resumedQuestionTitle = await page.locator('h2, h3').filter({ hasText: /question/i }).first().textContent();
...
test.describe('G-E2E-017: Learning Resources Display', () => {
  ...
  test('should display resources section after explanation when resources exist', async ({ page }) => {
    const planId = 'test-plan-id';
    await page.goto(`/guided-practice?plan=${planId}`);
    ...
    if (await optionButton.isVisible().catch(() => false)) {
      ...
      const hasResources = await resourcesSection.isVisible().catch(() => false);
  ...
  test('should display resource details correctly', async ({ page }) => {
    ...
    if (await optionButton.isVisible().catch(() => false)) {
      ...
      if (hasResources) {
        ...
        const resourceLinks = page.locator('a').filter({ hasText: /tutorial|guide|article|course/i });
  ...
  test('should NOT display resources section when question has no resources', async ({ page }) => {
    const planId = 'test-plan-id';
    await page.goto(`/guided-practice?plan=${planId}`);
  ...
  test('should NOT display resources section for code questions', async ({ page }) => {
    const planId = 'test-plan-id';
    await page.goto(`/guided-practice?plan=${planId}`);
  ...
  test('should open resource links in new tab', async ({ page, context }) => {
    ...
    if (await optionButton.isVisible().catch(() => false)) {
      ...
      if (hasResources) {
        ...
        const resourceLink = page.locator('a').filter({ hasText: /tutorial|guide|article|course/i }).first();
