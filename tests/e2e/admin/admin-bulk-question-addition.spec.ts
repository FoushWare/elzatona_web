...
test.describe('A-E2E-001: Admin Bulk Question Addition', () => {
  ...
  async function bulkDeleteQuestions(page: any, titles: string[]): Promise<void> {
    ...
    if (await selectAllCheckbox.count() > 0) {
      ...
      if (selectedCount > 0) {
        ...
        if (await deleteSelectedButton.count() > 0) {
          ...
          await expect(dialog.getByRole('heading', { name: /Delete Selected Questions/i })).toBeVisible({ timeout: 5000 });
  ...
  test.beforeEach(async ({ page, browserName }) => {
    ...
    const isEdgeForSubmit = process.env.PLAYWRIGHT_PROJECT === 'msedge' || 
                            (await page.evaluate(() => navigator.userAgent)).includes('Edg/');
    ...
    if (apiResponse) {
      console.log('✅ API response already received, skipping button state check (page may be navigating)');
      ...
      try {
        ...
        const buttonCheckPromise = page.waitForFunction(
          () => {
            ...
            const isSigningIn = text.includes('Signing In') || text.includes('Signing');
        ...
        if (apiResponse) {
          console.log('✅ API response received during button check, skipping button state');
        ...
        if (apiResponse) {
          console.log('✅ API response received, button state check not needed');
          submissionDetected = true;
        } else if (!page.isClosed()) {
          try {
            const currentUrl = page.url();
            if (!currentUrl.includes('/admin/login')) {
              console.log('✅ Page navigated away from login - login succeeded');
              submissionDetected = true;
            } else {
              console.log('⚠️ Button submitting state not detected, but continuing...');
            }
          } catch (urlError) {
            // Page might be closed/navigating
            if (apiResponse) {
              console.log('✅ API response received, page navigation in progress');
              submissionDetected = true;
            }
          }
        } else if (apiResponse) {
          console.log('✅ Page closed but API response received - navigation in progress');
          submissionDetected = true;
        }
      }
    }
    ...
    if (!apiResponse && !page.isClosed()) {
      try {
        ...
        if (page.isClosed()) {
          ...
          if (apiResponse) {
            console.log('✅ Page closed during wait, but API response already received - navigation in progress');
            submissionDetected = true;
          } else {
            throw new Error('Page was closed during login process before API response');
          }
        }
      }
    }
      ...
      while (apiRetryCount < maxApiRetries && !page.isClosed()) {
        ...
        try {
          ...
          if (page.isClosed()) {
            // If page closed but we have API response, that's OK
            if (apiResponse) {
              console.log('✅ Page closed during wait but API response received');
              break;
            }
            throw new Error('Page was closed while waiting for API request');
          }
        ...
        apiRetryCount++;
        console.log(`🔄 Checking for API requests (attempt ${apiRetryCount}/${maxApiRetries})...`);
          ...
          try {
            ...
            if (currentUrl.includes('/admin/login')) {
              ...
              if (buttonText?.includes('Signing In')) {
                console.log('✅ Button shows "Signing In" - form is processing');
                break; // Form is processing, wait for response
              }
            } else {
              // Page navigated away - API likely succeeded
              console.log('✅ Page navigated away from login - API likely succeeded');
              break;
            }
          } catch (e) {
            // Button might not be accessible (page navigating), continue
            console.log('⚠️ Could not check button state (page may be navigating)');
          }
        } else if (apiResponse) {
    ...
    if (allRequests.length > 0) {
      ...
      if (!page.isClosed()) {
        const retryButton = page.getByRole('button', { name: /Sign In|Login|Log In/i });
        const buttonCount = await retryButton.count();
        const isDisabled = buttonCount > 0 ? await retryButton.isDisabled() : true;
        
        if (buttonCount > 0 && !isDisabled) {
          console.log('🔄 Retrying form submission (no API request detected)...');
    ...
    if (page.isClosed() && response) {
      console.log('⚠️ Page closed but we have API response - likely navigating, continuing...');
    ...
    if (!response) {
      ...
      console.log('📡 API requests made:', allRequests.length > 0 ? allRequests.slice(0, 10) : 'None');
      ...
      if (!page.isClosed()) {
        ...
        console.log('📋 Form details - action:', formAction, 'method:', formMethod);
        ...
        console.log('📋 Inputs still filled - Email:', !!emailStillFilled, 'Password:', !!passwordStillFilled);
        ...
        console.log('🔘 Button state - Disabled:', buttonDisabled, 'Text:', buttonText);
      }
      ...
      if (!page.isClosed()) {
        ...
        if (errorBoxCount > 0) {
          ...
          throw new Error(`Login failed - no API response but error message appeared: ${errorText}. Check ADMIN_EMAIL and ADMIN_PASSWORD in .env.test.local`);
        }
      }
    ...
    try {
      ...
      if (!responseData.success) {
        ...
        throw new Error(`Login API failed: ${errorMsg}. Check ADMIN_EMAIL and ADMIN_PASSWORD in .env.test.local (or .env.test/.env.local as fallback)`);
      }
      ...
      if (errorBoxCount > 0) {
        const errorText = await errorBox.first().textContent();
        throw new Error(`Login failed: ${errorText}. Check ADMIN_EMAIL and ADMIN_PASSWORD in .env.test.local (or .env.test/.env.local as fallback)`);
      }
    ...
    try {
      ...
      if (currentURL && !currentURL.includes('/admin/dashboard') && !currentURL.includes('/admin/content')) {
        ...
        if (!page.isClosed()) {
          ...
          if (errorCount > 0) {
            const errorText = await errorBox.first().textContent();
            throw new Error(`Login failed - error message appeared: ${errorText}. Check ADMIN_EMAIL and ADMIN_PASSWORD in .env.test.local`);
          }
        }
    ...
    if (!currentURLAfterLogin && page.isClosed()) {
      throw new Error('Page was closed after successful login and could not recover. This may indicate a crash or unexpected redirect.');
    }
    ...
    if (currentURLAfterLogin && (
      ...
      try {
        ...
        if (recoveryURL.startsWith('chrome-error://') || recoveryURL.includes('error')) {
          throw new Error(
        ...
        throw new Error(`Page crashed to error page but recovered. Retrying test...`);
    ...
    if (currentURLAfterLogin.includes('/admin/content/questions')) {
      ...
      if (!currentURLAfterLogin.startsWith('chrome-error://') && 
          !currentURLAfterLogin.startsWith('chrome://') &&
          !currentURLAfterLogin.includes('error')) {
        await expect(page).toHaveURL(/.*admin\/(dashboard|content).*/, { timeout: 15000 });
      } else {
        throw new Error(`Page is on error page: ${currentURLAfterLogin}. Cannot verify redirect.`);
      }
      ...
      if (currentURLAfterLogin.includes('/admin/dashboard')) {
        ...
        await dashboardHeading.waitFor({ state: 'visible', timeout: 10000 }).catch(() => {
        ...
        try {
          ...
          if (!page.isClosed()) {
            ...
            if (currentURL.includes('/admin/content/questions')) {
              ...
              await page.goto('/admin/content/questions', { waitUntil: 'domcontentloaded', timeout: 15000 }).catch(() => {
                throw new Error(`Failed to navigate to questions page: ${navError.message}`);
              });
            }
          } else {
    ...
    console.log('Current URL after navigation:', currentURL || '(could not determine)');
    
    if (currentURL && !currentURL.includes('/admin/content/questions')) {
      throw new Error(`Failed to navigate to questions page. Current URL: ${currentURL}`);
    }
  ...
  test('SETUP: Ensure enough questions for pagination tests', async ({ page }) => {
    await page.waitForTimeout(2000);
    ...
    if (await totalQuestionsLabel.count() > 0) {
      ...
      const countElement = cardContent.locator('p.text-2xl, p.text-3xl').first();
    ...
    if (currentCount === 0) {
      try {
        const response = await page.request.get('/api/questions/unified?page=1&pageSize=1');
        ...
        const questionRows = page.locator('div').filter({ has: page.locator('input[type="checkbox"]') });
    ...
    if (neededQuestions > 0) {
      console.log(`📝 Creating ${neededQuestions} additional questions for pagination tests (current: ${currentCount})...`);
      ...
      console.log(`✅ Created ${neededQuestions} questions for pagination tests`);
    } else {
      console.log(`✅ Already have ${currentCount} questions, enough for pagination tests`);
    }
  });
  ...
  test('should navigate to next page', async ({ page }) => {
    ...
    const nextButton = page.getByRole('button').filter({ hasText: /→|Next|chevron-right/i });
    ...
    if (hasPagination && count > 0 && !isDisabled) {
      ...
      const pageIndicator = page.locator('text=/Page \d+ of \d+|Showing \d+ to \d+ of \d+/i');
      ...
      console.log('⚠️ Pagination not available - skipping test. Need more than pageSize questions for pagination.');
      test.skip();
    }
  });
  ...
  test('should navigate to previous page', async ({ page }) => {
    ...
    if (hasPagination && nextCount > 0 && !nextDisabled) {
      ...
      const prevButton = page.getByRole('button').filter({ hasText: /←|Previous|chevron-left/i });
      const prevCount = await prevButton.count();
      const prevDisabled = prevCount > 0 ? await prevButton.first().isDisabled() : true;

      if (prevCount > 0 && !prevDisabled) {
        // Set up API response listener for previous page
        const prevPageResponsePromise = page.waitForResponse(
          response => response.url().includes('/api/questions/unified') && response.request().method() === 'GET',
          { timeout: 20000 }
        ).catch(() => null);
        ...
        console.log('⚠️ Previous button not found or disabled - skipping previous page test');
      }
    } else {
  ...
  test('should change page size', async ({ page }) => {
    ...
    if (!hasShowLabel) {
      console.log('⚠️ "Show:" label not found - page size selector may not be visible (pagination might not be available)');
      test.skip();
      return;
    }
    ...
    try {
      const questionsAPIResponse = await page.waitForResponse(
        response => response.url().includes('/api/questions/unified') && response.request().method() === 'GET',
        { timeout: 5000 }
      ).catch(() => null);
      ...
      if (questionsAPIResponse) {
        ...
        if (totalQuestions <= pageSize) {
          console.log(`⚠️ Only ${totalQuestions} questions available (pageSize: ${pageSize}) - pagination not available, skipping test`);
          test.skip();
          return;
        }
      }
    ...
    try {
      ...
      const comboboxInContainer = showContainer.locator('button[role="combobox"]');
    ...
    if (!trigger || (await trigger.count().catch(() => 0)) === 0) {
      try {
        ...
        const allComboboxes = page.locator('button[role="combobox"]:not([data-disabled]):not([aria-disabled="true"])');
            ...
            if (isVisible) {
              ...
              if (cbBox && showBox) {
                ...
                if (verticalDistance < 50) {
                  trigger = cb;
                  console.log('✅ Found page size selector using proximity method');
                  break;
                }
              }
            }
    ...
    try {
      ...
      if (!isEnabled) {
        console.log('⚠️ Page size selector is disabled, waiting for it to be enabled...');
    ...
    try {
      const showingTextBefore = page.locator('text=/Showing \\d+ to \\d+ of \\d+/i');
      beforeText = await showingTextBefore.first().textContent({ timeout: 5000 }).catch(() => null);
    } catch (e: any) {
    ...
    try {
      ...
      for (let retry = 0; retry < maxClickRetries; retry++) {
        try {
          ...
          if (errorMessage.includes('Target page, context or browser has been closed') ||
            ...
            throw new Error(`Page was closed during click attempt ${retry + 1}: ${errorMessage}`);
          }
          
          if (retry < maxClickRetries - 1) {
            console.log(`⚠️ Click attempt ${retry + 1} failed, retrying... (${errorMessage})`);
      ...
      if (!clickSucceeded) {
        throw new Error('Failed to click page size selector after all retry attempts');
      }
    ...
    const responseHandler = (response: any) => {
      const url = response.url();
      if (url.includes('/api/questions/unified') && 
    ...
    const option20 = page.locator('[role="option"]')
      .filter({ hasText: '20' })
      .filter({ hasNot: page.locator('[data-disabled], [aria-disabled="true"]') })
      .first();
    ...
    try {
      ...
      const pageSizeResponsePromise = page.waitForResponse(
        response => {
          const url = response.url();
          return url.includes('/api/questions/unified') && 
                 response.request().method() === 'GET' && 
                 (url.includes('pageSize=20') || url.includes('pageSize=5'));
        },
    ...
    // Don't check page.isClosed() - let Playwright handle actual closure errors
    const showingTextAfter = page.locator('text=/Showing \\d+ to \\d+ of \\d+/i');
    const afterText = await showingTextAfter.first().textContent({ timeout: 5000 }).catch(() => null);
  ...
  test('should update stats after creating question', async ({ page }) => {
    ...
    const initialCountText = await totalQuestions.first().locator('..').textContent();
    ...
    await newTotalQuestions.first().waitFor({ state: 'visible', timeout: 10000 });
    
    // Get new count
    const newCountText = await newTotalQuestions.first().locator('..').textContent();
  ...
  test('should open bulk upload modal when Bulk Upload button is clicked', async ({ page }) => {
    await page.waitForTimeout(2000);
    ...
    await expect(dialog.getByRole('heading', { name: /Bulk Upload Questions/i })).toBeVisible({ timeout: 5000 });
    await expect(dialog.getByText(/Upload multiple questions at once/i)).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should accept JSON file for bulk upload', async ({ page }) => {
    ...
    const dropZone = dialog.locator('text=/Drag and drop.*CSV or JSON.*click to select/i');
    ...
    const dropZoneClickable = dialog.locator('div').filter({ hasText: /Drag and drop.*click to select/i });
  ...
  test('should handle bulk upload cancellation', async ({ page }) => {
    ...
    if (await cancelButton.count() > 0) {
      ...
      const modalHeading = page.getByRole('heading', { name: /Bulk Upload Questions/i });
  ...
  test('should show "Delete Selected" button when questions are selected', async ({ page }) => {
    await page.waitForTimeout(2000);
    ...
    if (checkboxCount > 0) {
      ...
      const deleteSelectedAfter = page.getByRole('button', { name: /Delete Selected/i });
  ...
  test('should delete multiple selected questions', async ({ page }) => {
    ...
    if (checkboxCount >= 2) {
      ...
      const firstQuestionTitle = await firstCheckbox.evaluate((checkbox) => {
        // Find the parent row (div with padding classes)
      ...
      const secondQuestionTitle = await secondCheckbox.evaluate((checkbox) => {
        // Find the parent row (div with padding classes)
        let parent = checkbox.parentElement;
        while (parent && !parent.className.includes('p-4') && !parent.className.includes('p-5')) {
          parent = parent.parentElement;
      ...
      if (firstQuestionTitle && secondQuestionTitle) {
        ...
        if (firstQuestionTitle) {
          const deletedQuestionHeading = page.getByRole('heading', { name: firstQuestionTitle.trim() });
          const isVisible = await deletedQuestionHeading.isVisible().catch(() => false);
          expect(isVisible).toBe(false);
        }
        if (secondQuestionTitle) {
          const deletedQuestionHeading2 = page.getByRole('heading', { name: secondQuestionTitle.trim() });
          const isVisible2 = await deletedQuestionHeading2.isVisible().catch(() => false);
  ...
  test('should cancel bulk deletion', async ({ page }) => {
    ...
    if (checkboxCount > 0) {
      ...
      const questionCards = page.locator('[class*="question"], [class*="card"]').filter({ hasText: /Question|HTML|CSS|JavaScript/i });
  ...
  test('CLEANUP: Bulk delete all test questions', async ({ page }) => {
    ...
    console.log(`🧹 Cleaning up ${createdQuestionTitles.length} test questions...`);
