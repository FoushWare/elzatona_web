...
test.describe('A-E2E-001: Admin Bulk Question Addition - Bulk Operations', () => {
  ...
  test('should show checkboxes for question selection', async ({ page }) => {
    ...
    test.skip(count === 0, 'No checkboxes found - questions may not be loaded or page may be empty');
  ...
  test('should show "Delete Selected" button when questions are selected', async ({ page }) => {
    ...
    test.skip(checkboxCount === 0, 'No question checkboxes found - cannot test delete selected button');
  ...
  test('should open bulk delete confirmation modal', async ({ page }) => {
    ...
    test.skip(checkboxCount === 0, 'No question checkboxes found - cannot test bulk delete modal');
  ...
  test('should delete multiple selected questions', async ({ page }) => {
    ...
    test.skip(checkboxCount < 2, `Only ${checkboxCount} question checkbox(es) found - need at least 2 for bulk deletion test`);
  ...
  test('should cancel bulk deletion', async ({ page }) => {
    ...
    test.skip(checkboxCount === 0, 'No question checkboxes found - cannot test cancel bulk deletion');
    ...
    await firstHeading.waitFor({ state: 'visible', timeout: 10000 }).catch(() => {});
    ...
    const questionTitleFromCheckbox = await checkboxes.first().evaluate((checkbox) => {
      ...
      while (parent && depth < maxDepth) {
        ...
        if (typeof className === 'string' && (className.includes('p-4') || className.includes('p-5'))) {
          const heading = parent.querySelector('h4');
          if (heading) {
            return heading.textContent?.trim() || null;
          }
    ...
    if (questionTitleFromCheckbox) {
      ...
      const checkboxIndex = await checkboxes.first().evaluate((checkbox) => {
      ...
      const allHeadings = await page.locator('main h4, [role="main"] h4, body > div:not([role="dialog"]) h4').all();
      if (allHeadings.length > checkboxIndex && checkboxIndex >= 0) {
        finalQuestionTitle = await allHeadings[checkboxIndex].textContent().catch(() => null);
      }
    }
    ...
    test.skip(!finalQuestionTitle, 'Could not find question title - cannot verify question still exists after cancel');
    ...
    if (finalQuestionTitle) {
      const questionHeading = page.getByRole('heading', { name: finalQuestionTitle.trim() });
  ...
  test('should handle "Select All" functionality', async ({ page }) => {
    ...
    test.skip(checkboxCount === 0, 'No checkboxes found - cannot test Select All functionality');
