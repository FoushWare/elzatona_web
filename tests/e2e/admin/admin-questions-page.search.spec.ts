...
test.describe('A-E2E-001: Admin Bulk Question Addition - Search', () => {
  ...
  test('should show "No questions found" for non-existent search term', async ({ page }) => {
    ...
    try {
      await page.waitForFunction(
        () => {
          ...
          const questionListContainer = document.querySelector('[class*="divide-y"]');
          const questionRows = questionListContainer 
            ? questionListContainer.querySelectorAll('div[class*="question"], h4, [data-testid*="question"]')
      ...
      console.log('⚠️ waitForFunction timed out, checking for heading directly...');
    }
    ...
    try {
      noResultsHeading = page.getByRole('heading', { name: /No questions found/i });
