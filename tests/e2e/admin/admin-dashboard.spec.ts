...
test.describe("A-E2E-003: Admin Dashboard", () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (
      !filledEmail ||
      !filledPassword ||
      filledEmail !== adminEmail ||
      filledPassword !== adminPassword
    ) {
      throw new Error(
    ...
    try {
      ...
      if (await errorMessage.isVisible().catch(() => false)) {
        const errorText = await errorMessage.textContent();
        throw new Error(
          `Login succeeded but navigation failed. Error: ${errorText}`,
        );
      }
  ...
  test("should display dashboard stats", async ({ page }) => {
    ...
    await expect(statsGrid.getByText(/Learning Plans/i)).toBeVisible({
      timeout: 5000,
    });
    await expect(statsGrid.getByText(/^Topics$/i)).toBeVisible({
      timeout: 5000,
    }); // Exact match for Topics card
    await expect(statsGrid.getByText(/^Categories$/i)).toBeVisible({
      timeout: 5000,
    }); // Exact match for Categories card
    await expect(statsGrid.getByText(/Total Tasks/i)).toBeVisible({
      timeout: 5000,
    });

    // Verify Topics card shows a number (even if 0)
    const topicsCard = statsGrid
      .getByText(/^Topics$/i)
      .locator("..")
      .locator("..");
    const topicsValue = await topicsCard
      .locator("text=/\\d+/")
      .first()
      .textContent();
    ...
    const categoriesCard = statsGrid
      .getByText(/^Categories$/i)
      .locator("..")
      .locator("..");
    const categoriesValue = await categoriesCard
      .locator("text=/\\d+/")
      .first()
      .textContent();
  ...
  test("should switch between light and dark theme", async ({ page }) => {
    ...
    const finalHasDarkClass = await htmlElement.evaluate((el) =>
      el.classList.contains("dark"),
    );
  ...
  test("should persist theme preference after page reload", async ({
    ...
    const themeToggleButtonAfterReload = page.getByRole("button", {
      name: /Toggle theme/i,
    });
