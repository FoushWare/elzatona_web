...
import { setupAdminPage, createQuestion, bulkDeleteQuestions, createdQuestionTitles, testPrefix } from './admin-questions-page.setup';
...
test.describe('A-E2E-001: Admin Bulk Question Addition - Pagination', () => {
  ...
  test('should navigate to next page', async ({ page }) => {
    ...
    let showingText = page.locator('text=/Showing \\d+ to \\d+ of \\d+ questions/i');
    ...
    try {
      ...
      try {
        const response = await page.request.get('/api/questions/unified?page=1&pageSize=10');
        const data = await response.json();
        const totalCount = data?.pagination?.totalCount || 0;
        const pageSize = data?.pagination?.pageSize || 10;
        const totalPages = Math.ceil(totalCount / pageSize);
        console.log(`📊 API Response: totalCount=${totalCount}, pageSize=${pageSize}, totalPages=${totalPages}`);
        ...
        showingText = page.locator('text=/Showing \\d+ to \\d+ of \\d+ questions/i');
      } catch (apiError) {
    ...
    const pageText = await pageIndicator.first().textContent().catch(() => null);
    const hasMultiplePages = pageText ? /Page \d+ of (\d+)/.test(pageText) && parseInt(/Page \d+ of (\d+)/.exec(pageText)?.[1] || '1') > 1 : false;
    
    if (!hasPagination && !hasMultiplePages) {
      console.log('⚠️ Pagination not available - skipping test. Need more than pageSize questions for pagination.');
      console.log(`   hasPagination: ${hasPagination}, hasMultiplePages: ${hasMultiplePages}, pageText: ${pageText}`);
      test.skip();
      return;
    }
    ...
    if (await pageIndicator.count() > 0) {
      ...
      const flexContainer = pageIndicatorSpan.locator('xpath=ancestor::div[contains(@class, "flex") and contains(@class, "items-center")]').first();
      ...
      if (buttonCount >= 2) {
        ...
        const buttonBefore = pageIndicatorSpan.locator('xpath=preceding::button[1]').first();
        const buttonAfter = pageIndicatorSpan.locator('xpath=following::button[1]').first();
    ...
    if (!nextButton || (nextButton && await nextButton.count() === 0)) {
      const pageIndicatorText = page.locator('text=/Page \\d+ of \\d+/i').first();
      // Look for button that comes after "Page X of Y" in the DOM
      const followingButton = pageIndicatorText.locator('xpath=following::button[1]').first();
    ...
    if (count > 0 && !isDisabled) {
      ...
      const currentPageText = pageText || await pageIndicator.first().textContent();
      ...
      console.log('⚠️ Pagination not available - skipping test. Need more than pageSize questions for pagination.');
      console.log(`   hasPagination: ${hasPagination}, hasMultiplePages: ${hasMultiplePages}, buttonCount: ${count}, isDisabled: ${isDisabled}`);
      if (pageText) {
  ...
  test('should navigate to previous page', async ({ page }) => {
    ...
    if (!nextButton || (nextButton && await nextButton.count() === 0)) {
      const pageIndicatorText = page.locator('text=/Page \\d+ of \\d+/i').first();
      const followingButton = pageIndicatorText.locator('xpath=following::button[1]').first();
      const precedingButton = pageIndicatorText.locator('xpath=preceding::button[1]').first();
      if (await followingButton.count() > 0) {
        nextButton = followingButton;
      }
      if (await precedingButton.count() > 0) {
        prevButton = precedingButton;
      }
    }
    ...
    if (nextCount > 0 && !nextDisabled) {
      ...
      const beforePageText = pageText || await pageIndicator.first().textContent();
      ...
      while (retryCount < maxRetries && !afterNextPageText) {
        try {
          ...
          const pageIndicatorLocator = page.locator('text=/Page \\d+ of \\d+/i').first();
          await pageIndicatorLocator.waitFor({ state: 'visible', timeout: 5000 });
          
          // Use evaluate to get text directly from DOM (more reliable)
          afterNextPageText = await page.evaluate(() => {
            const spans = Array.from(document.querySelectorAll('span'));
            const pageSpan = spans.find(span => /Page \d+ of \d+/.test(span.textContent || ''));
          ...
          if (retryCount < maxRetries) {
            ...
            try {
              const showingText = page.locator('text=/Showing \\d+ to \\d+ of \\d+ questions/i').first();
      ...
      if (!afterNextPageText) {
        ...
        try {
          ...
          throw new Error('Could not find page indicator after navigation. Pagination may not be working correctly.');
        }
      }

      const afterNextPageMatch = afterNextPageText?.match(/Page (\d+) of/);
      const afterNextPage = afterNextPageMatch ? parseInt(afterNextPageMatch[1]) : beforePage;
      ...
      const pageIndicatorText = page.locator('text=/Page \\d+ of \\d+/i').first();
      await pageIndicatorText.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
      ...
      if (await pageIndicatorText.count() > 0) {
        ...
        if (await precedingButton.count() > 0) {
          ...
          if (!isDisabled) {
            ...
            console.log('✅ Found previous button using XPath (before page text)');
          } else {
        ...
        if (!updatedPrevButton || await updatedPrevButton.count() === 0) {
          const flexContainer = pageIndicatorText.locator('xpath=ancestor::div[contains(@class, "flex") and contains(@class, "items-center")]').first();
          if (await flexContainer.count() > 0) {
            const paginationButtons = flexContainer.locator('button');
            const buttonCount = await paginationButtons.count();
            console.log(`🔘 Found ${buttonCount} buttons in pagination container`);
            ...
            if (buttonCount > 0) {
              ...
              if (!isDisabled) {
                ...
                console.log('✅ Using first button as previous button (fallback)');
              }
            }
          }
        }
      ...
      const prevDisabled = prevCount > 0 ? await updatedPrevButton.first().isDisabled() : true;
      ...
      if (prevCount > 0 && !prevDisabled) {
        ...
        console.log(`📄 Current page before clicking previous: ${currentPageBeforePrev}`);
        
        if (currentPageBeforePrev !== 2) {
          console.log(`⚠️ Expected to be on page 2 before clicking previous, but was on page ${currentPageBeforePrev}`);
        }
        ...
        const prevPageResponsePromise = page.waitForResponse(
          response => {
            ...
            const isUnifiedAPI = url.includes('/api/questions/unified') && response.request().method() === 'GET';
            if (isUnifiedAPI) {
        ...
        if (isActuallyDisabled) {
          ...
          throw new Error('Previous button is disabled on page 2, which should not happen');
        }
        
        console.log('🖱️ Clicking previous button...');
        
        // Ensure button is visible and ready before clicking
        await updatedPrevButton.first().waitFor({ state: 'visible', timeout: 5000 });
        ...
        try {
          await updatedPrevButton.first().click({ timeout: 5000 }).catch(() => {
            console.log('⚠️ Playwright click failed, but JavaScript click should have worked');
          });
        } catch (e) {
        ...
        if (prevResponse) {
          ...
          if (responseUrl.includes('page=1') || !responseUrl.includes('page=')) {
            console.log('✅ API called with correct page number (page 1)');
          } else {
            console.log(`⚠️ API called with unexpected page number in URL: ${responseUrl}`);
          }
        } else {
          console.log('⚠️ No previous page API response received within timeout');
        ...
        while (prevRetryCount < maxPrevRetries && !afterPrevPageText) {
          try {
            const finalPageIndicator = page.locator('text=/Page \\d+ of \\d+/i').first();
            await finalPageIndicator.waitFor({ state: 'visible', timeout: 5000 });
        ...
        if (!afterPrevPageText) {
          ...
          try {
            ...
            throw new Error('Could not find page indicator after clicking previous. Pagination may not be working correctly.');
          }
        }
        
        const afterPrevPageMatch = afterPrevPageText?.match(/Page (\d+) of/);
        const afterPrevPage = afterPrevPageMatch ? parseInt(afterPrevPageMatch[1]) : afterNextPage;
        
        console.log(`📄 Page after clicking previous: ${afterPrevPage}, expected: ${beforePage}`);
        ...
        if (afterPrevPage === 2 && beforePage === 1) {
          ...
          console.log('⚠️ Still on page 2 after clicking previous, trying to verify button state...');
          ...
          const finalPage = finalCheckMatch ? parseInt(finalCheckMatch[1]) : afterPrevPage;
          
          if (finalPage === 2) {
            throw new Error(`Previous button click did not navigate back to page 1. Still on page ${finalPage}. The previous button may not be working correctly.`);
          }
        ...
        console.log(`   prevButtonCount: ${prevCount}, prevDisabled: ${prevDisabled}`);
      }
    } else {
      // Skip if pagination not available
      console.log('⚠️ Pagination not available - skipping test. Need more than pageSize questions for pagination.');
      console.log(`   hasPagination: ${hasPagination}, hasMultiplePages: ${hasMultiplePages}, nextButtonCount: ${nextCount}, nextDisabled: ${nextDisabled}`);
      test.skip();
    }
  });
  ...
  test('should change page size', async ({ page }) => {
    ...
    try {
      const showingTextBefore = page.locator('text=/Showing \\d+ to \\d+ of \\d+ questions/i');
    ...
    // The actual text format is "Showing X to Y of Z questions" (includes "questions" at the end)
    const showingTextAfter = page.locator('text=/Showing \\d+ to \\d+ of \\d+ questions/i');
    ...
    while (textRetryCount < maxTextRetries && !afterText) {
      try {
        await showingTextAfter.first().waitFor({ state: 'visible', timeout: 5000 });
        afterText = await showingTextAfter.first().textContent({ timeout: 5000 });
        if (afterText) break;
      } catch (e) {
        textRetryCount++;
    ...
    if (!afterText) {
      try {
        afterText = await page.evaluate(() => {
          ...
          const showingDiv = divs.find(div => /Showing \d+ to \d+ of \d+ questions/i.test(div.textContent || ''));
    ...
    if (beforeText && afterText) {
      ...
      await showingTextAfter.first().waitFor({ state: 'visible', timeout: 10000 }).catch(() => {
        // If not visible, that's okay - the page size change might have worked anyway
        console.log('⚠️ "Showing X to Y of Z questions" text not found after page size change, but continuing...');
      });
    }
  });
  ...
  test('CLEANUP: Delete pagination test questions', async ({ page }) => {
    ...
    console.log(`🧹 Cleaning up ${createdQuestionTitles.length} pagination test questions...`);
