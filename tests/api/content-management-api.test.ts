...
describe('Content Management API Tests', () => {
  ...
  describe('Categories API', () => {
    ...
    it('GET /api/categories/[id] - should retrieve specific category', async () => {
      const response = await fetch(`${BASE_URL}/api/categories/${testData.categoryId}`);
  ...
  describe('Topics API', () => {
    ...
    it('GET /api/topics/[id] - should retrieve specific topic', async () => {
      const response = await fetch(`${BASE_URL}/api/topics/${testData.topicId}`);
  ...
  describe('Learning Paths API', () => {
    ...
    it('GET /api/learning-paths/[id] - should retrieve specific learning path', async () => {
      const response = await fetch(`${BASE_URL}/api/learning-paths/${testData.learningPathId}`);
  ...
  describe('Questions API', () => {
    it('POST /api/questions/unified - should create a question', async () => {
      const questionData = {
        ...
        options: ['Variables are moved to top', 'Functions are moved to top', 'Both', 'None'],
        correctAnswer: 'Both',
        explanation: 'Hoisting moves both variable and function declarations to the top.',
        difficulty: 'intermediate',
        category: testData.categoryId,
    ...
    it('GET /api/questions/unified - should retrieve questions by category', async () => {
      const response = await fetch(`${BASE_URL}/api/questions/unified?category=${testData.categoryId}`);
    ...
    it('GET /api/questions/unified - should retrieve questions by learning path', async () => {
      const response = await fetch(`${BASE_URL}/api/questions/unified?learningPath=${testData.learningPathId}`);
    ...
    it('GET /api/questions/unified/[id] - should retrieve specific question', async () => {
      const response = await fetch(`${BASE_URL}/api/questions/unified/${testData.questionId}`);
  ...
  describe('Learning Plans API', () => {
    ...
    it('GET /api/guided-learning/plans/[id] - should retrieve specific plan', async () => {
      const response = await fetch(`${BASE_URL}/api/guided-learning/plans/${testData.planId}`);
    ...
    it('PUT /api/guided-learning/plans/[id]/sections/[sectionId] - should update plan section with questions', async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/guided-learning/plans/${testData.planId}/sections/js-fundamentals`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  describe('Content Linking Validation', () => {
    it('should maintain data consistency across all entities', async () => {
      ...
      const categoryResponse = await fetch(`${BASE_URL}/api/categories/${testData.categoryId}`);
      expect(categoryResponse.status).toBe(200);

      // Verify topic is linked to category
      const topicResponse = await fetch(`${BASE_URL}/api/topics/${testData.topicId}`);
      const topic = await topicResponse.json();
      expect(topic.data.category).toBe(testData.categoryId);

      // Verify learning path is linked to category
      const learningPathResponse = await fetch(`${BASE_URL}/api/learning-paths/${testData.learningPathId}`);
      ...
      const questionResponse = await fetch(`${BASE_URL}/api/questions/unified/${testData.questionId}`);
      ...
      const planResponse = await fetch(`${BASE_URL}/api/guided-learning/plans/${testData.planId}`);
      const plan = await planResponse.json();
      const jsSection = Object.values(plan.data.sections).find((section: any) => 
