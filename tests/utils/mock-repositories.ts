...
import { ICategoryRepository, Category, CategoryInput } from '../../libs/database/src/repositories/interfaces/ICategoryRepository';
...
function createCategoryRepositoryMock(): ICategoryRepository {
  ...
  return {
    ...
    async createCategory(data: CategoryInput) {
      const newCat: Category = { id: (categories.length + 1).toString(), ...data };
...
export function createMockRepositories() {
  function createTopicRepositoryMock() {
    const topics = [
      { id: '1', name: 'Algebra', categoryId: '1', description: 'Algebra desc' },
      { id: '2', name: 'Physics', categoryId: '2', description: 'Physics desc' },
    ];
    return {
  ...
  function createSectionRepositoryMock() {
    const sections = [
      { id: '1', name: 'Section 1', topicId: '1', description: 'Section 1 desc' },
      { id: '2', name: 'Section 2', topicId: '2', description: 'Section 2 desc' },
    ];
    return {
  ...
  function createFlashcardRepositoryMock() {
    ...
    return {
      ...
      async createFlashcard(data) {
        const newFlashcard = { id: (flashcards.length + 1).toString(), ...data };
  ...
  function createProgressRepositoryMock() {
    const progresses = [
      { id: '1', userId: 'u1', flashcardId: '1', status: 'new', updatedAt: new Date() },
      { id: '2', userId: 'u2', flashcardId: '2', status: 'reviewed', updatedAt: new Date() },
    ];
    return {
      ...
      async createProgress(data) {
        const newProgress = { id: (progresses.length + 1).toString(), ...data, updatedAt: new Date() };
  ...
  function createUserRepositoryMock() {
    let users = [
      { id: '1', email: 'user1@example.com', name: 'User One', role: 'user', active: true },
      { id: '2', email: 'admin@example.com', name: 'Admin', role: 'admin', active: true },
    ];
    return {
      async create(user) {
        const newUser = { id: (users.length + 1).toString(), ...user, active: true };
        users.push(newUser);
        return newUser;
      },
      async findById(id) {
      ...
      async getProgress() { return { completed: 0 }; },
      async getPreferences() { return { theme: 'light' }; },
      async getUserStatistics() { return { logins: 0 }; },
      async count() { return users.length; },
      async countActive() { return users.filter(u => u.active).length; },
      async exists(id) { return users.some(u => u.id === id); },
      async emailExists(email) { return users.some(u => u.email === email); },
    };
  }
  return {
