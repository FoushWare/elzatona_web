...
afterAll(async () => {
  ...
  for (const questionId of testDataIds.questions) {
    cleanupPromises.push(
      fetch(`${BASE_URL}/api/questions/unified/${questionId}`, { method: 'DELETE' })
        .catch(err => console.warn(`Failed to delete question ${questionId}:`, err))
    );
  }
  ...
  for (const pathId of testDataIds.learningPaths) {
    cleanupPromises.push(
      fetch(`${BASE_URL}/api/learning-paths/${pathId}`, { method: 'DELETE' })
        .catch(err => console.warn(`Failed to delete learning path ${pathId}:`, err))
    );
  }
  ...
  for (const topicId of testDataIds.topics) {
    cleanupPromises.push(
      fetch(`${BASE_URL}/api/topics/${topicId}`, { method: 'DELETE' })
        .catch(err => console.warn(`Failed to delete topic ${topicId}:`, err))
    );
  }
  ...
  for (const categoryId of testDataIds.categories) {
    cleanupPromises.push(
      fetch(`${BASE_URL}/api/categories/${categoryId}`, { method: 'DELETE' })
        .catch(err => console.warn(`Failed to delete category ${categoryId}:`, err))
    );
  }
  ...
  for (const planId of testDataIds.plans) {
    cleanupPromises.push(
      fetch(`${BASE_URL}/api/guided-learning/plans/${planId}`, { method: 'DELETE' })
