...
/**
 * Clear All Questions and Create Choice-Only Questions
 * Removes all existing questions and creates only single and multiple choice questions
 */
...
async function clearAllQuestions() {
  try {
    ...
    if (data.success && data.data.length > 0) {
      console.log(`Found ${data.data.length} existing questions to clear`);
      
      // Delete each question
      for (const question of data.data) {
        try {
          await fetch(`${BASE_URL}/api/questions/unified/${question.id}`, {
            method: 'DELETE'
          });
        } catch (error) {
...
// Main function to clear and create all questions
async function clearAndCreateQuestions() {
  try {
    console.log('🚀 Starting fresh choice-only question creation...');
    ...
    console.log(`📝 Creating ${allQuestions.length} choice-only questions...`);
    ...
    if (result.success) {
      ...
      console.log('\n🔄 Updating learning path question counts...');
      const updateResponse = await fetch(`${BASE_URL}/api/admin/update-learning-paths`, {
        method: 'POST'
      });
      const updateResult = await updateResponse.json();
      
      if (updateResult.success) {
        console.log('✅ Learning path question counts updated');
      }
...
if (require.main === module) {
  clearAndCreateQuestions().then((result) => {
    if (result.success) {
...
module.exports = { clearAndCreateQuestions };
